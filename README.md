...
- **Admin**: interests moderation (soft delete, merge, restore, default sort newest-first, category combo-box), user account management (search, suspend/unsuspend, subscription plan dropdown, setup-status chip), plan moderation, community moderation, safety/concern review, KPIs and growth-loop dashboard, QR redirects inventory. Requires `super_admin` role.
...
- **Tags**: after a plan, attendees tag each other from a fixed catalogue of 64 (Good Energy, Quick Wit, Salt Miner, Group MVP…), anonymous, three per giver per plan, collected as counts on public profiles. Internally the feature is still called kudos.
...
- **In-app notifications**: bell icon with unread state for chum, plan, join-request, and kudos notification types. Unread chat message indicators derived from per-plan read tracking.
