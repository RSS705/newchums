...
| **QR redirects** | Implemented (migration 085). Internal super-admin redirect layer at `https://newchums.com/qr/{code}` so printed QR posters can be remapped to new destinations without reprinting. Two tables: `qr_redirects` (code UNIQUE, title, destination_url, notes, is_active, created_by, timestamps) and `qr_redirect_scans` (user_agent, referer, CF-IPCountry, no raw IPs). Codes are normalized UPPERCASE and constrained to `[A-Z0-9][A-Z0-9_-]{1,63}` at both API + DB. `destination_url` is validated with the WHATWG `URL` constructor and must be `http`/`https`. Public surface is a Next.js route handler (`web/src/app/qr/[code]/route.ts`) that logs a scan via `POST /public/qr/:code/scan` and issues a 302; unknown/inactive codes gracefully 302 to `/` so posters never dead-end. Super-admin UI at `/admin/qr-redirects` (list + new dialog) and `/admin/qr-redirects/[id]` (detail + edit + delete + recent scans). See **Technical_Specs.md → QR Redirects** for the full contract, including security invariants around destination-URL validation. | Preserve destination-URL validation (scheme + parseability) and the UPPERCASE code normalization. Do not add raw IP logging without a privacy review. |
