...
| Add Plan / Edit Plan form parity | `AGENTS.md` → Add Plan / Edit Plan Parity Rule; both form files (`CreateEventClient.tsx`, `EditEventClient.tsx`); shared section components and collapsed-header summaries under `web/src/components/events/planForm/` |
...
| **Post-plan wrap-up** | Implemented (July 2026 rebuild; replaced the rating-grid feedback form). `PlanWrapUp` (`web/src/components/events/PlanWrapUp.tsx`) mounts on past attended plans (`?section=feedback` deep-link key kept for old emails). Attendees get "Anyone earn a tag?": per-person fixed-tag picker ("tags" to users, kudos internally; anonymous, three per giver per plan, 201-tag catalogue in `api/src/lib/kudos.ts` with search, replaced text shout-outs Sept 2026), Save to Chums, Message, no submit gate, 7-day window. Hosts get the same thing under "Give tags and save chums for next time!" (Rob, 2026-09-12: the card is about staying in touch, not about collecting data on how the plan went), with the binary Came/No-show list of committed attendees still there writing host-only `attendance_issues` rows, plus a prominent "Run it again" into `?copy_from=`. A recipient can hide any one of their tags from their public profile (`kudos_hidden_tags`, migration 125). API: `GET /events/:id/wrap-up`, `POST /events/:id/wrap-up/dismiss` (historical `plan_feedback_dismissals` table), `POST`/`DELETE /events/:id/attendance-issue`. Role-varied wrap-up email (`planWrapUp_host` / `planWrapUp_attendee` subjects) on the hourly cron, ~3h after start, deduped on the historical `events.feedback_email_sent_at` column; pref key stays `feedback_requests` (stored-JSON name, renaming would reset saved choices). Server analytics: `plan_copied` product event when a create was hydrated from a previous plan. | Nothing in the host check-in may notify anyone. Keep the attendee half gratitude-shaped: no rating grids, no per-person questions (see Motions). |
...
- **Two-tier structure (Aug 2026).** Both forms present the same two tiers. Tier one is a single always-open card with the minimum viable plan: **Title, Date + Time, Where (in person / online + venue or link), Seats**. Everything else is a collapsed section rendered through the shared `CollapsibleSection` wrapper, in this order: **Description, Banner, Hobbies, Alternate times, Who can see this?, Extra options, Community, QA plan (super admin, not collapsed), Submit.** Each collapsed header shows a one-line summary of the section's current value ("Visibility: Public" style); summaries for the client-rendered sections come from `planForm/summaries.ts`, and the shared sections compute their own. Keep new summary copy in those places so Add and Edit cannot drift.
- **Every collapsed section must keep a safe default**, because skipping all of them must always publish a complete plan. That is why hobbies became optional end to end in Aug 2026 (POST /events and PATCH /events/:id no longer require one; `events.interest_id` was always nullable), why the RSVP auto-cancel threshold lives inside Extra options, and why the exact-location select lives inside "Who can see this?" rather than tier one. A validation error inside a collapsed section force-opens it via `SECTION_OF_FIELD` + `revealAndScrollToErrors` in each client; a new validated field in a collapsed section must be added to that map.
- The drift-prone pieces are **extracted** into shared components under `web/src/components/events/planForm/`: `CollapsibleSection` (collapse chrome + header summary line), `ExtraOptionsSection` (now including the minimum-attendees auto-cancel threshold), `CommunityLinkSection`, and `QAPlanSection`. Both forms import and render the same components; those sections cannot drift visually. `CommunityLinkSection` is a single unified render: multi-select Autocomplete + members-only toggle. Add and Edit pass the same prop shape (`myCommunities`, `selectedCommunityIds`, `hideFromExplore` + change handlers). Re-parenting a plan on edit is supported; server-side membership validation fires for any newly added community linkage, but is skipped when only existing linkages are reused.
- The remaining sections (tier-one fields, Banner crop, Description, Hobbies, Alternate times, Who can see this?, Submit) are still duplicated between the two files. They have been stable in practice, so the extraction cost has not been spent yet. Extract them when and if they drift, or as part of a dedicated scope.
