...
| Subscription plans, premium access, Community Pro inheritance | `AGENTS.md` → Organizer Plans, Community Pro, and Module Gating; `docs/Technical_Specs.md` → section 5 (Organizer subscription plans); `docs/System_Map.md` → section 3 + Community premium direction; Super Admin System Logic page (Subscription plans); `api/src/lib/subscriptionAccess.ts` |
...
## Community Operating Hours and Banner (Community Pro)
...
**Banner.** Community Pro only. Uses the shared media upload pipeline with purpose `community_banner`; object key `community_banners/{userId}/{timestamp}.{ext}`, 1600x400 WebP target, max 600KB. The Pro gate is enforced in `POST /media/finalize` against the **community owner's** subscription plan (via `hasCommunityProAccess()`), super admins bypass so they can manage on behalf of an owner. Non-Pro owners see **no** banner-upload UI at all (no locked control, no upgrade nag) in both Create Community and Edit Community. `viewerHasProBannerAccess` is returned on `GET /communities/:slug` (owner/admin only) for the Edit form to gate the uploader. The banner can be cleared via `PATCH /communities/:slug` with `{ banner_key: null }`; setting a key is only accepted through `/media/finalize` so the gate can't be bypassed.
...
- The four historically drift-prone sections are **already extracted** into shared components under `web/src/components/events/planForm/`: `ExtraOptionsSection`, `CommunityLinkSection`, `MatchingPreferencesSection`, and `QAPlanSection`. Both forms import and render the same components; the sections cannot drift visually. `CommunityLinkSection` is a single unified render: dropdown + members-only toggle. Add and Edit pass the same prop shape (`myCommunities`, `selectedCommunityId`, `hideFromExplore` + change handlers). Re-parenting a plan on edit is supported; server-side membership validation fires only when the linkage actually changes to a *different* community.
...
| Banner / hobby / community initial state | Edit hydrates from `GET /events/:id`; Add starts empty (or pre-selects from `?community_id=` on Create) | Inherent to edit-vs-create. |
| Community PATCH diff semantics | Edit only omits `community_id` / `hide_from_explore` from the PATCH body when unchanged | Prevents the server's "must be a member" validation from re-firing on a no-op save for a host who has since left the linked community. |
...
3. **Always** inspect `template_reference/` before implementing a new UI view.
4. Prefer modifying shared components (`web/src/components/`), layouts, and theme.
5. Keep changes minimal and consistent with template patterns.
6. When in doubt: copy a template component and adapt it rather than inventing new structure.
