...
| **Communities** | Implemented. Community pages with create, browse, join/leave, member management, community plan feeds, share tokens for private communities. Schema: migration 055 (communities, community_members, community_join_requests + community_id/hide_from_explore on events). Email templates for join request/approved/declined (Postmark IDs pending). Super admin moderation. Community chat deferred (`chat_enabled` column exists but no chat implementation). | Polish UI, fix bugs. Do not build community chat without being asked. Postmark template IDs need to be configured once templates are created. |
