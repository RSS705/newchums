...
| **Plan feedback / matching quality** | Phase 1 implemented (migration 049). Post-plan feedback UI on event detail page for past plans. Hidden metric foundations: reliability, sociability, presentation, hosting_skills, match_quality. Separate attendance issue and conduct/safety reporting. Post-plan feedback reminder email via cron (3h after plan start). | Phase 1 captures data only. Do not implement metric aggregation, chum preference filtering, or plan-level strictness without being asked. Future: chum preference thresholds, compatibility filtering, plan eligibility rules. |
