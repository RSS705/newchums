...
NewChums is introducing organizer-facing subscription planning **before** billing exists. The goal is to define durable product boundaries now, wire future features to those boundaries, and continue assigning access manually while the product is still in validation.

**Current product plans:**
- **Free**, baseline user and community functionality
- **Super Host**, advanced **plan / event-level** functionality for a user wherever they host
- **Community Pro**, advanced **community-level** functionality for communities owned by that user, and it **includes Super Host benefits**

**Current access model:**
- Plans are assigned at the **user level**, not sold through billing yet.
- A user with **Community Pro** grants Community Pro functionality to the communities they own.
- Community Pro should be treated as covering **up to 5 owned communities** for now; this is a product rule intended to prevent unusual overuse while the system is early.
- There is **no separate Founding Access system** right now. Early pilots are handled by manually assigning **Super Host** or **Community Pro** through internal admin controls.
...
- The backend should resolve effective access from the user's assigned plan and any temporary internal overrides.
...
| **24-hour attendance check** | Fully implemented. User-facing name: "24-hour attendance check". Host-configurable confirmation window, reminders, cutoff processing, fallback policies (proceed/notify host/auto-cancel), in-app and email confirmation flows. Migrations 028, 039. Cron-based lifecycle. Internal field: `require_reconfirmation`. Guest attendees (user_id IS NULL, guest_email set) receive confirmation emails and their confirmations count toward `min_confirmed_attendees` on equal footing with registered users; auto-cancel emails also go to guests. | Operational. Use "24-hour attendance check" in all user-facing copy. Polish and enhance as needed. **Guest counting invariant:** attendee-counting SQL for plan viability/cancellation MUST treat rows with `user_id IS NULL` as attendees. Use `er.user_id IS DISTINCT FROM e.host_user_id`; a plain `user_id != host_user_id` silently drops guests (NULL-compares) and must not be used. |
