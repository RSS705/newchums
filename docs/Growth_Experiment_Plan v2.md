...
`utm_content` per creative, its string identical to the ad's name inside Meta so Ads Manager and
the Growth tab cross-reference by eye; each becomes its own funnel row. Plus Meta campaign setup
per §7 and the §8 interview emails during the run.
...
- **The four creatives, decided 6 Aug** (`utm_content` = ad name in Meta): `pain-groupchat`
  (coordination pain), `promise-confirms` (turnout promise), `occasion-gamenight`
  (occasion-specific), `plain-descriptor` (control — a plain statement of what the product is, so
  the hooks have something to beat).
- **Round-2 creative, parked deliberately: parents and kids' birthday parties.** The sleeper
  audience found while choosing targeting (6 Aug). Parents organizing kids' parties have the exact
  grievance this product answers — "nobody RSVPs anymore" is a genuine cultural complaint —
  headcount drives real money (cake, loot bags, venue minimums), and the guest list is other
  parents who will never download an app, which is what the one-link flow is for. Strong enough to
  deserve its own hook ("Nobody RSVPs to kids' parties anymore. Fix that.") and probably its own
  ad set, since it pairs with a demographic (*Parents with children aged 4-12*) rather than the
  round-1 interest stack. Deliberately NOT in round 1 — a fifth creative aimed at a different
  audience would add an axis to a $50 cell.
- **Audience: occasion interests that imply participation, not fandom** (decided 6 Aug). Interest
  targeting measures what people consume, so prefer interests where affinity means *doing*: Board
  games, Tabletop role-playing games, Dungeons & Dragons, Magic: The Gathering, Poker, Book clubs,
  Dinner party, Potluck, Barbecue — plus the *Parents with children aged 4-12* demographic. Avoid
  broad sports interests (they target fans on couches, not the person organizing Tuesday skates)
  and "Event planning" (professional planners, not our user). **Canada only, English, 25-55** —
  one market per read: mixing countries lets delivery chase the cheapest clicks and blends CPMs
  into an unreadable average. A second market (likely the US) joins as its own ad set in a
  post-green-light round. The ad copy does the organizer-selection that targeting cannot — the
  creative's job is to make the herder, not the attendee, feel seen.
