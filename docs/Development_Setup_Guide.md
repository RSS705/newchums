...
- **Notifications:** In-app bell with unread state for chum, plan, join-request, and kudos notification types. Unread chat indicators derived from per-plan read tracking. 14 email notification types with per-type Settings toggles and tokenized email unsubscribe links.
...
- **Admin:** Interests moderation, user account management (super_admin only), plan moderation, community moderation, safety/concern review, KPIs and growth-loop dashboard, QR redirects inventory.
...
- **Tags (kudos internally):** After a plan, attendees tag each other from a fixed 106-tag catalogue (anonymous, three per giver per plan, 7-day window); counts collect on public profiles. Replaced text shout-outs in Sept 2026.
...
- `kudosReceived`, `concernReportAlert`
...
Chunk 27, 2026-07-31
- Goal: remove the reputation-and-matching system and rebuild the post-plan moment around the product that works (post a plan, share a link, see who is coming). Owner-directed, staged plan with a Stage 0 read-only challenge phase; four deploys plus one migration.
- Why: chum_preferences defaulted everyone to reliability_level='preferred', which compiled into the Explore WHERE clause (thresholds 35/45/55 hardcoded in SQL) and both digest directions, while user_metrics pinned essentially every account at 50.00 because the rating form was rarely completed. The only live behaviors were a "Required" setting silently hiding the whole userbase and a single no-show report silently dropping someone below "Important" everywhere. The form itself asked ~30 questions on a ten-person plan and gated shout-outs behind submission.
- Stage 1 (deploy 1, the deadline checkpoint): PlanWrapUp replaced PlanFeedback. Attendees get "Say thanks" (shout-outs, Save to Chums, Message; no gate; 7-day window); hosts get a private binary Came/No-show check-in plus "Run it again". GET /events/:id/wrap-up replaced the feedback GET; the feedback POST and all three scoring functions died; dispute and admin resolution became plain status writes; role-varied wrap-up email (planWrapUp_host / planWrapUp_attendee subjects); give_first_feedback objective became send_first_shoutout; copied_from -> plan_copied product event + funnel plansCopied. Also fixed a stale-load race (strict-mode double mount fetch could revert an optimistic checklist toggle; loads are sequence-guarded and never overwrite a touched checklist).
- Stage 2 (deploy 2): matching excised end to end. Explore SQL filter + advisory JS pass, community-feed flag, plan-detail notes, digest post-filter (candidate selection was already hobbies + travel radius from user_interests/user_profile, so the digest survives untouched otherwise), GET/PUT /chum-preferences, PUT /admin/users/:id/metrics, chumPreferences lib + tests, matching-only loaders incl. batchLoadDobs (all four callers were pref paths), pref_overrides read/write in both plan forms, admin diagnostics panels, hard-delete impact counts. Feed-contract conditions in Explore byte-identical.
- Stage 3 (migration 107, applied test-first then prod): drop plan_feedback, user_metrics, chum_preferences, events.pref_overrides, attendance_issues.confidence/applied_penalty; clear orphaned give_first_feedback completions. Kept: attendance_issues with status (dismissed is the only path that restores public credit), is_host_report, and plan_feedback_dismissals (historical name, now wrap-up dismissals). CSV snapshots of all dropped data taken to /home/rob/newchums-backups/2026-07-31-matching-removal/ before applying. Public attendance record verified byte-identical before/after on the isolated DB and production.
- Stage 4 (deploy 3): blocking extended product-wide via the renamed pairBlocked helper. Gates: Explore/community feed/digest branches, plan detail + RSVP + join-request (existence-hiding NOT_FOUND, QA-gate shape, super admins exempt on detail), invite route (folds into alreadyInvited; inviter-pair OR host-pair; raw emails covered post-resolution) and both creation loops (silent continue), notifications (null-actor system rows always deliver), chat/alt-time fan-outs, people search (email path answers with the unknown-address shape to avoid the inviteEligible inversion), shout-outs (SQL pair filter works for anonymous viewers; compose refused as RECIPIENT_NOT_PARTICIPANT; approval skips the notification), wrap-up people list. Deliberate non-filters commented: third-party plan roster, plan-chat visibility, anonymous feeds. Block/Unblock added to public profiles (viewerHasBlocked); Settings/inbox copy broadened. AGENTS.md feed contract updated in the same commit.
- Stage 5 (deploy 4): copy/docs sweep. how-it-works lifecycle step 6 + trust + discovery sections reframed (15 claims), landing line, privacy loses the word "reputation" (TSX + docs/legal mirror), AGENTS.md rows (attendance, post-plan wrap-up) + two Motions entries (matching removal; host-authored post-plan questions declined) + parity-rule lists, Technical_Specs and System_Map rewritten per the Documentation Contract, db-cleanup scripts stripped of dropped tables, 15 unreferenced images/home files deleted (community-hero.png kept as the documented WebP source).
- Isolated-DB note: the legacy pre-chain tables (interests, user_profile, user_interests) live in the newchums schema, not public, and the prod role carries search_path=newchums,public at role level. The catalog-DDL generator in this chunk's harness targets newchums accordingly; psql -tA output needs splitlines() (a trailing empty field's tab is eaten by strip()).
- Verification: five-persona wrap-up matrix (host / attendee / reported / non-participant / logged out) at 390/1280 with a UI-driven no-show toggle write + retract; zero notifications and zero emails from the host check-in; role subjects + pref opt-out + maybe-exclusion on the cron; plan_copied recorded once with junk sources ignored; attendance record byte-identical across the migration; badges cron and digest run clean post-migration; two-account blocked matrix in both directions (feeds, digest with a non-blocked host's plan still delivering, detail/RSVP/join 404s, handle + raw-email + third-party invites folded into alreadyInvited with zero rows, search shapes indistinguishable, notifications keep system rows only, shout-out disappears for every viewer class and returns on unblock, profile Block/Unblock roundtrip with Message correctly gone).

---

Chunk 26, 2026-07-28
- Goal: full review of all three signup paths (Google, email + password, invitee via share/invite link) after the recent funnel, OTP, consent and schema changes. Two real bugs found and fixed, plus two owner-requested refinements.
- Changes:
  - **Bug, invite adoption never worked.** Both sites that adopt an email-only invite onto a new account (`POST /auth/signup` and `GET /events/:id`) set `user_id` without clearing `email`, which violates the `event_invites_single_identity` CHECK from migration 075. Both writes sit in a non-fatal try/catch, so the violation was swallowed and the invite was never linked. Someone invited by email therefore stayed unlinked: on an `invite_only` plan they depended entirely on still holding the `invite_token`, and their "Invited" status and any `reserve_seats` accounting keyed on `user_id` were wrong. Both statements now `SET user_id = ..., email = NULL`, the same normalization migration 075 applied to existing rows. Existing affected rows self-heal on the next plan-page visit (1 such row in production at the time of the fix).
  - **Bug, Google signups were missing from the funnel entirely.** `POST /auth/email-verify/mark-oauth` recorded `signup_completed` only on the `email_verified_at` NULL to set transition, but a first Google sign-in has its account row created by the web layout's `getOrCreateAppUser`, which already sets `email_verified_at`. The UPDATE therefore matched zero rows and no event was ever written, so the admin Funnel's "Signups" undercounted by every Google signup. The endpoint now reads the row first and records when the account was created within `SIGNUP_COMPLETED_GRACE_MS` (1 hour) or when the call itself verified an existing unverified account. The partial unique index keeps it to one row per user; the recency window stops a long-standing account signing in today from backdating a signup.
  - **Code lifetime 10 to 30 minutes** (`PLAN_SIGNUP_CODE_EXPIRY_MS`), with the email copy and a new test that asserts the constant and both templates stay in step. Ten minutes was tight for someone who opens the invite on a phone, switches to a mail app and gets interrupted.
  - **Space under the code input reduced.** The 20px status line was reserved unconditionally, so the untouched state carried an empty band. It is now reserved as soon as the field is focused or has digits, keeping the no-layout-jump property where it matters while removing the dead space. Measured input-bottom to Resend-top: 40px before, 16px after; card 24px shorter at both widths.
- Verification: isolated database built from the full chain, plus API worker, web dev server and a local email sink. All three signup paths driven end to end with database assertions: email+password (acceptance recorded with pinned versions, `signup_completed(password)` on verify), Google (row created by the layout-shaped insert, `mark-oauth`, `signup_completed(google)` now recorded, idempotent across repeat calls, and a 30-day-old account confirmed NOT to backdate), invitee (request with no consent flag, 30-minute TTL confirmed in the token row and the email copy, code verify, grant exchange, `signup_email_sent` + `rsvp_verified` + `signup_completed(plan_signup)`), existing-account branch, invite-email adoption (now linked, email cleared), magic-link fallback, and the 5-attempt cap followed by a dead code. Funnel endpoint confirmed reporting all three methods. Layout-jump check: 0px shift when the error message appears.
- Deploy: API worker + web worker. No migrations.
- Open item for the owner: 3 of 4 Google-OAuth accounts in production predate the funnel fix and have no `signup_completed` row. The fix is forward-only by design (the 1-hour window prevents backdating), so those three remain absent unless a deliberate backfill is run.
...
- Changes:
  - **Web, `PlanSignupCard.tsx` (presentation only):** intent action area constrained to `maxWidth: 560` with equal-width side-by-side buttons on desktop and full-width stacking on mobile; the three tertiary actions ("Continue without answering", "Wrong email? Edit", "Resend code") converted from the theme's filled peach text-button treatment to the quiet transparent style used by the PlanFeedback footer, with the resend button visibly disabled while counting down and enabled (primary color) at zero; code-entry state unified into one centered column (chips centered, input centered at `maxWidth: 340` via an explicit flex wrapper because auto margins lose to the parent Stack's stretch, digits at 1.5rem / 0.45em tracking); the two captions under resend merged into one lighter line; long email addresses now wrap (`wordBreak`) in both the code and existing-account panels; hardcoded `#fff7ed`/`#ffffff` gradients replaced with palette-derived tokens so the card stays readable under the dark palette; icon colors moved to `primary.contrastText`. Input mechanics untouched: single input, `inputMode="numeric"`, `autoComplete="one-time-code"`, paste, auto-submit at 6 digits, `aria-live` errors, visible label. No analytics or accessibility attributes changed.
  - **DB migration 105:** `newchums.admin_audit` (actor, action, subject type/id/label, JSONB detail, created_at; no FKs so rows survive the subjects they describe).
  - **API, `src/lib/adminHardDelete.ts` (new):** impact counting + single-transaction cascades whose FIRST statement writes the audit row. FK graph enumerated from the live catalog (not assumed) and documented in the file header. Explicit statements cover `product_events` (SET NULL by design, so test cleanup deletes them outright), owned communities (NO ACTION blocker: deleted), and the two moderator-reference columns (nulled).
  - **API endpoints (all `requireSuperAdmin`):** `GET/POST /admin/users/:id/delete-impact|hard-delete`, `GET/POST /admin/plans/:id/delete-impact|hard-delete`, `GET /admin/events/:id/chat-transcript`. Typed confirmation (username, `@`/case-insensitive; or exact plan title). Self-delete and super-admin targets refused server-side. No email helper is reachable from any of these paths. Post-commit the per-plan ChatRoom DO is purged (new `/purge` route: close sockets + `storage.deleteAll()`).
  - **API, `GET /events/:id`:** adds an `adminView` block (exact location incl. lat/lng and online link) ONLY on super-admin responses; normal-user redaction paths untouched.
  - **Web:** shared `AdminHardDeleteDialog` (impact list + typed confirmation) wired into `/admin/plans` rows, `/admin/chums` rows (disabled for super admins), and a Danger zone on `/admin/chums/[id]`; new `AdminPlanPanel` on the plan detail page rendering the labeled "Admin view" location block and the read-only transcript dialog.
  - **Documentation:** Technical_Specs section 8 (new admin subsection: cascade, audit, KPI semantics, moderation posture), section 11 (migrations 105 + product_events deletion wording); AGENTS.md moderation posture next to the DM no-browse note and the analytics-invariant amendment.
- Verification: isolated `newchums_admtest` DB + local worker + email sink. (a) user cascade removed the user, their hosted plan and its RSVPs/chat/confirmations, their chat on other plans, owned community, DM conversations, tokens, and all `product_events` rows for both the user and their plans; (b) plan cascade removed RSVPs, confirmations, chat, chat_reads, and funnel rows; (c) funnel endpoint before/after confirmed the cleanup (verified 1 to 0, rsvpRecordedUsers 2 to 0, signups 1 to 0, firstPlans 2 to 1, plansReached3Rsvps 1 to 0); (d) email sink line count identical before and after both deletions (zero emails); (e) all five new endpoints return 403 for a normal user and unauthenticated; self-delete and super-admin-target refused at both preview and execute, wrong confirmation text rejected; (f) admin transcript view left `event_chat_reads` byte-identical and added no rows, while logging exactly one `plan_chat_transcript_viewed` audit row (pagination does not re-log). `adminView` present for super admin, absent for normal users and logged-out; approximate-location plan showed "London, ON" to a normal user and the exact venue/address/coords to the admin. Playwright: invitee card at 390px and 1280px in light and the dark palette (intent, form, code, code-error, existing-account) plus admin plan panel, transcript dialog, and hard-delete dialog at both widths. API vitest 68/68, wrangler dry-run clean, web tsc/vitest/build clean, eslint zero new findings on touched files.
- Deploy: Run migration 105 against production DB first, then deploy API worker, then web worker.
- Notes: the app ships light-mode only (`buildTheme("light")` is hardcoded in `web/src/theme/index.ts`); dark-palette verification was done by temporarily switching that token in dev and reverting. The plan hero card and some existing surfaces do not yet invert cleanly under the dark palette; out of scope here and not fixed.

---

Chunk 23, 2026-07-27
- Goal: B1, intent-first invitee onboarding with inline one-time-code verification: an unauthenticated invitee on a plan link states Going/Maybe first, verifies their email with a 6-digit code without leaving the page, and lands RSVP'd with no extra tap. Magic link kept as an equal fallback in the same email.
- Changes:
  - **DB migration 104** (additive only): `email_verification_tokens` gains `otp_hash`, `otp_attempts`, `event_id` (FK SET NULL), `signup_intent` (CHECK going/maybe); `token_hash` semantics untouched, both credentials share one row. New `newchums.auth_rate_events` table backing the Postgres sliding-window rate limiter.
  - **API:** `src/lib/planSignupOtp.ts` (rejection-sampled 6-digit codes, TTL/cap/cooldown constants), `src/lib/dbRateLimit.ts` (atomic INSERT-if-under-limit, read-only variant + explicit record for success-charged cooldowns; the KV limiter is a production no-op, repo-wide adoption tracked as A3). `POST /auth/plan-signup/request` gains `intent`, mints code + link on one row (10-min TTL), swaps KV limits for Postgres (10/IP, 3/email per 10 min, 45s cooldown charged only after a successful send), fires first-party `signup_email_sent`. New `POST /auth/plan-signup/verify-code` (atomic attempt counting, 5-guess cap then invalidate, stale-superseded codes answer CODE_EXPIRED without burning attempts, uniform responses for unknown account vs no active code, suspended users never burn the credential, success mints a 5-minute single-use session grant consumed through the EXISTING magic-link provider so `/auth/magic-link/consume` semantics stay untouched and product events fire exactly once). New `POST /auth/plan-signup/resend` (no Turnstile, shared issuance buckets + cooldown, uniform ok, last-issued-wins). `GET /events/:id` exposes `viewerPendingIntent` (crash recovery, only while no RSVP row exists). RSVP rows remain created exclusively by `POST /events/:id/rsvp`.
  - **Email:** `magicLinkSignup` template rewritten code-first (code in a display box plus the one-tap button; link-light; 10-minute copy); subject is now "{{otpCode}} is your NewChums code". Resend dev-mode logging no longer prints subjects so codes never reach logs. `RESEND_BASE_URL` optional binding added as a test seam (unset in production).
  - **Web, `PlanSignupCard.tsx`:** full state machine rework: intent stage (Going / Maybe primary, quiet continue-without-answering), form stage (intent chips, existing email + DOB + legal + Turnstile), code stage (single input, `inputmode=numeric`, `autocomplete=one-time-code`, paste-friendly, auto-submit at 6 digits, `aria-live` errors, distinct wrong/expired/capped states, resend with live countdown, "Wrong email? Edit" remounting Turnstile), existing-account panel unchanged in behavior. On success: signIn via the grant, then hard navigation per the post-auth rule.
  - **Web, `EventDetailClient.tsx`:** new guarded `?intent=` auto-apply effect (authenticated, no existing RSVP row only, host/canceled excluded, once per mount) fed by the URL param or the server-persisted `viewerPendingIntent`; the legacy `?rsvp=` CTA path keeps its unguarded overwrite semantics (and still carries `cant_make_it`). Card receives `initialIntent` from Going/Maybe email CTAs.
  - **Analytics:** GA `rsvp_intent_selected` (status), `rsvp_otp_entered` (result ok/wrong/expired/capped), `rsvp_code_resent`; first-party `signup_email_sent` (kind initial/resend). `rsvp_verified` / `signup_completed` fire on the OTP path through the same NULL-to-set guard as the link path.
  - **Documentation:** Technical_Specs section 8 (Lightweight plan signup rewritten, new endpoints), section 11 (migration 104), section 12 (catalogue rows + funnel reading note); AGENTS.md flow descriptions updated in the guest-RSVP guardrail and the 24-hour-check row.
- Verification: API vitest 68/68 (new suite: code generation, intent parsing, template + subject render incl. no-em-dash check); wrangler dry-run clean; web tsc, vitest, production build clean; eslint on touched files reports only the pre-existing EventDetailClient baseline. Isolated `newchums_b1test` database (full migration chain incl. 104 ran clean) + local worker + local email sink (`RESEND_BASE_URL`): all seven scenarios scripted end to end and passing: (1) new-email happy path incl. `signup_email_sent`/`rsvp_verified`/`signup_completed` once, grant consume without event double-fire, `viewerPendingIntent` surfacing then suppression, `rsvp_recorded` on the canonical RSVP endpoint; (2) existing-account branch (planSignin email, zero token rows, intent preserved in next); (3) expired code; (4) wrong code x5 to cap then dead row, resend recovery; (5) Maybe picked, Going applied at RSVP time (client authority), final status going; (6) magic-link fallback consuming the same row (code dead afterwards, events exactly once); (7) two-device last-issued-wins (old code answers CODE_EXPIRED without burning new-code attempts, fixed during testing). Regressions: signed-in RSVP unchanged (third non-host going fired `plan_reached_3_rsvps` once), legacy signin-link consume produced zero events, verify per-IP cap returned RATE_LIMITED on the 31st guess, QA plan produced zero product events, per-event rows == distinct users for all once-per-user events. Playwright pass over the real UI (web dev + local API + sink, Turnstile absent locally): intent stage, form with chips, code stage with countdown, wrong-code inline error, full success path ending authenticated with the auto-applied Going RSVP and "You're going!" toast, existing-account panel; six screenshots captured.
- Deploy: Run migration 104 against production DB first, then deploy API worker, then web worker.
...
- Changes:
  ...
  - **API, PATCH /events/:id banner removal fix (pre-existing bug):** a body of exactly `{ banner_key: null }` is now handled as a banner-only partial update that clears the banner and returns early. Previously the Edit form's banner-removal follow-up PATCH always failed the unconditional title/starts_at validation with a swallowed 400, so "Remove photo" + Save silently kept the banner. Clearing remains the only banner mutation PATCH accepts; setting a key still goes exclusively through `/media/finalize`.
...
Chunk 19, 2026-07-14
- Goal: Copy-a-plan QC fixes (banner carry-over bug) plus plan-form polish from the same review.
- Changes:
  - **API, CORS middleware:** Root-caused the copy-a-plan banner not pre-populating: media routes stream from R2 via raw `new Response(...)`, which bypasses Hono's prepared headers, so `GET /events/:id/banner` carried no `Access-Control-Allow-Origin` and the browser blocked the copy flow's `fetch()` (while `<img>` tags rendered fine). The middleware now re-applies CORS onto `c.res` after `next()` for allowlisted origins (skipping 101 WebSocket upgrades, whose headers are immutable). Covers all raw-Response media routes.
  - **Web, CreateEventClient.tsx:** Banner copy fetch uses `cache: "no-store"` so day-old browser-cached banner responses (stored without CORS headers before the API fix) are bypassed. "Copy a previous plan" header button slimmed down (smaller font/icon, tighter padding, no min-height) so it reads as a quiet link rather than a bulky button.
  - **Web, CopyPlanDialog.tsx:** Subtitle now warns the pick replaces anything already entered in the form.
  - **Web, ExtraOptionsSection.tsx + both plan forms:** "Let Going attendees invite others" (default on) inverted to "Prevent attendees from inviting others" (default off) with a help tooltip, so every Extra-options toggle defaults to off. Presentation-only: the DB column and wire field remain `allow_attendee_invites` (default true), forms send `allow_attendee_invites: !preventAttendeeInvites`, and copy/edit hydration maps `ev.allowAttendeeInvites === false`. No migration; existing plans and the `POST /events/:id/invite` permission check are untouched.
  - **Web, HobbyPickerField.tsx + both plan forms:** New optional `helperText` prop (error still wins). Plan forms pass "People nearby who share these hobbies may get notified about this plan, depending on who can see it." so hosts learn the hobby field drives match notifications. Other HobbyPickerField consumers (profile, onboarding, communities) are unchanged.
- Verification: `npm run build` passes; changed files lint clean.
- Deploy: API worker (CORS fix) + web worker.

---

Chunk 18, 2026-07-14
- Goal: Let hosts copy a previously used plan into a pre-filled create form (weekly re-runs).
- Changes:
  - **Web, CreateEventClient.tsx:** New `?copy_from=<planId>` mode. Fetches `GET /events/:id`, requires `isHost`, and pre-fills every form field: title, description, hobbies, seats, min attendees, location, visibility, alternate-times mode, extra options, matching-preference overrides, QA flag, and community linkage (intersected with current memberships; `hide_from_explore` carried only when linkage survives). Date/time advance to the next future occurrence of the source weekday at the same time of day. Banner image is fetched and re-staged through the normal media upload pipeline (best-effort). Availability deadline and attendees are not copied. Loading spinner while hydrating; hero subtitle notes the pre-fill; failure falls back to a blank form with a toast. No API changes.
  - **Web, EventDetailClient.tsx:** "Copy plan" button (EventRepeat icon) added to the host actions card, never disabled by the edit lock. The card now also renders for canceled hosted plans (Copy only, with a short explainer), so a canceled plan can be re-run.
  - **Web, CopyPlanDialog.tsx (new):** In-form entry point so users don't have to find Copy on old plan pages. A quiet "Copy a previous plan" action in the Start a plan header opens a picker dialog listing the viewer's hosted plans from `GET /events/mine` (past first, then upcoming; canceled included with a chip; client-side search appears at 7+ plans; skeleton loading, retry, and encouraging empty states). Selecting a plan hydrates the form in place through the same copy path, behind the same full-page spinner so mount-initialized controls (rich text editor, pickers) re-initialize cleanly. Hydration now resets the form before applying, so copying over a half-filled form stays faithful. Documented as the "Entity picker dialog" pattern in docs/UI_Patterns.md.
  - **Documentation:** AGENTS.md parity-table row for the Add-only copy hydration; System_Map.md route table + logged-in flow; Technical_Specs.md web-pages table; UI_Patterns.md new "Entity picker dialog" entry.
- Verification: `npm run build` passes cleanly.
- Deploy: Web worker only; no migrations.
