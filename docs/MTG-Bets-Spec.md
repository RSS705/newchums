# MTG Prediction Challenge — Game Design & Build Spec
...
This document records the final decisions for MTG Prediction Challenge, a card-prediction game inside NewChums. Each section states what we decided first and then why, so it still makes sense when reread months from now.
...
- The game is called **MTG Prediction Challenge** — the name players pick from the Specialized Community dropdown.
...
- An **MTG Prediction Challenge is a NewChums community.** The Create Community form (newchums.com/communities/create) gets a *Specialized Community* option with a dropdown, and choosing *MTG Prediction Challenge* creates one (section 3).
...
- **Picks lock the night before the set launches on MTG Arena.** For Reality Fracture, that's Monday, September 28, 2026 at 11:59 PM ET.
- **Standings update once a day** from the morning after the Arena launch.
- **The season ends 28 days after the Arena launch.** For Reality Fracture, the final standings are taken on Tuesday, October 27, 2026.
...
## 3. Where MTG Prediction Challenge lives in NewChums

**Decision:** an MTG Prediction Challenge is a NewChums community with a special type. The Create Community form at newchums.com/communities/create gets a **Specialized Community** option with a dropdown, and choosing **MTG Prediction Challenge** creates the community. Throughout this document, "group" means one of these communities.

**How it works**

- The Create Community form works as it does today, with one addition: a *Specialized Community* option and a dropdown of available types. For now the only type is *MTG Prediction Challenge*. Leaving the option off creates a normal community.
- The type is set when the community is created and can't be changed afterwards, so normal communities are never affected.
- A specialized community's home view is the challenge (section 10.2) instead of the normal community home: the Plans, Announcements and other tabs are replaced entirely, while the community header (name, picture, members, public or private label, Share, Start a plan and the owner's Edit) stays. Everything else — members, join requests, public or private visibility, owner settings, moderation and Plans — works exactly as it does for any community.
- Specialized communities appear in NewChums' community directory like any other community, labelled "MTG Prediction Challenge," so public ones can be found and joined there.
...
- NewChums communities have no invite links and no member chat. A private community is reached by its address (outsiders see a restricted preview and can request to join), the owner controls entry through the join mode (open or approval required), and pre-lock discussion happens through the community's Discord or WhatsApp links, which stay in the header. This is acceptable for season one.
...
- **Previews run from Tuesday, September 8 to Thursday, September 17, 2026.** Wizards and content creators reveal new cards every day. The card sync picks them up from Scryfall, and they appear in the pick screens automatically.
- **The full card list arrives on Friday, September 18, 2026,** when Wizards completes its card image gallery. This is the target date to open picks, which gives players about ten days to make them.
- **Prerelease events run from Friday, September 25 to Thursday, October 1, 2026.** These are tabletop events at stores and produce no 17Lands data. The Season timeline shows them for context, with a link to make a Plan.
- **The lock warning email goes out on Sunday, September 27, 2026 at 10:00 AM.**
- **Picks lock on Monday, September 28, 2026 at 11:59 PM,** the night before the Arena launch. After that moment no picks can change, and the Reveal page opens.
- **The picks-revealed email goes out on Tuesday, September 29, 2026 at 9:00 AM.**
...
- **Weekly standings emails go out on Tuesdays — October 6, October 13 and October 20, 2026 — at 10:00 AM.**
- **The final standings are taken on Tuesday, October 27, 2026 at about 9:00 AM,** using 28 days of Arena data. Badges are awarded at the same time, and the season results email goes out at 10:00 AM.
- **Next season:** Star Trek releases in paper on Friday, November 13, 2026. Once it's confirmed to have Premier Draft on MTG Arena, it becomes the next season, with its dates set up the same way.
...
The timeline shows only the dates that matter to players: when previews run, when picks open, the prerelease weekend, the lock (with a live countdown), the Arena launch, the first standings, the weekly standings days, and the final day. Each entry shows the date and time in the viewer's own time zone, a one-line explanation, and a status of done, happening now, or upcoming. The current phase is highlighted. The lock and the final day each get an *Add to calendar* link (an .ics file). The welcome and lock warning emails repeat the key dates.
...
6. **Scores update every morning** from 17Lands Premier Draft data and become final on the season's last day — Tuesday, October 27, 2026 for Reality Fracture.
...
- **Stability:** the Day 1 leader ends up champion about 60–65% of the time; by Day 7, about 85%. Expect drama in week one and a settled race by week two, which is why four weeks is plenty.
...
**Decision:** MTG Prediction Challenge sends five kinds of email and no others. All of them go through Resend.
...
- Every email ends with a one-click link to turn off MTG Prediction Challenge emails.
...
5. **Season results and congratulations.** Sent at 10:00 AM ET on the final day — Tuesday, October 27, 2026 — once the final standings are in. Every player gets their final rank and points, every badge they earned during the season, their group's champion and top badge moments, and a shareable results image. The champion gets a "Congratulations, champion" version. It ends with when the next season's picks open, if that's known.
...
- **Politeness.** Requests send a descriptive User-Agent (for example, `NewChums/1.0 (MTG Prediction Challenge)`), and there's never more than one successful pull per day.
...
- **Sync.** `GET https://api.scryfall.com/cards/search?q=e%3Afra+-t%3Abasic&unique=prints&order=set` returns 175 cards per page; follow `next_page`. Group the results by `oracle_id` and keep the lowest collector number (the regular printing), so showcase and borderless versions don't double up. Keep cards with `booster = true` and a rarity of common, uncommon, rare or mythic. *As built:* during previews Scryfall's `booster` flag and `arena_id` are unreliable (146 printings on September 10 collapsed to 100 oracle cards), so until the full-gallery date every non-digital card of the four rarities counts as in the pool; from that date on, only `booster = true` cards stay in. The sync lives in `api/src/lib/mtg.ts` (`syncScryfallSet`).
...
- **Technical rules.** Scryfall rejects API requests that don't send a `User-Agent` naming the app (for example, `NewChums/1.0`) and an `Accept` header. Wait 50–100 ms between API calls, cache results for at least 24 hours, and show card images whole — never cropped, covered or watermarked.
...
- **Creating.** On newchums.com/communities/create, the organizer fills in the usual community fields, turns on *Specialized Community* and picks *MTG Prediction Challenge* from the dropdown. The new community opens straight to the challenge view; the header's Share button copies the community's address.
- **Joining.** Opening the community's address shows the community, its members and the lock countdown, with a big *Make your picks* button. Anyone without a NewChums account signs up through the normal flow and lands back on the community.
- **Finding public challenges.** Public MTG Prediction Challenge communities appear in NewChums' community directory with their label.
...
**How NEW works.** The card sync stamps each card's `first_seen_at`. For each player, set and rarity, the app stores `last_reviewed_at`. When a step opens, cards with `first_seen_at` later than `last_reviewed_at` are marked NEW, and `last_reviewed_at` updates to now — so badges stay visible for that visit and clear on the next. For Reality Fracture the full card list lands on September 18, about when picks open, so NEW badges pay off mostly from the next set onward.
...
- **Trophy case:** every player's NewChums profile gets an MTG Prediction Challenge section listing their badges across all seasons, grouped by set. It's visible to anyone who can see the profile; badges from private groups don't name the group to non-members.
...
>
> **Season dates.** Picks lock Monday, September 28 at 11:59 PM ET — the night before Reality Fracture hits Arena. Standings update daily from September 30. The season ends Tuesday, October 27; the standings that morning are final, and badges are awarded.
>
...
- **Private groups** are reached by their address only; outsiders see a restricted preview and can request to join (the owner approves), and only members see the standings and picks.
- **Joining a public group** shows a one-line heads-up: "Your picks will be visible to anyone once picks lock."
- **Sharing.** NewChums has no invite links: the header's Share button copies the community's address, and the owner controls entry through the community's join mode (open or approval required).
...
alter table communities add column specialization text;  -- null = normal community; 'mtg_prediction_challenge' = MTG Prediction Challenge
...
create table mtg_sets (
  ...
  lock_at             timestamptz not null,          -- 2026-09-29 03:59Z = Mon Sept 28, 11:59 PM ET
  arena_release_at    timestamptz,                   -- 2026-09-29 18:00Z
  tabletop_release_at timestamptz,                   -- 2026-10-02
  final_at            timestamptz not null,          -- 2026-10-27 13:00Z = Tue Oct 27, 9 AM ET
...
- `GET  /mtg/sets/:code/cards?rarity=common` → cards, plus `is_new` for the caller
- `POST /mtg/sets/:code/reviewed` `{ rarity }` → stamps `last_reviewed_at`
- `GET  /mtg/sets/:code/entry` and `PUT /mtg/sets/:code/entry` (full replace, validated by the server)
...
- **Phase 2 — about September 14 to 17: the pick flow.** Build the pick wizard, card viewer, pick tray with reordering, autosave, NEW badges, the server-side lock, the Season timeline and the How Scoring Works page.
...
- **Fixed — Tuesday, October 27: the final day.** Final standings, badge awards and the season results email.
