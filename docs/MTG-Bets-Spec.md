...
- **Whose badge it is.** The group honors, Beat the Crowd, Wire to Wire, Told You So and Lone Wolf belong to a group, so a player in two groups can earn them in each; the rest are the player's own and show in every group they're in.
- **Stacking.** Clean Sweep, Perfect Order, Sniper, Called It and Bust count once per rarity, so they stack up to ×4. Badges about particular cards (Pick of the Season, Whiff of the Season, Sleeper Agent, Told You So, Lone Wolf, Bullseye, Rock Bottom and Eats Words) are earned once and name every card that qualifies.
...
- **Group sizes.** Group honors, Wire to Wire, Lone Wolf and Told You So need three players with points in the group; Contrarian, Hive Mind, Photo Finish, Rollercoaster and Beat the Crowd need four, and so does Wooden Spoon, so last place in a group of three keeps Third Place without it. Contrarian and Hive Mind compare complete entries only (at least four of them), since an unfinished entry overlaps less by leaving slots empty, and nobody gets either when every entry overlaps the Group Mind equally. Told You So and Beat the Crowd need a Group Mind, stored at the lock or worked out as the Reveal works it out; Beat the Crowd scores its picks exactly like the leaderboard's ghost row.
