...
- The game sends **four emails** and no others: a welcome, a lock warning, a picks-revealed email, and an end-of-season congratulations (section 8).

**Badges**

- There are **43 badges** (46 until Version 22 retired the three that needed a note on a pick) in five tiers: Common, Uncommon, Rare, Mythic and a Hall of Shame. Many are awarded every season, and everyone can see them (section 7).
...
- A specialized community's home view is the challenge (section 10.2) instead of the normal community home: the Plans, Announcements and other tabs are replaced entirely, while the community header (name, picture, members, public or private label, Share and the owner's Edit) stays; a challenge community has no *Start a plan* button. Everything else (members, join requests, public or private visibility, owner settings, moderation and Plans) works exactly as it does for any community. *Version 20:* the Plans, Announcements and Schedule tabs give way to a *Challenge* tab, which comes first, while the *Members* tab and the owner's *Requests* tab stay beside it, so owners review join requests and manage members as in any community (before this, an approval-required challenge group had no way to let anyone in). `?tab=members` and `?tab=requests` open those tabs, as the join-request email expects.
...
5. **Your picks count in every group you're in when picks lock.** You can join or create groups any time before the lock. Anyone can still join a group after the lock, to follow the season and play from the next one.
...
- **Incomplete entries** still count, with empty slots at 0. Members with no picks are listed under the standings as following along.
...
2. **Runner-Up** (Uncommon). Finish second.
3. **Third Place** (Common). Finish third.
4. **Common Sense** (Uncommon). Have the highest commons subtotal in the group.
5. **Uncommon Knowledge** (Uncommon). Have the highest uncommons subtotal in the group.
6. **Rare Insight** (Uncommon). Have the highest rares subtotal in the group.
7. **Mythic Vision** (Uncommon). Have the highest mythics subtotal in the group.
8. **Pick of the Season** (Uncommon). Own the single pick that earned the most points of any pick in the group.
...
22. **Told You So.** *Retired in Version 22, with the notes on picks.* A pick you shared your thoughts on, and that wasn't in your group's Group Mind, finished in the top five at its rarity.
...
36. **Food for Thought** (Common). *Retired in Version 22, with the notes on picks.* You shared your thoughts on at least five picks. (Named Receipts on File until Version 21.)
...
45. **Eats Words.** *Retired in Version 22, with the notes on picks.* A pick you shared your thoughts on finished in the bottom quarter of its rarity.
...
- **Whose badge it is.** The group honors, Beat the Crowd, Wire to Wire and Lone Wolf belong to a group, so a player in two groups can earn them in each; the rest are the player's own and show in every group they're in.
- **Stacking.** Clean Sweep, Perfect Order, Sniper, Called It and Bust count once per rarity, so they stack up to ×4. Badges about particular cards (Pick of the Season, Whiff of the Season, Sleeper Agent, Lone Wolf, Bullseye and Rock Bottom) are earned once and name every card that qualifies.
- **Ranks.** "Finished" is the card's rank at its rarity on the day judged. A rarity counts toward rank badges only once more than ten of its cards are ranked: 17Lands leaves win rates blank under about 500 games in hand, so the first days rank a handful of cards, and "#1" among a handful means nothing. Every rarity is far past that by the final day. A card without a rank never qualifies. The bottom quarter is a rank above three quarters of the ranked cards; "drafted late" is an ALSA above the median ALSA of the rarity's cards that have one; top 5% and 25% of the Everyone board are ranks at or inside that share of its entries. The same threshold holds back badges about points while the data is thin: a rarity's subtotal honor waits for that rarity; Champion, Runner-Up, Third Place, Wooden Spoon, Photo Finish, Beat the Crowd, King of the Hill, Wire to Wire, Rollercoaster and Comeback Kid wait until at least one rarity passes it, since before then every total sits near 1,000; and Monkey Business waits until all four do.
- **Group sizes.** Group honors, Wire to Wire and Lone Wolf need three players with points in the group; Contrarian, Hive Mind, Photo Finish, Rollercoaster and Beat the Crowd need four, and so does Wooden Spoon, so last place in a group of three keeps Third Place without it. Contrarian and Hive Mind compare complete entries only (at least four of them), since an unfinished entry overlaps less by leaving slots empty, and nobody gets either when every entry overlaps the Group Mind equally. Told You So and Beat the Crowd need a Group Mind, stored at the lock or worked out as the Reveal works it out; Beat the Crowd scores its picks exactly like the leaderboard's ghost row.
- **Days.** Each day is ranked as the leaderboard ranked it, among the group's players for the season: its roster, fixed when picks lock (section 11, Version 18). A day counts toward these honors only when the group had at least three ranked players and the day ranked at least one rarity past the threshold, so a founder alone for a week leads nothing. King of the Hill counts days in first; Wire to Wire needs first on every day that counts; Rollercoaster adds up the places the leaderboard's arrows showed, up and down; Comeback Kid measures from each player's first day that counts. A player who leaves a group and rejoins keeps every day of the season in it.
- **Everything else.** Pick of the Season is the most points a single ranked pick earned (Card Score × slot multiplier); a Card Score tops out at 100, so every #1 pick that finishes #1 earns 150, and the card with the higher adjusted win rate wins those ties, leaving a share only when players picked the same card. Whiff of the Season is the lowest Card Score among every #1 pick in the group, all four rarities. Wooden Spoon is last place. Oracle and Sharp Eye need 20 entries on the Everyone board, and players hidden from it can't earn them. Monkey Business needs a complete entry, since the random-picks line assumes 20 picks. Ties share every badge.
- **Reasons.** Each award keeps what its reason needs (the cards, ranks, points or places) in `detail`, and `badgeDescription` in `api/src/lib/mtg.ts` writes it in the past tense once earned and the present while on track: "Sky Tyrant, the #1 mythic pick, is #1 among mythics right now."
...
*As built (September 16, 2026):* a 200-season simulation of the rules as built (six-player groups, production pool sizes, synthetic 17Lands data) gave a typical player about 13 badges by the final day, lock badges included, and more high tiers than the estimate above: at least one Rare for most players and a Mythic for about one in six. The group honors cause most of it, since each group hands out a Mythic Champion and six Rare honors. The tiers are unchanged until a real season shows how they land.

*Version 18 (September 16, 2026):* the tiers are recalibrated before the first season rather than after it, since most players would otherwise have earned a Rare. Runner-Up, the four subtotal honors, Pick of the Season and Sleeper Agent become Uncommon, and Third Place becomes Common; Champion stays Mythic. The same simulation, rerun with the Batch 7 review's rules, gives about 13 badges a player, lock badges included, with a Rare for about one player in four (Perfect Order, Oracle and Told You So) and a Mythic for about one in six. The Mythics are nearly all Champions, since every group crowns one, so the estimate of one in twenty above can only hold in groups of twenty or more.

---

## 8. Emails

**Decision:** MTG Card Evaluation Challenge sends four kinds of email and no others. All of them go through Resend.
...
- These four are the only emails the challenge adds. Otherwise, specialized communities get whatever notifications NewChums already sends for any community. If NewChums already emails people when they join a community, the challenge's welcome details go into that email instead of a separate one.
...
- There are **no** preview-season card alerts, **no** incomplete-entry reminders, **no** lead-change alerts and **no** weekly standings emails. Players who want daily news can check the group home.

The four emails:

*As built (Batch 3, September 15, 2026):* welcome and lock warning are live. They share one preference, **MTG Card Evaluation Challenge** (`mtg_challenge`), whose one-click unsubscribe link ends every challenge email, and `mtg_email_log` records each send. The welcome goes out straight away from the join and join-request approval flows, and only before the lock; for a challenge group it replaces NewChums' generic "request approved" email, so an approved player gets one email. (Until Version 15 the creator of a group got a welcome too, with the invite link; creating a group now sends no email.) The lock warning is queued by the hourly job from 10:00 AM ET on the day before the lock until the lock, so a missed hour catches up, and is delivered through NewChums' email outbox with its retries. Its per-group line shows how many members have finished. Both emails link to How Scoring Works and to the lock's calendar file, and show times in Eastern time.

*Removed (September 16, 2026):* a weekly standings email, sent on Tuesdays during the season, was built in Batch 7 and removed the same day; the standings, player pages and on-track badges on the group home carry that news instead.
...
*As built (Batch 8, September 16, 2026):* season results is live. Once the season is finalized (section 12.3), the hourly job queues it from 10:00 AM ET on the final day, for three days from the later of that time and the finalize, so a late finalize still gets its three days. It goes to every player on a group's roster for the season (section 11) who is still a member, and not to members following along, late joiners or players in no group. One email covers every group: the player's finish in each ("You finished 2nd of 6 with 1,143 points."), the group's champion or "You're the champion," the group's three biggest honors after the Champion (Wooden Spoon and the Hall of Shame left out), every badge the player earned in the season with its reason, stacks counted ("Called It ×2") and group honors naming their group, and when the next season's picks open. The subject is "Final standings for Reality Fracture," or "Congratulations, champion of <group>" for a champion. Its button, "See the final standings," opens the season's own page (section 10.8), which keeps working after the next season starts. The shareable image isn't attached: it is made on that page, since the platform draws it in the browser. Everything is read when the email is sent, `mtg_email_log` keeps it to one per player per season, and reopening and finalizing a season again doesn't send it twice. *Batch 9:* an email still waiting when an admin reopens the season waits until it's finalized again (within its window) instead of being dropped, a failed read leaves it for the next pass, and the champion lines follow the Champion badge, so a group of fewer than three players, which has none, tells nobody they're the champion.

*Version 20 (September 17, 2026):* a season email waiting in the outbox is checked again when it is sent, so a player who has since turned the emails off, or whose account was suspended, doesn't get it; suspended accounts aren't queued at all, and they drop out of the lock warning's "finished" counts and Pick status. The welcome and lock warning count only picks that will stand at the lock, leaving out any whose card has left the pool, and a finished player's lock warning reads "All 20 of your picks are in. You can still change any of them until picks lock."

1. **Welcome.** Sent right after a player joins their first group for a set; creating a group sends nothing, and joining more groups for the same set is confirmed in the app only. It confirms the group, states the lock date and time, lists the season's key dates, and has a *Make your picks* button. If the player already has an entry for the set, it tells them their picks already count in this group.
...
3. **Picks revealed.** Sent at 9:00 AM ET the morning after the lock (Saturday, September 26, 2026 for Reality Fracture). It announces that everyone's picks are out and links to the Reveal page. It highlights one fun fact from each group, such as the most-picked mythic or the boldest #1 pick, lists the entry badges the player earned, and says when the first standings arrive.
4. **Season results and congratulations.** Sent at 10:00 AM ET on the final day (Friday, November 13, 2026) once the final standings are in. Every player gets their final rank and points, every badge they earned during the season, their group's champion and top badge moments, and a shareable results image. The champion gets a "Congratulations, champion" version. It ends with when the next season's picks open, if that's known.
...
*As built (Batch 5, September 15, 2026):* the hourly job tries at 9 and 11 AM, 1, 4 and 8 PM ET from the day after the Arena launch through the final day, and stops once the day is published. The checks: the response must parse as the wrapper or a bare list, with the expected fields and win rates between 0 and 1 (17Lands serves the JSON as `text/html`, so the content type is ignored); at least 95% of the pool must match; total games in hand must not fall; no more than five cards may lose games; and data identical to the last standings, or with no games at all, counts as not newer. Every response is archived to R2 under `mtg/17lands/<set>/`, and a published day also keeps its response in the database so it can be re-scored. A failed check emails the admin inbox straight away, and a day that ends without new standings emails after the 8 PM attempt, at most once a day, with a Sentry warning. *MTG Seasons* can fetch now, paste a response copied from a browser, publish past the checks, re-score a day, match a pool card to a differently named 17Lands record, and void or restore a card; an admin's fetch or paste replaces the day's standings when its data is newer. *MTG Seasons* can also name a day other than today, so the final day can be retried after it has ended (since Version 18 a fetch can name only today or, once it has ended, the final day, because the feed only ever has today's numbers; any other day is filled by pasting a response saved that day, and a dry run may name any day), and can rehearse a fetch as a dry run: everything is fetched, matched, checked and archived, and the attempt logged, but nothing is published, so card matching can be sorted out before the season's first morning. A bad day sends at most two emails, one for the first failed check and one after the last attempt. The Public Datasets fallback isn't built. *Version 19:* when the final day ends without standings, the next day tries again at the same five hours, publishing under the final day's date, and the season finalizes with the latest standings only after that day's 8 PM ET attempt (36 hours after `final_at`). The final day's last-attempt email says so, and the next day's last attempt sends one more, saying when the season ends and with which day's standings. *Version 20:* the checks catch more before they reach the standings. An empty card list fails the checks (17Lands answers a wrong expansion code the same way), so the first attempt alerts the admin to check the code instead of the day waiting until 8 PM. A card that matched the last standings and doesn't match now, because its record was renamed or its name is listed twice, counts as a card that lost games, since its Card Score would fall to a neutral 50. A day where any picked card doesn't match fails and names the cards; *publish anyway* still works. A card, or the pool's total, may lose the larger of 10 games or 0.5% before it counts as losing games, and a total that dips that little reads as no new games. Standings can't be published under a day that hasn't come yet, and a dry run before the season with no day named rehearses its first morning.
...
- **Sync.** `GET https://api.scryfall.com/cards/search?q=e%3Afra+-t%3Abasic&unique=prints&order=set` returns 175 cards per page; follow `next_page`. Group the results by `oracle_id` and keep the lowest collector number (the regular printing), so showcase and borderless versions don't double up. Keep cards with `booster = true` and a rarity of common, uncommon, rare or mythic. *As built:* during previews Scryfall's `booster` flag and `arena_id` are unreliable (146 printings on September 10 collapsed to 100 oracle cards), so until the full-gallery date every non-digital card of the four rarities counts as in the pool; from that date on, only `booster = true` cards stay in (since Version 19, once some printing in the set carries the flag, which Scryfall adds weeks or months after release; until then every card stays in). Before the lock a sync never takes more than a tenth of the pool out at once: such a run keeps the pool, notes it and warns Sentry, since a pick of a card that leaves the pool is deleted. A card the sync no longer sees leaves the pool, but only before the lock, when the pool is still allowed to move: a retracted or renumbered preview would otherwise stay pickable and spend the ingest's 5% unmatched budget for the rest of the season. The sync lives in `api/src/lib/mtg.ts` (`syncScryfallSet`). *Version 20:* before the lock a card the sync doesn't see, or that stops qualifying, stays in the pool for a day (`mtg_cards.missing_since`) and leaves only if it is still missing then, and a card listed again has its clock cleared. On the morning of September 17, 2026 Scryfall's search left out 24 of Reality Fracture's cards for about two hours; under the old rule the sync would have taken them out and players' picks of them would have been deleted. The tenth-of-the-pool hold counts only cards a run would actually take out. A card that can't be saved is noted, warned to Sentry, and skipped, and the rest of the run carries on.
...
- *Version 20:* before joining, a challenge group's page talks about the game, not plans. The preview of an approval-required or invite-only group lists *Make your picks*, *See everyone's picks* and *Daily standings*; a valid invite link reads "Your invite link checks out. Join to make your picks and follow the standings."; the signed-out footer and a private group's link preview ("Join this group's MTG Card Evaluation Challenge on NewChums.") name the challenge; and an open group shows section 11's heads-up under *Join*. Someone who signs up by email from an invite link keeps it through the signup, the verification email and signing in, so they land back on the group. The *Approval required* option on Create and Edit says picks, standings and members are only visible to members.
- *Version 23 (September 18, 2026):* a visitor who presses Join while signed out is joined as soon as they come back signed in, so nobody presses Join twice: through an invite link, from an open group's page, or from the "Join <name>" sign-up panel at its foot. The Join button remembers the intent in the browser (two hours, one group), never in the address, so a crafted link can't enrol anyone, and a signed-in visitor who opens an invite link still presses Join once. A request to join is never sent for them, since it can carry a note. A challenge group's buttons and cards read "Join this challenge" ("Request to join this challenge" for approval-required groups).
...
The community's usual header stays in every phase (name, picture, members, public or private label, Share and owner settings), with a season switcher added for past seasons. The tabs below the header (Plans, Announcements and the rest) are replaced by the challenge (since Version 20, a *Challenge* tab beside *Members* and the owner's *Requests*; section 3). Below the header, the page changes with the season:

- **During previews** (built), it shows the phase card with the lock countdown, the pool count by rarity as cards are revealed, a read-only card grid with rarity tabs, the Season timeline and the attribution footer. Since Version 14 picks open with previews, so the *Make your picks* button is live.
- **Between seasons,** it shows the last season's podium and badges, and the next set's own phase card and timeline once it's set up.
...
- **After the lock,** it shows the Reveal. From the first standings onward, it shows the leaderboard with a "Last updated" line and "Day 6 of 45," the Season timeline, and the Reveal one tap away.
...
*Version 21 (September 17, 2026):* the home drops the card pool section and the blank leaderboard's grey placeholder rows. Until the season goes live, the Standings card lists every member with a "?" for rank and, in place of points, how far along their picks are ("All 20 picked", "Picking, 12 of 20", "Not started") or, after the lock, how many picks they locked in and who is following along; counts only, never cards. It replaces the separate Pick status card, keeps the column headers (Rank, Player, Picks; the live board shows Rank, Player, Points), and its caption says when scores start. A non-member sees the card with that one line until the lock, and nothing after it. On the live board the benchmark line reads "Random picks score about 1,000". In the community header, a challenge group's members find Share (or Invite link) and Edit at the end of the members row, since their action row would otherwise hold nothing else; both are the same outlined buttons as a plan's Edit plan and Send invite (the Share button used to be a tinted text button).
...
*Version 21 (September 17, 2026), from Rob's review of the picks page:* each rarity's list holds up to ten cards. The top five are the picks, and up to five more wait below a "Shortlist, not scored" divider, so a player can gather candidates and then drag their best five to the top (migration 134, `mtg_pick_shortlist`). The shortlist never scores, never counts toward the twenty or in Pick status, and nobody else sees it. When a pick leaves the pool, the cards below it move up, so the first shortlisted card can become a pick; the lock does the same. In the card viewer, past five picks the button reads "Add to your shortlist as #6", and a full list offers Replace; grid tiles mark shortlisted cards in grey. The list has no up and down arrows: players drag by the handle (keyboard dragging still works on it), and tapping a card in the list opens it. "Saved" sits on the list's title line and fades after four seconds, while problems stay on screen. Removing a card shows the app's usual four-second toast (no Undo: Rob asked for the plain ones), above the phone's pick bar. Next, Back and the step tabs scroll to the top of the page, and while the page loads it shows a skeleton of itself. The back button to the group sits under the page's intro, which reads "Current Set: <set>. Your picks count in every challenge group you're in.", and the step header says "best first" instead of "Your #1 counts 1.5×". Receipts are "your thoughts" wherever players see them: the field reads "Your thoughts? (optional, visible to others)" (shorter on phones), the review says "Drag to reorder. Nobody sees your picks, or your thoughts on them, until picks lock.", and the review's notices use the app's own style instead of MUI's blue and orange alerts. A two-faced card's title shows each face with its own cost ("Semester Foreseer {3}{U} // Peer Review {2}{W/U}").

*Version 22 (September 18, 2026), from Rob's test on a phone:* reordering now works on phones. Dragging a row by its small handle was unreliable under a thumb, so below the desktop breakpoint, and on any touch screen, each card in a list has **up and down buttons** instead of a drag handle (a mouse on a desktop keeps dragging, and the keyboard still works on the handle). The phone's bottom bar has a labelled **Reorder** button in place of the "5/5" count, and the row of picks on it opens the list too; moving the first shortlisted card up makes it a pick. On phones narrower than 360 pixels the list drops its thumbnails so names keep their room, and names run to two lines. The **notes on picks are removed from the whole system** (Receipts, later "your thoughts"): the review tab has no fields, nothing shows notes on the Reveal, player or card pages, the API ignores a `note` from an older page and returns none, and the three badges that needed one (Told You So, Food for Thought and Eats Words) are retired, leaving 43. Nobody had written a note. The review's line reads "Use the arrows to reorder. Nobody sees your picks until picks lock." on phones and "Drag to reorder. …" with a mouse.
...
*Batch 8:* the Reveal shows only the badges earned at the lock, before and after the season ends; the season's other badges live on the results card and player pages. A past season's Reveal opens with `?set=<code>` from its season page, without the first-visit confetti.

### 10.5 Leaderboard

- **Header:** "Updated Wed 9:04 AM ET · Day 2 of 45 · 17Lands Premier Draft."
...
*As built (Batch 5, September 15, 2026):* the group home's *Standings* card shows "Updated Wed 9:04 AM ET · Day 4 of 45 · 17Lands Premier Draft", or "Last updated" with the date when today's data still hasn't arrived by 10 AM ET, an hour after the first attempt. Each player's row has rank with movement, name, points gained today and points behind the leader, the top three badges (names on wide screens, tier dots on phones, and a count) and points; a row opens to its points by rarity and its badges. The Group Mind ghost and the dashed *Random picks ≈ 1,000* line sit where their points fall, and members without picks are listed as following along. Standings are for the group's members and super admins, with `?date=` for a past day. Day 1 is the first morning after the Arena launch. The final-day podium comes in a later batch. The card appears from the Arena launch, saying when the first standings arrive, and it reloads when the tab comes back into view and every half hour, so a page left open doesn't show yesterday all day; a refresh that fails while the board is up says so and offers to try again. *Batch 6 (September 16, 2026):* the card has *This group* and *Everyone* tabs. *Everyone* ranks every entry for the season on the latest day by handle, rank and points: the top 100, plus your own row if you're further down, with a player who has no handle shown as "Anonymous player". Anyone with an entry gets a *Show me on this board* switch there. An opened row now links to that player's page. *Batch 7 (September 16, 2026):* a row's top three badges are icons in their tier colors, earned badges only, with "+N" for the rest; on phones the icons overlap, best on top, so the name keeps its room. The badge chips in an opened row, on the Reveal and on player pages carry the same icons. *Polish (September 16, 2026):* phones show only a row's best icon and count the rest, since most rows carry three badges or more from the lock and three icons left a name a few letters at 320 px. A past day's ranking counts the members who had joined by 9:00 AM ET on the next published day, so a member who joins after a day's standings isn't shown as new two days running, and re-publishing a day doesn't change it (replaced in Version 18: every day ranks the group's roster for the season, section 11); a player's points change always comes from their own entry's day before, whenever they joined. The *Everyone* tab loads the first time it opens and then stays loaded, reloading when new standings arrive; its switch shows what saved straight away, keeps keyboard focus, and the board says when a refresh failed. Once the season is over the header reads "Final standings from Fri, Nov 13", and if the first standings are an hour late the card says they're running late.

*As built (Batch 8, September 16, 2026):* the podium sits above the final standings: first in the middle and a little higher, second on the left, third on the right, tied players sharing a step, each name opening that player's page. Phones show first names on the podium, since the standings below give full names. Under it the viewer reads their own finish ("You finished 3rd of 6 with 1,043 points, and earned 12 badges," or "You're the champion") beside *Share your results*, which draws a 1,080 by 1,080 PNG in the browser: the set, "Final standings · <group>," the final day, the top three with medals and points, the viewer's finish, badge count and three best badges, and newchums.com with the Fan Content line. It opens in a dialog with *Download* and, where the device's share sheet takes images, *Share*. The badge ceremony below the standings lists every badge the group's players earned in the season, lock badges included, rarest first; each shows its tier, what it's for (or the player's own reason when one player earned it) and who earned it, best-ranked first, with the first eight shown and *Show all* for the rest. Members who joined after picks locked are listed under the standings as having joined after the lock, so they play from the next season.
...
*As built (Batch 6, September 16, 2026):* the player page is `/communities/<slug>/players/<userId>`, reached from an opened leaderboard row or a card's *Picked by* list. Three tiles show rank with movement, points with the change since the day before, and points behind the leader, computed exactly as the leaderboard computes them. A points-over-time line follows, with the random-picks line for reference and each day ranked as the group was that day, then the player's badges, then a block per rarity: each slot's card, win rate in hand and games, rank ("3rd of 81 commons"), Card Score with its change since the day before, multiplier, points and Receipt. A card without a win rate yet says so and scores 50, as does a voided card. *Actual top 5 right now* folds out under each block and marks the cards the player picked; *Compare with me* adds the viewer's own pick under each slot. Another player's page opens at the lock; your own opens before it, with your picks and no numbers. *Batch 7 (September 16, 2026):* the Badges card lists what the player has earned, then *On track*: the badges the latest standings would give them, as dashed outlines under the line "You earn these on the final day if the standings stay this way." A tap shows each badge's reason, in the present tense while it's on track, and a badge that stacks shows "×2" with every reason. On-track badges are refreshed after each day's scoring and never appear on the leaderboard. *Polish (September 16, 2026):* each pick is one tap target that opens its card, with a chevron, and so is each row of *Actual top 5 right now*; a voided pick says "Removed from scoring, so it scores 50" and a card without data "No 17Lands data yet, so it scores 50"; the Card Score change shows to one decimal, with ±0 when flat; a picked top-five card is marked "Their #2" or "Your #2"; the random-picks line is labelled under the chart instead of across it; a failed load offers *Try again*; and screen readers hear each badge's reason, each chart as one named image, and which rarity each *Compare* switch belongs to. Thumbnails load Scryfall's small image. On-track Oracle and Sharp Eye are left off the page of a player who hides from the Everyone board.
...
*As built (Batch 6, September 16, 2026):* the card page is `/communities/<slug>/cards/<cardId>`, linked from every card on a player page. It shows the card whole, with a flip for double-faced cards; tiles for rank, Card Score, GIH WR, games in hand and ALSA on the latest day; a rank-over-time line with first place at the top; *Picked by*, with each member's slot and Receipt, sealed until the lock; and links to the card on Scryfall and to 17Lands' card data for the set. It's for the group's members and super admins, because *Picked by* belongs to the group. Both pages' charts have a *Show the numbers* table, so no value lives only in a tooltip. *Polish (September 16, 2026):* the tiles read *Win rate in hand* (GIH WR), *Games in hand* and *Last seen in packs* ("Pick 10.0 on average (ALSA); later means drafters pass it"), the last spanning its row; a voided card is "Removed from scoring"; each *Picked by* row opens that player's page; the rank axis ticks at round places (#1, #10, #20); and the image loads the normal scan with the large one offered for sharper screens.
...
*As built (Batch 8, September 16, 2026):* a finished season has its own page, `/communities/<slug>/seasons/<code>`: the podium and the viewer's finish with the share image, the final standings, every badge, and *See everyone's picks*, for the group's members and super admins. A *Season* switcher lists the seasons the group played, with the season being played opening the group home. Everything the page links to keeps the season with `?set=<code>`: player pages, card pages and the Reveal, whose back buttons return to the season. The trophy case is a card on the NewChums profile, below Kudos, for anyone who can see the profile, and it doesn't appear for someone who hasn't played. It lists the player's earned badges by season, newest first, marking a season still being played: first the badges from their own picks, then each group's honors under "In <group>," linking to the group. A group is named when the viewer is a member of it, is the player or is a super admin, or when the group is public and the player shows their communities on their profile; otherwise its honors read "In a private group." *Version 20:* many badge reasons name picks ("<card>, the #1 mythic pick…") or describe them ("At least 10 of 20 picks are blue"), and after the lock picks are shown only to members of the player's groups (section 12.6), so the reasons, and names such as "Blue Loyalist," are shown only to the player, super admins and people in a challenge group with the player. Everyone else sees each badge's general name and description.
...
- **Moderation:** group names are user-written text, so they use the community's existing reporting tools. (Players' notes on their picks were user-written text too, until Version 22 removed them.)
...
*As built (Version 18, September 16, 2026):* the lock job writes each group's roster for the season to `mtg_group_rosters`: the members who had joined by `lock_at` and had picks. Standings, player and card pages, the Reveal, badges, results and the results email all read the roster through one SQL function, `newchums.mtg_season_player`, which for a group the lock job hasn't reached yet applies the same rule directly (joined by `lock_at`). A player who leaves and rejoins keeps their place and every day of their season; a member who joined after the lock is flagged as such on the standings and their player page, and earns nothing in the group that season. This replaces Batch 4's rule that a late joiner with a locked entry brought their picks into the group, and Batch 7's date cutoffs for past days. The Everyone board and its Oracle and Sharp Eye judging count only players in at least one active challenge group, so an entry whose player left every group drops off; such a player can't reach the board's hide switch, and doesn't need it. *Batch 9:* a finished season is kept as it ended (`newchums.mtg_group_season_players`): its players who have since left the group stay on its podium, standings, Reveal, card pages and player pages, and members who joined after its lock aren't listed with it at all. Members who were there at the lock without picks read as having not made picks that season. *Version 20:* the finalize records on the roster which players the final standings and badges were judged on (`mtg_group_rosters.in_final`), and a finished season shows exactly those players. A player who left mid-season no longer comes back onto the podium above the Champion badge's holder, while a player who leaves after the season ended stays in it. A member the owner removed can't undo it by leaving and joining again. The *Leave* dialog explains that a player's picks are their own and still count in their other challenge groups, and that leaving after the lock drops them off this group's standings; the *Remove* dialog says the same of a removed player. Closing a challenge group, from Edit, warns that it takes the group out of the challenge, and a challenge group's Edit page has no Schedule tab switch. Once a season has locked with players in a group, its owner can't delete it (409 `MTG_SEASON_HISTORY`, "You can close it instead"); super admins still can.
...
create table mtg_picks (
  ...
  note     text     check (char_length(note) <= 140),  -- Receipts; unused since Version 22, always null
...
create table mtg_group_rosters (                     -- who plays a season in a group, fixed at the lock
  community_id int         not null,
  set_id       int         not null references mtg_sets(id),
  user_id      int         not null,
...
-- As built (Version 18, migration 131): uuid keys, plus mtg_sets.finalized_at (when the finalize job ended
-- the season) and the function newchums.mtg_season_player(community, set, user, joined_at): on the roster,
-- or, for a group without a mtg_group_locks row, joined by lock_at.
-- Batch 9 (migration 132): mtg_sets.reopened_at, the 'catchup' alert kind on mtg_ingest_runs, an index on
-- mtg_badge_awards (user_id, set_id), and newchums.mtg_group_season_players(community, set): the season's
-- players, a finished season keeping those who have since left the group.
-- Version 20 (migration 133): mtg_entries.revision (moves on with every change to the stored picks),
-- mtg_entries.prior_completed_at and incomplete_since (the completion time an entry below twenty can get
...
- **Lock** (built in Batch 4 as `processMtgLock`) runs 30 seconds after the lock time, so a save that began just before it has committed: the hourly run scheduled at the lock waits those seconds out (Batch 9; before, a 6 PM ET lock waited for the 7 PM run), and a lock between hours runs on the next pass. The server refuses saves from the lock time itself. It first drops picks whose card left the pool, was voided or changed rarity, moving the rest up as a visit would have, then stamps each entry with `locked_at` and its pick count, awards entry badges to everyone with picks, locks every challenge group that exists (Group Mind, Early Bird and a `mtg_group_locks` row), and records `mtg_sets.locked_at`. A group that fails to lock is logged and retried on later passes without holding up the others or the reveal email, and super admins can run the job again from *MTG Seasons*. The Reveal opens at `lock_at`; until the job has run, and for groups formed later, it works the Group Mind out from the members' locked entries.
...
- **Scoring** (built in Batch 5) runs right after each successful ingest, in the same transaction that publishes the day. It computes adjusted win rates, ranks, Card Scores, entry scores and group standings. It can safely re-run for the same day. Right after the publish, and after any re-score, `refreshMtgOnTrackBadges` (Batch 7) replaces the season's "on track" badges with the latest day's verdict in one transaction; a failure there is logged and reported to Sentry and never undoes the publish.
- **Emails** (welcome and lock warning built in Batch 3, picks revealed in Batch 4, season results in Batch 8; `processMtgLockWarnings`, `processMtgRevealedEmails` and `processMtgResultsEmails` queue their rows after the lock, ingest and finalize jobs and before the challenge outbox pass in the same hourly run) follow section 8: the welcome goes out when a player joins their first group for a set; the lock warning at 10:00 AM ET the day before the lock; picks revealed at 9:00 AM ET the morning after the lock; and season results at 10:00 AM ET on the final day. Each checks the email log before sending.
- **Finalize** runs on the final day after that morning's scoring. It marks the final snapshot, awards the final-day badges, freezes the season, and triggers the season results email. *As built (Batch 8, September 16, 2026):* `processMtgFinalize` runs right after the ingest in the hourly pass. Once `final_at` has passed it finalizes an active season as soon as the final day's standings are published, which on schedule is the same 9 AM ET run; if 17Lands never delivers them, it finalizes with the latest standings 26 hours after `final_at` and reports to Sentry. It judges the day with the on-track refresh's own `judgeMtgSeason`, then in one transaction, under the refresh's advisory lock and fingerprint guard: marks that day's snapshot, and only that one, `is_final`; deletes the on-track rows; writes every season badge as awarded, turning on-track rows into awards; and sets the season `final` with `finalized_at`. A final season refuses new standings and re-scores. *MTG Seasons* has **Finalize now** (from `final_at` on, with the latest standings) and **Reopen the season**, which clears `is_final`, takes back every award except the lock's, sets the season active and refreshes the on-track badges, so standings can be fixed and the season finalized again; the status field no longer moves a season to or from final. *Batch 9:* a reopened season is held (`reopened_at`) until an admin clicks Finalize now, which works once the final day's standings are in or after its 8 PM ET attempt, and only with the latest day. The grace period is 36 hours, after the next day's retries (section 9.1). Publishing and re-scoring take the same advisory lock as finalize and refuse to write once the season has ended, so neither can slip in beside a finalize, and an archived season is treated as over like a final one.
...
- `GET  /mtg/communities/:id/players/:userId` (built) → the player's standing and points history, picks with each card's numbers, the actual top five, the viewer's own picks to compare, and badges, earned first and then on track; members and super admins; another player's page only after the lock
...
- `GET  /mtg/communities/:id/results` (built in Batch 8) → a finished season's podium (ranks 1 to 3, ties included), every badge the group's players earned with recipients and reasons, and the viewer's finish, badge count and three best badges; `results` is null until the season is final; members and super admins
- `GET  /mtg/communities/:id/seasons` (built in Batch 8) → the seasons the group played, newest first, each `final` or not and the current one marked; members and super admins
- `?set=<code>` (Batch 8) on the results, leaderboard, player, card and Reveal routes reads a past season; an invalid code is ignored and an unknown one is 404
- `GET  /public/users/:handle/mtg-badges` (built in Batch 8) → the trophy case, replacing the planned `/mtg/players/:userId/badges`: earned badges by season, a group's honors naming the group or marked private (section 10.8); anyone who can see the profile
...
- Admin (built so far): `GET /admin/mtg/sets`, `PUT /admin/mtg/sets/:code` (dates, feed address, status), `POST /admin/mtg/sets/:code/sync`, `POST /admin/mtg/sets/:code/lock`, `POST /admin/mtg/sets/:code/ingest` and `/ingest/paste` (fetch or paste stats, optionally past the checks), `POST /admin/mtg/sets/:code/snapshots/:date/rescore`, `GET /admin/mtg/sets/:code/stats`, `PUT /admin/mtg/cards/:id/stats-match` and `PUT /admin/mtg/cards/:id/voided`, and since Batch 8 `POST /admin/mtg/sets/:code/finalize { date? }` (409 `NOT_YET` before `final_at`, and until the final day's standings are in or its 8 PM ET attempt has passed; 409 `NOT_LATEST` for a day other than the latest) and `POST /admin/mtg/sets/:code/reopen` (final or archived seasons), all behind the *MTG Seasons* super-admin page
...
- **Entry validation:** at most five picks per rarity, slots 1 to 5 unique, each card's rarity matching its list, no duplicates, and every card in the set's pool. The list sizes are checked before any database work.
- **No lost saves:** a save builds on the revision its page loaded, so an out-of-date tab or device can't erase newer picks.
- **Season dates:** a lock time can't be set in the past or moved once it has passed, the Arena launch comes after the lock, no standings are published under a day that hasn't come, and the Everyone board is empty before the lock.
...
- **Phase 5, about October 1 to 26: during the season.** Add player and card pages with charts, "on track" badges, the Everyone board, past seasons and the trophy case. *Done September 16, 2026 (batches 6 to 8).*
- **Fixed: Friday, November 13, the final day.** Final standings, badge awards and the season results email. *Built September 16, 2026 (Batch 8) and reviewed September 17 (Batch 9).*
...
- **Version 15 — September 16, 2026.** The creator's welcome email ("<group> is ready. Share the invite link") is removed: creating a challenge group sends no email, since the creator already knows it's ready and the invite link is on the community page. Players who join still get the welcome, and a creator who later joins someone else's group gets it then, as their first. The welcome also drops the line "One entry plays in every group you're in."
- **Version 16, September 16, 2026.** Batch 7 built: every remaining badge rule, "on track" badges refreshed after each publish and re-score, badge icons on the leaderboard, and the weekly standings email, whose promise is back on the Season timeline (no migration). Design changes: a rarity counts toward rank badges only once more than ten of its cards are ranked, so the first days' handful of ranked cards can't hand out #1 badges; Clean Sweep, Perfect Order, Sniper, Called It and Bust stack once per rarity, while badges about particular cards are earned once and name every card; Beat the Crowd, Wire to Wire, Told You So and Lone Wolf belong to a group like the group honors; Contrarian and Hive Mind compare complete entries only and go to nobody when every entry overlaps equally; Lone Wolf and Wire to Wire need three players and Told You So a Group Mind; Monkey Business needs a complete entry; players hidden from the Everyone board can't earn Oracle or Sharp Eye; the leaderboard shows earned badges only, with icons that overlap on phones; the weekly email goes to players with points and not to members following along, keys its week by the Tuesday's date, names the day its standings are from when they're late, and is dropped when it can't be delivered within a day.
- **Version 17, September 16, 2026.** The weekly standings email is removed at Rob's request, along with the season timeline's weekly entries; the challenge now sends four kinds of email. A review of Batches 6 and 7 followed. Design changes: a past day's group standings count the members who had joined by 9:00 AM ET on the next published day, instead of by that day's publish time, so re-publishing a day never changes who counted and a late joiner isn't shown as new twice; points change always comes from the player's own previous day; day-based honors count only days with three ranked players and a ranked rarity; badges about points wait for ranked data (subtotal honors for their rarity, standings honors for any rarity, Monkey Business for all four); Told You So needs three players and Wooden Spoon four; Pick of the Season breaks its many 150-point ties on adjusted win rate; the on-track refresh reads in one consistent moment and never lets an older verdict overwrite a newer one; on phones a leaderboard row shows its best badge icon and a count; picks, top-five cards and *Picked by* rows are whole-row links; and badge borders meet 3:1 contrast on white.
...
- **Version 21, September 17, 2026.** Rob's review of the challenge pages (migration 134). Scoring: the slot multipliers are gone, so every pick counts its Card Score, while the order still breaks ties (the four #1 picks) and drives the order badges; nothing had been scored yet. Picks: each rarity's list holds up to ten cards, the top five being the picks and the rest a shortlist that never scores and nobody else sees; the list drags by its handle with no up and down arrows, a card in it opens on a tap, "Saved" lines up with the list's title and fades, removals use the app's own toast, Next and Back scroll to the top, the page loads with a skeleton, its back button sits under the intro, notices use the app's style instead of MUI's blue and orange alerts, and two-faced cards show each face's cost. Wording: Receipts are "your thoughts" wherever players see them (badge 36 becomes Food for Thought), the picks intro reads "Your picks count in every challenge group you're in", and the step header says "best first". The challenge home merges Pick status into the Standings card (members with a "?" rank and their pick status until scores start), drops the card pool section and the grey placeholder rows, and explains the random-picks line; a challenge group's Share and Edit move onto the members row; How Scoring Works loses the multiplier grid and gives season dates that fit every set; and the Reveal, player, card and season pages take the same back button and skeleton loading as the picks page.
- **Version 22, September 18, 2026.** Rob's test on a phone. Picks couldn't be reordered on phones, since a small drag handle is unreliable under a thumb: lists there now use up and down buttons (touch screens and anything below the desktop breakpoint), the bottom bar gets a labelled Reorder button, and a mouse keeps dragging. The notes on picks are removed from the whole system, with the three badges that depended on them (Told You So, Food for Thought, Eats Words), leaving 43 badges; nobody had written one. On phones a community's name gets the full width of its row and can wrap, and a private community's "Invite only" or "Approval required" tag moves down beside the hobby tags.
- **Version 23, September 18, 2026.** Joining finishes by itself after sign-in: pressing Join while signed out (an invite link, an open group, or the sign-up panel under "Join <name>") is remembered in the browser, and the group's page joins the visitor once they return signed in, where joining is instant. Nothing in the address can trigger it, and requests to join are never sent automatically. Challenge groups say "Join this challenge".
