...
- **Ranks.** "Finished" is the card's rank at its rarity on the day judged. A rarity counts toward rank badges only once more than ten of its cards are ranked: 17Lands leaves win rates blank under about 500 games in hand, so the first days rank a handful of cards, and "#1" among a handful means nothing. Every rarity is far past that by the final day. A card without a rank never qualifies. The bottom quarter is a rank above three quarters of the ranked cards; "drafted late" is an ALSA above the median ALSA of the rarity's cards that have one; top 5% and 25% of the Everyone board are ranks at or inside that share of its entries.
- **Group sizes.** Group honors, Wire to Wire and Lone Wolf need three players with points in the group; Contrarian, Hive Mind, Photo Finish, Rollercoaster and Beat the Crowd need four. Contrarian and Hive Mind compare complete entries only (at least four of them), since an unfinished entry overlaps less by leaving slots empty, and nobody gets either when every entry overlaps the Group Mind equally. Told You So and Beat the Crowd need a Group Mind, stored at the lock or worked out as the Reveal works it out; Beat the Crowd scores its picks exactly like the leaderboard's ghost row.
- **Days.** Each day is ranked as the leaderboard ranked it, a past day leaving out members who joined after it, and a group's days start with its first standings. King of the Hill counts days in first; Wire to Wire needs first on every one; Rollercoaster adds up the places the leaderboard's arrows showed, up and down; Comeback Kid measures from each player's first day in the group.
- **Everything else.** Pick of the Season is the most points a single ranked pick earned (Card Score × slot multiplier). Whiff of the Season is the lowest Card Score among every #1 pick in the group, all four rarities. Wooden Spoon is last place. Oracle and Sharp Eye need 20 entries on the Everyone board, and players hidden from it can't earn them. Monkey Business needs a complete entry, since the random-picks line assumes 20 picks. Ties share every badge.
...
*As built (Batch 7, September 16, 2026):* weekly standings is live. The hourly job (`processMtgWeeklyEmails`, after the picks-revealed step) queues it on each weekly Tuesday, the same Tuesdays the Season timeline lists (after the first standings, with none in the six days before the final day), from 10:00 AM ET once that day's standings are published, or from 8:00 PM ET with the latest standings there are. It goes to every player: a member of a challenge group whose entry has points. Members following along without picks get none. The email log keys each week by its Tuesday's date, so a week's email goes out once. Everything in it is read when it's sent: the player's points; their best and worst pick by Card Score, with ranks like "2nd of 40 commons"; each group's top three and "You're 2nd of 6, up 1 since last week" (in the first week, since the first standings); the badges they're on track for, naming the group on a group's badges; and the countdown to the final standings. The button opens the group with the most players. When the standings are older than that Tuesday, the email says which day they're from. The subject is "This week's standings for Reality Fracture," and an email not delivered within a day of being queued is dropped. Super admins can run the job from `POST /admin/mtg/sets/:code/weekly-email`, which still only sends a week that is due and hasn't gone out.
...
- **Emails** (welcome and lock warning built in Batch 3, picks revealed in Batch 4, weekly standings in Batch 7; `processMtgLockWarnings`, `processMtgRevealedEmails` and `processMtgWeeklyEmails` queue their rows after the lock job and before the challenge outbox pass in the same hourly run, the weekly step in its own try so it can't hold up the others) follow section 8: the welcome goes out when a player creates or joins their first group for a set; the lock warning at 10:00 AM ET the day before the lock; picks revealed at 9:00 AM ET the morning after the lock; weekly standings on Tuesdays at 10:00 AM ET during the season, after that day's scoring; and season results at 10:00 AM ET on the final day. Each checks the email log before sending.
