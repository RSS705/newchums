...
| `share_link_copied` | Copy tapped in the share moment, or the legacy Copy/Share handler on the plan detail page. Fires at tap time so a clipboard failure still counts the intent. | `surface` (`post_publish` \| `plan_page`), `is_host` (boolean) | GA |
| `share_prompt_shown` | The share moment opened: automatically on arrival after publishing, or from the plan page's Share button. Once per opening. | `surface` (`post_publish` \| `plan_page`) | GA |
| `share_sheet_opened` | Historical (Aug 2026): fired when the native share sheet was invoked from the share moment. The sheet button was removed when the share moment went single-action; the event no longer fires but stays documented for old GA data. | `surface` (`post_publish` \| `plan_page`) | GA |
