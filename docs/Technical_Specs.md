...
- Schema: `mtg_snapshots` (one row per set per Eastern `snapshot_date`, with `source` feed or paste, `raw_key`, `raw_json`, `total_games`, `matched`, `pool_size`, `scoring_version`, `taken_at`, `scored_at`, `is_final`), `mtg_ingest_runs` (every attempt: `trigger` schedule, admin or paste; `outcome` published, not_newer, fetch_failed or failed_validation; `notes`, `raw_key`, `total_games`, `matched`, `alerted`, `snapshot_id`), `mtg_card_stats` (per snapshot and pool card: `gih_games`, `gih_wr`, `adj_wr`, `rarity_rank`, `ranked`, `card_score`, `alsa`, `ata`, `iwd`), `mtg_entry_scores` (per snapshot and entry: `total`, the four rarity subtotals, `slot1_points`), and `mtg_cards.stats_arena_id` / `stats_name` match overrides. Select `snapshot_date::text`: the driver would otherwise turn a DATE into a local-midnight Date.
...
- `processMtgIngest` runs in the hourly pass right after the lock job. `runMtgIngest(sql, env, set, { trigger, text?, force? })` fetches `feed_url` with `MTG_USER_AGENT` and a 30-second timeout (or takes pasted text), archives the body to R2 `mtg/17lands/<code>/<date>-<ms>.json`, parses it, matches the scoring pool (in the pool and not voided), compares it with the latest snapshot on or before today, and records the attempt. A published day is written in one `sql.transaction`: the snapshot upsert, `mtgSnapshotWrites` (stats for every pool card, points for every entry with picks) and its run row. Scheduled attempts skip a published day; admin attempts replace it. `rescoreMtgSnapshot` re-parses a day's `raw_json` with the current pool, overrides and picks. `alertMtgIngest` marks the day's latest run `alerted` and sends `sendMtgIngestAlertEmail` (template `mtgIngestAlert`) to the admin inbox with a `Sentry.captureMessage` warning, once per set per day: on a failed check, or after the 8 PM attempt without new standings.
- Admin routes (super admins): `POST /admin/mtg/sets/:code/ingest { force? }`, `POST /admin/mtg/sets/:code/ingest/paste { raw, force? }`, `POST /admin/mtg/sets/:code/snapshots/:date/rescore`, `GET /admin/mtg/sets/:code/stats` (recent runs, published days, the latest day's unmatched cards and records, overrides and voided cards), `PUT /admin/mtg/cards/:id/stats-match { statsArenaId, statsName }` and `PUT /admin/mtg/cards/:id/voided { voided }`, rendered on *MTG Seasons* by `web/src/app/(app)/admin/mtg/AdminMtgStats.tsx`.
- `GET /mtg/communities/:id/leaderboard?date=` (members and super admins, after the lock; `standings: null` before the first day) ranks the day and the day before with `rankStandings` and returns rows (`rank`, `previousRank`, `total`, `change`, `behind`, rarity `subtotals`, top three `badges` and `badgeCount`), `noEntry`, `groupMind` (stored picks, or `computeGroupMind` over the members, scored with that day's Card Scores), `randomPicks: 1000` and the published `dates`. `mtgSetPayload` adds `standings` (latest date, time taken, day and total days).
