...
- `rsvp_by_at`: timestamptz nullable (migration 119), optional "RSVP by" deadline. Informational only: shown under the plan page's RSVP heading (and above the logged-out signup card), as a "Please RSVP by" line in invite emails, and as "RSVP by ..." in the share-link unfurl description. RSVPs stay open after it and no cron acts on it. Validated before `starts_at` on create and update; edits appear in the attendee update email as "RSVP by". Set from Extra options -> Joining in the Add/Edit Plan forms (date + time in the plan's local time).
