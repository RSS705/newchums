...
- **Cron Triggers:** `[triggers] crons = ["0 * * * *"]` in `api/wrangler.toml`. Runs hourly. The `scheduled` handler processes attendance assurance (confirmation requests, reminders, cutoff processing), event match digest, post-plan wrap-up emails (role-varied, sent ~3h after plan start, tracked via the historical `events.feedback_email_sent_at` column), and the one-time "Run it again" host nudge (bell + email, 48h+ after start and 24h+ after the wrap-up send, tracked via `events.run_again_nudge_processed_at`, migration 108). Integrated into the Sentry-wrapped export alongside `fetch`.
