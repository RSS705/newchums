...
- **Attendees, "Anyone deserve kudos?":** per-person "Give kudos" button opening a picker of fixed tags (catalogue in `api/src/lib/kudos.ts`, sixteen tags as of Rob's second pass on 2026-09-06: Good Energy, Quick Wit, Banter Merchant, Social Glue, Side-Quest Energy, Good Sport, Knows Cool Stuff, Great Taste, Refreshingly Normal, Delightfully Unhinged, Weird in a Good Way, Sweaty, Glad You Came, Would Hang Again, Would Recommend, Group MVP), Save to Chums, and Message. One tap gives the tag and closes; the chip can be re-tapped to change or crossed out to take back. No free text, nothing to moderate. Rules (enforced by the API): giver and recipient must both have been on the plan as host or Going; at most 3 kudos per giver per plan; 7-day window after plan start; blocked pairs refused with the ordinary `RECIPIENT_NOT_PARTICIPANT`. Kudos are anonymous on every surface: the recipient's bell row and daily email name the tag and the plan, never the giver; the public profile shows counts per tag only, and a tag appears to visitors once **two different people** have given it (the owner always sees all of theirs, with the still-private ones flagged). Replaced the text shout-outs in Sept 2026 (Rob: permanent prose on someone else's profile from the same few people felt wrong, and the moderation queue was overhead).
