...
- `rsvp_by_at`: timestamptz nullable (migration 119), optional "RSVP by" deadline. Informational only: shown as an icon-orb row in the plan facts card (date / location / going) for every viewer state, host and logged-out public preview included, with a countdown ("3 days left", "Less than a day left") or, once passed, "This deadline has passed. Late RSVPs are still welcome"; hidden on canceled and past plans. Also a "Please RSVP by" line in invite emails and a second "RSVP by ..." line in the share-link unfurl description. RSVPs stay open after it and no cron acts on it. Validated before `starts_at` on create and update (the Add Plan form additionally rejects a deadline already in the past; Edit allows one so a passed deadline never blocks other edits). A PATCH that moves `starts_at` to or before a stored deadline without touching `rsvp_by_at` clears it. Edits, including that clear, appear in the attendee update email as "RSVP by". Set from Extra options -> Joining in the Add/Edit Plan forms (date + time in the plan's local time).
