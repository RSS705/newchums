...
- **Welcome** (`sendMtgWelcomeIfFirst` in `api/src/index.ts`, templates `mtgWelcome.html/.txt`, subjects `mtgWelcome` / `mtgWelcome_creator`): called after the owner membership insert in `POST /communities` when a specialization is set, after the join in `POST /communities/:id/join` (open and invite-only), and in `PUT /communities/:id/join-requests/:requestId` on approve, where a sent welcome replaces `sendCommunityJoinApprovedEmail`. Skipped for non-challenge communities, after the lock, when the preference is off, and when the log already has a welcome for the player and set. Sent directly, waiting up to eight seconds for Resend: a refused send deletes the log row and returns false (so a later join tries again and an approval falls back to the generic approval email); a slow send finishes under `waitUntil` and counts as sent.
