...
| `GET /me/attendance-record/details` | Owner-only breakdown behind the four reliability tiles (the plans that counted, newest first, with what happened on each). Mirrors the public attendance-record filters exactly. Never exposed on other people's profiles. |
