...
- **Attendees, "Anyone earn a tag?":** users see **tags** (renamed from "kudos" the same day it shipped; kudos stays the internal name for the table, routes, preference key and notification type). Each person row carries "Tag them" inline with Save to Chums and Message; it opens a picker of fixed tags (catalogue in `api/src/lib/kudos.ts`, 64 tags as of 2026-09-06 evening, sorted alphabetically with a search box). One tap gives the tag and closes; the chip can be re-tapped to change or crossed out to remove. No free text, nothing to moderate. Rules (enforced by the API): giver and recipient must both have been on the plan as host or Going; at most 3 tags per giver per plan; 7-day window after plan start; blocked pairs refused with the ordinary `RECIPIENT_NOT_PARTICIPANT`. Tags are anonymous on every surface: the recipient's bell row ("New tag for you: 😏 Quick Wit from someone at …") and daily email name the tag and the plan, never the giver; the public profile "Tags" section shows counts per tag only, and a tag appears to visitors once **two different people** have given it (the owner, and super admins so a test tag can be checked, see all of them with the still-private ones flagged). The wrap-up payload carries `kudos.viewerCanTag` (host or Going); a Maybe viewer gets Save to Chums and Message but no tag control, and Maybe attendees have no tag control on anyone's card, matching the API rule. Replaced the text shout-outs in Sept 2026 (Rob: permanent prose on someone else's profile from the same few people felt wrong, and the moderation queue was overhead).
...
| `GET /public/users/:handle/kudos` | Aggregated counts per tag for the profile shelf; visitors only see tags with ≥ 2 distinct givers, the owner sees all with `publicYet`. Blocked pairs excluded for everyone. |
