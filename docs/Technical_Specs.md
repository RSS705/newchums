...
- `GET /mtg/sets/:code/entry` returns only the caller's entry; other players' picks never leave the server before the lock. Before the lock it also self-heals: a pick whose card is no longer `in_pool` (or is voided) is deleted, the rest at that rarity are renumbered from 1, and the response lists it once in `dropped`.
- `PUT /mtg/sets/:code/entry` is a full replace: validation first (at most five per rarity, unique slots 1 to 5, every card in the pool at the listed rarity, no card twice, notes at most 140 code points after collapsing whitespace), then an entry upsert and a single `sql.transaction` that deletes the entry's picks, inserts the new ones via `UNNEST`, and updates `updated_at` / `completed_at`. 423 `LOCKED` at or after `lock_at` or when the set is not active; 409 `NOT_OPEN` before `previews_start_at` (or `picks_open_at` when there is no previews date).
...
**Web:** `/communities/[slug]/picks` renders `web/src/components/mtg/picks/PickWizard.tsx` (steps Commons, Uncommons, Rares, Mythics, Review; autosave about 0.7 seconds after the last change as a serialised full replace, with a retry on network errors and a reload of the stored entry on 400, 409 or 423; a beforeunload warning while unsaved), `CardGrid.tsx` (search on name and rules text, colour / type / mana value chips, New only, Hide picked, sort; memoised tiles with slot badge and NEW ribbon), `CardViewer.tsx` (large image, flip for double-faced cards, preview credit, previous and next by button, arrow key or swipe, Add as #n / Remove / Replace which pick), `PickList.tsx` (dnd-kit sortable list with a drag handle, up and down buttons as the fallback, Receipts fields on the Review step), `PickTray.tsx` (sticky rail on desktop, a bottom bar with a sheet on phones, save status and lock countdown), `ReviewStep.tsx`, `ManaCost.tsx` and `pickUtils.ts`. `MtgChallengeHome` links to the wizard ("Make your picks" or "Edit your picks (14 of 20)") and shows **Who's finished** to members. Dependencies: `@dnd-kit/core`, `@dnd-kit/sortable`, `@dnd-kit/utilities`.
