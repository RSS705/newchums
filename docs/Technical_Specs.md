...
- **Cron Triggers:** `[triggers] crons = ["0 * * * *"]` in `api/wrangler.toml`. Runs hourly. The `scheduled` handler processes attendance assurance (confirmation requests, reminders, cutoff processing), event match digest, post-plan wrap-up emails (role-varied, ~3h after plan start; `events.feedback_email_sent_at` now marks expansion into the outbox), the one-time "Run it again" host nudge (bell + email, 48h+ after start and 24h+ after the wrap-up send, `events.run_again_nudge_processed_at`, migration 108), the daily kudos notice (`processKudosNotices`, 16:00 UTC, batched per recipient, `kudos.notified_at`, migration 120, `kudos_received` pref), the day-before reminder for plans without the attendance check (`processPlanReminders`, window [T-24h, T-21h], `events.reminder_processed_at`, migration 112, `plan_reminder` pref), and email-outbox delivery (`processEmailOutbox`, migration 110): every scheduled email job enqueues per-recipient rows and the processor delivers with bounded retries and a per-logical-send Idempotency-Key. Retryable = provider 429/5xx (up to 3 attempts); other 4xx = gave_up; no HTTP response at all = ambiguous, recorded and never retried because a duplicate is worse than a miss. Integrated into the Sentry-wrapped export alongside `fetch`.
...
- `planWrapUp`
- `kudosReceived` (subjects `kudosReceived_one` / `kudosReceived_many`), `concernReportAlert`
...
1. **Intent stage.** Going / Maybe are the primary actions, shown before any form. Tapping one reveals the form with intent-aware copy; a quiet "Continue without answering" link opens the form with no intent. The chosen intent stays changeable (chip row on the form and code stages) until verification.
2. **Form stage.** Email, date of birth (18+), Terms/Privacy acceptance, Turnstile. No password is collected. Submitting creates (or reuses) the unverified account and emails a **6-digit one-time code plus a magic link** (one email, both credentials on the same `email_verification_tokens` row, 30-minute TTL, code also in the subject line). The chosen intent is sent with the request and persisted server-side on the token row (`signup_intent`, `event_id`, migration 104).
3. **Code stage.** A single code input on the plan page (`inputmode="numeric"`, `autocomplete="one-time-code"`, paste-friendly, auto-submits at 6 digits, errors announced via `aria-live`). Shows the submitted address with a "Wrong email? Edit" path (remounts Turnstile since its token is single use), and a Resend button under a server-enforced 45-second cooldown. The magic link in the email remains a full alternate path through `/auth/magic`. A correct code verifies the email and returns a short-lived single-use **session grant** which the card exchanges through the existing Auth.js `magic-link` provider, then hard-navigates back to the plan (post-auth navigation rule).

**Verification never creates RSVP rows.** RSVP rows are created exclusively by `POST /events/:id/rsvp`. After verification (either path), the plan page auto-applies the stored intent through that endpoint, guarded: only when the viewer has no existing RSVP row (an existing RSVP always wins), not for hosts, not on canceled plans, and never when no intent was chosen (completing signup never implies Going). Crash recovery: if the client dies between verify and the RSVP call, `GET /events/:id` surfaces `viewerPendingIntent` (from the stored token-row intent, only while no RSVP row exists) and the same guarded effect applies it on the next visit. The legacy `?rsvp=` email-CTA param keeps its original unguarded overwrite semantics for `cant_make_it` and for already-signed-in users; the B1 `?intent=` param is the guarded one.

**OTP security model:** codes are 6-digit numeric (rejection-sampled uniform), stored only as sha256 hashes, single use, expire with the row (30 minutes), and the row is invalidated after 5 failed attempts (counter incremented atomically before comparison). Issuance invalidates all prior unused rows, so two devices resolve as last-issued-wins. Verify responses are uniform for "no account" and "no active code" (both `CODE_EXPIRED`) so the endpoint is not an account-existence oracle. Raw codes never appear in logs. Rate limits are **Postgres-backed** (`newchums.auth_rate_events` + `api/src/lib/dbRateLimit.ts`, migration 104) because the KV limiter is a no-op without its binding, which is the production state: issuance 10/IP and 3/email per 10 minutes (buckets shared between request and resend), verify guesses 30/IP per 10 minutes, resend cooldown 45s/email charged only after a successful send. Repo-wide limiter adoption is roadmap A3.
...
- `POST /auth/plan-signup/request` (unauthenticated). Body: `{ email, date_of_birth, turnstile_token, next, intent }` (`intent` optional, `"going" | "maybe"`). Validates email format, 18+ (via `isAtLeast18`) and Turnstile. **No legal-acceptance flag:** consent is implicit in submitting the card, which states it next to the submit button; the server still pins and records the current legal versions on the user row (clients can't forge them). Rate-limited per IP and per email via `checkDbRateLimit` (Postgres, migration 104), plus a 45s per-email send cooldown (`COOLDOWN` + `retry_after_seconds` when hit; charged only after a successful send). Branches on account state:
  - **No account / unverified account** → creates or refreshes the user row (auto-generated fun username via `generateFunUsername`, legal versions pinned, `email_verified_at = NULL`, `password_hash = NULL`, `password_setup_pending = TRUE` on fresh insert, DOB set), invalidates any prior unused `email_verification_tokens` rows for that user, mints one row carrying BOTH a hashed magic-link token and a hashed 6-digit code (**30-minute TTL**, `signup_intent` + `event_id` stored when `next` points at a real plan), emails the code + link, and fires the first-party `signup_email_sent` event. Returns `{ ok: true, state: "pending", resend_cooldown_seconds }`. An in-flight unverified row is refreshed but its `password_setup_pending` state is left untouched so a repeat submission doesn't clobber a completed setup.
...
- `POST /auth/plan-signup/resend` (unauthenticated, B1). Body: `{ email, next, intent }`. No Turnstile (single-use widget tokens make mid-flow re-solves hostile UX); bounded instead by the shared Postgres issuance buckets plus the 45s cooldown. Acts only on unverified, unsuspended accounts and responds `{ ok: true }` uniformly otherwise (no account-existence oracle). Re-mints the code + link row (prior rows invalidated, last-issued-wins), resolving plan context from `next` or the latest stored row, and fires `signup_email_sent` with `kind: "resend"`.
- `POST /auth/magic-link/consume` (unauthenticated). Body: `{ email, token }`. Verifies the token against `email_verification_tokens` (shared table with credential-signup email verification), marks the row `used_at = NOW()`, sets `email_verified_at = NOW()`, and returns the user record for the Auth.js `magic-link` Credentials provider to build a session from. Rejects suspended users without consuming the token. **Semantics deliberately untouched by B1** (the signin-link returner flow and the OTP session-grant exchange both ride on it); consuming a plan-signup row's link also retires that row's code, since both credentials share the row.
...
**RSVP requires authentication.** All RSVP / alt-time / confirmation / chat actions require a logged-in user. Unauthenticated visitors see the plan preview plus the intent-first Lightweight signup card (see the "Lightweight plan signup" section above). Once they verify, by typing the emailed 6-digit code in place or by clicking the magic link, they are authenticated and the normal RSVP flow unlocks; a stored Going/Maybe intent auto-applies through the normal RSVP endpoint when they have no RSVP row yet.
...
### Admin, hard delete + moderation visibility (super_admin only, July 2026)

**Hard delete (test-data hygiene).** Real accounts/plans created while testing in production contaminate KPIs and the funnel; QA-flagged plans are excluded up front, this is the after-the-fact cleanup. Endpoints (all `requireSuperAdmin`, server-side): `GET /admin/users/:id/delete-impact`, `POST /admin/users/:id/hard-delete`, `GET /admin/plans/:id/delete-impact`, `POST /admin/plans/:id/hard-delete`. Semantics:
- Impact preview first (counts of RSVPs, confirmations, chat messages, funnel analytics rows, hosted plans, community memberships/ownerships, DM conversations, tokens, feedback/metric rows, notifications), then TYPED confirmation (the username, leading `@` and case ignored, or the exact plan title).
- One non-interactive Postgres transaction (`sql.transaction`), whose first statement writes `admin_audit`; the cascade rides the FK graph enumerated in `api/src/lib/adminHardDelete.ts` (from the live catalog), with explicit statements for `product_events` (its FKs are SET NULL by design) and the NO ACTION blockers (owned communities deleted; `community_members.removed_by_user_id` / `community_join_requests.reviewed_by_user_id` nulled).
- Deleting a user deletes their hosted plans and owned communities as part of the cascade; the preview says so.
- HARD RULES: no emails or notifications of any kind fire; a super admin cannot hard-delete their own account or another super admin (enforced server-side); the per-plan ChatRoom Durable Object is purged post-commit (stateless relay: live sockets closed + defensive storage wipe; the DB cascade removed the history).
- KPI effect: funnel counts (signup_email_sent, rsvp_verified, signup_completed, rsvp_recorded, first/second_plan_created, plan_reached_3_rsvps) drop by the subject's rows, including other users' `rsvp_recorded` rows on a deleted plan; events-table KPIs (plans created, participation, plan health) drop via the cascade itself. The moderation "Remove plan" flow (`/admin/plans/:id/remove`, which emails the host) is unchanged and does NOT clean `product_events`; use hard delete for test data.
- UI: trash-forever actions on `/admin/plans` and `/admin/chums` rows plus a Danger zone on `/admin/chums/[id]`, all through the shared `AdminHardDeleteDialog` (impact list + typed confirmation).

**Super-admin plan visibility.** `GET /events/:id` returns an `adminView` payload (exact location fields regardless of `location_visibility`, which stays fully redacted for normal viewers) only on super-admin responses; the plan page renders it as a clearly labeled "Admin view" panel. `GET /admin/events/:id/chat-transcript` serves a READ-ONLY plan-chat transcript: no membership requirement, no read-state reads or writes, no unread side effects, no websocket/presence; each open (first page) is logged to `admin_audit` as `plan_chat_transcript_viewed`. Plan group chat is admin-readable for moderation; DMs remain not browsable (snapshot-on-report only, see Direct messages).
...
| `GET /inbox/contacts` | Candidate recipients for the "New message" picker: the viewer's On NewChums chums (`chums`, alphabetical) plus people from published, non-QA plans shared within the last 120 days (`fromPlans`, most recent plan first, deduped against chums, each with `planTitle`/`planAt`). Excludes suspended users and blocked pairs; deliberately does NOT evaluate `dm_privacy` per candidate, the compose flow surfaces the standard denial notice after a pick instead of silently hiding people. |
...
- `fallback_policy`: text (migration 039), what happens if minimum is not met at cutoff: `proceed` (default; nothing happens, no email), `notify_host` (host gets the at-risk email), or `auto_cancel`. Form labels since 2026-09-02: "We'll do the plan anyway" / "Notify me so I can decide" / "Cancel the plan".
...
- `rsvp_by_at`: timestamptz nullable (migration 119), optional "RSVP by" deadline. Informational only: shown as an icon-orb row **in the RSVP section, never in the facts card** (2026-09-08: a second date beside the plan date was too easy to read as the plan date). For people who can RSVP it sits at the top of the RSVP card ("Are you in?" / the share-link signup card); the host and the logged-out preview have no RSVP card, so for them it sits under the "Who's in" header. Countdown reads "19 days left to RSVP" / "Less than a day left to RSVP", or once passed "This deadline has passed. Late RSVPs are still welcome"; hidden on canceled and past plans. Also a "Please RSVP by" line in invite emails and a second "RSVP by ..." line in the share-link unfurl description. RSVPs stay open after it and no cron acts on it. Validated before `starts_at` on create and update (the Add Plan form additionally rejects a deadline already in the past; Edit allows one so a passed deadline never blocks other edits). A PATCH that moves `starts_at` to or before a stored deadline without touching `rsvp_by_at` clears it. Edits, including that clear, appear in the attendee update email as "RSVP by". Set from Extra options -> Joining in the Add/Edit Plan forms (date + time in the plan's local time).
- `min_attendees_required`: integer nullable (migration 094), optional RSVP-based threshold. **No longer offered in the Add or Edit Plan forms since 2026-09-02** (hosts confused it with `min_confirmed_attendees`; the confirmation-based `auto_cancel` fallback covers the need). The API still accepts it and the cron still honors it for plans that set it earlier; the Edit form passes a loaded legacy value straight back so saves never clear it. When set, the cron auto-cancels the plan 2 hours before `starts_at` if fewer than this many people are "going" (host counts toward the total, matching the existing `goingCount` definition). Distinct from `min_confirmed_attendees` (which is keyed off `event_confirmations` rows from the 24-hour attendance check). DB CHECK enforces `>= 1` and `<= max_seats` when seat count is set; defaults to NULL (no auto-cancel by RSVP threshold). Cron lives in `processAttendanceAssurance` Phase 4 (`api/src/index.ts`); it runs after the existing `min_confirmed_attendees` cutoff phase and is gated on `status = 'published'` so a plan already cancelled in the same tick (e.g. by Phase 3 with `fallback_policy = 'auto_cancel'`) is skipped, guaranteeing one cancellation email per recipient. Cancellation reason is `min_attendees_required_not_met` (separate from the existing `min_attendees_not_met` used by Phase 3); the new reason is excluded from host-completion / host-follow-through metric denominators alongside `no_attendees`.
...
| `GET /explore/local-signal` | Lightweight support signal for the bottom of the logged-in Explore feed. Returns one local-interest data point (`{ hobbyName, count }`) or `null`. Selection: (1) if `hobby` query param is set, try that exact hobby; (2) if exact hobby < 5, try its category; (3) else iterate viewer's profile hobbies and pick highest qualifying count (exact first, then category fallback). Threshold: minimum 5. "Active" = `last_active_at` within 6 months, not suspended. "Local" = within viewer's travel radius via Haversine. Degrades gracefully (returns null on error). With `hobbies=slug,slug` (a community page's own tags) the candidates are only those hobbies, with no fallback to the viewer's. |
...
**Unauthenticated share-link / invite-link signup** is handled by `POST /auth/plan-signup/request` + `POST /auth/plan-signup/verify-code` / `POST /auth/magic-link/consume` (+ `POST /auth/plan-signup/resend`; see "Lightweight plan signup" section above). These endpoints replaced the retired `POST /events/:id/email-rsvp`, `POST /events/:id/public-rsvp/request-code`, `POST /events/:id/public-rsvp/confirm-code`, `POST /events/:id/guest-confirm`, and `POST /events/:id/guest-alt-time` endpoints (migration 084, guest participation removed).
...
- **Per-message chat notifications:** the daily unread-chat digest was retired when plan chat went silent-by-default with per-message opt-in notifications (the sender ticks "Notify attendees"; the recipient-side `unread_chat_digest` pref governs those emails). Its cooldown column `user_profile.chat_digest_sent_at` was dropped by migration 109.
...
- **Attendees, "Anyone earn a tag?":** users see **tags** (renamed from "kudos" the same day it shipped; kudos stays the internal name for the table, routes, preference key and notification type). Each person row carries "Tag them" inline with Save to Chums and Message; it opens a picker of fixed tags (catalogue in `api/src/lib/kudos.ts`, 201 tags as of 2026-09-12, sorted alphabetically with a search box). One tap gives the tag and closes; the chip can be re-tapped to change or crossed out to remove. No free text, nothing to moderate. Rules (enforced by the API): giver and recipient must both have been on the plan as host or Going; at most 3 tags per giver per plan; 7-day window after plan start; blocked pairs refused with the ordinary `RECIPIENT_NOT_PARTICIPANT`. Tags are anonymous on every surface: the recipient's bell row ("New tag for you: 😏 Quick Wit from someone at …") and daily email name the tag and the plan, never the giver; the public profile "Tags" section shows counts per tag only, visible to anyone who can see the profile even when a tag was given once (Rob dropped the earlier two-giver threshold on 2026-09-06 evening: it hid too much). **Per-tag hide (migration 125):** the recipient gets a small eye button on each tag tile and chip; hiding writes a `kudos_hidden_tags` row and takes that tag off the profile for everyone else and out of the visible total. The owner keeps seeing it dimmed with a dashed border so it can be restored, and the underlying kudos rows are never touched. The wrap-up payload carries `kudos.viewerCanTag` (host or Going); a Maybe viewer gets Save to Chums and Message but no tag control, and Maybe attendees have no tag control on anyone's card, matching the API rule. Replaced the text shout-outs in Sept 2026 (Rob: permanent prose on someone else's profile from the same few people felt wrong, and the moderation queue was overhead).
...
| Endpoint | Purpose |
|----------|---------|
| `GET /events/:id/wrap-up` | Post-plan payload: thank-you targets (+ per-attendee `rsvpStatus`), `kudos` (`given`, the tag catalogue `tags`, `maxPerPlan`, `windowClosesAt`), issues against the viewer, viewer's own reports |
| `POST /events/:id/kudos` | `{ recipientUserId, tag }`: give or change one kudos tag (upsert on plan+giver+recipient). Errors: `CANNOT_KUDOS_SELF`, `NOT_PAST`, `KUDOS_WINDOW_CLOSED`, `NOT_PARTICIPANT`, `RECIPIENT_NOT_PARTICIPANT`, `KUDOS_LIMIT` (409). First insert creates a `kudos_received` bell row (no actor). |
| `DELETE /events/:id/kudos/:recipientUserId` | Take a kudos back while the window is open. |
| `GET /public/users/:handle/kudos` | Aggregated counts per tag for the profile section. Blocked pairs excluded for everyone. Tags the recipient has hidden are omitted for every viewer except the owner, who gets them with `hidden: true`; `total` always counts only what the public sees. |
| `PATCH /me/kudos/:tag` | `{ hidden: boolean }`: hide or restore one of the caller's own tag types on their public profile. Only ever writes the caller's row, so it cannot affect anyone else. Unknown tag → 400. |
| `GET /admin/kudos` | Super admin. Every tag ever given, newest first, with giver, recipient, plan, and whether the recipient has hidden it. Supports `q` (person, plan, or tag), `limit` (max 200) and `offset`. Powers **Tags Given** (`/admin/kudos`), the only surface anywhere that names a giver. |
| `GET /me/attendance-record/details` | Owner breakdown behind the four reliability tiles (the plans that counted, newest first, with what happened on each). Mirrors the public attendance-record filters exactly. |
| `GET /admin/users/:id/attendance-record/details` | The same breakdown for another person, super admins only. The public-user response carries `viewerIsSuperAdmin` (server-decided) so `/u/<handle>` can show the breakdown affordance to admins. |
...
*Analytics:* server-side `plan_copied` product event when a create was hydrated from a previous plan (`copied_from` in the POST body, validated against source ownership, QA-excluded, repeatable, `params.source_plan_id`); surfaced as `plansCopied` in `GET /admin/kpis/funnel` and, since Aug 2026, rendered in the KPIs Host loop table alongside the nudge rows. The old `feedback_*` GA events were removed with the form.

*"Run it again" nudge (Aug 2026):* the proactive half of the repeat-hosting loop (the reactive half is the post-plan "Run it again" button). The hourly cron's `processRunItAgainNudges` selects published plans with `starts_at <= NOW() - 48h`, `run_again_nudge_processed_at IS NULL`, and `feedback_email_sent_at <= NOW() - 24h` (the last predicate guarantees the wrap-up email and the nudge can never land on the same day for the same plan, even after cron downtime). Per plan it then excludes: no non-host `going` RSVP or every non-host `going` attendee carrying an active host-filed `no_show` issue; hosts with another non-cancelled plan created after this one started or scheduled after it (batch-created future sessions count); QA plans whose host is not a super admin; hosts with the `run_it_again` pref off (suppresses the bell too, the pref governs the whole nudge); and a second plan by the same host within one run. Every scanned plan is stamped processed whether sent or skipped, so a plan is considered exactly once, ever; migration 108 backfilled all pre-launch history so the first run had no backlog to blast. Sends: a system bell notification (`type = 'run_it_again'`, `entity_id` = plan, links to `/events/create?copy_from=<id>`) and the `runItAgain` email template (subject "Want to run {plan} again?", CTA to the same prefilled create URL, scoped unsubscribe token for `run_it_again`). The email copy deliberately offers rather than presumes (one-off plans exist) and no next date is asserted; the `?copy_from=` hydration already proposes the next future occurrence of the same weekday once the host is in the form.
...
**Objective sequence:** add_display_name → add_hobbies → set_location → set_travel_distance → add_bio → add_avatar → join_first_plan → attend_first_plan → send_first_message → give_first_kudos → create_first_plan → add_first_chum.

**Reliability tiles, owner breakdown (Sept 2026):** on `/profile` each of the four reliability tiles carries a small list button in its top-right corner opening a dialog listing the plans behind that number. It replaced a repeated "See the plans" text link under every tile on 2026-09-12 (Rob: too repetitive and too in-your-face); a list icon rather than an eye, because the eye means "hide this" on the Tags section of the same page. The icon is quiet by default and brightens on hover, and the dialog, the data and the permission rule are unchanged. The dialog lists (title, date, and what counted: kept / backed out, came / reported no-show, answered / no answer, went ahead / canceled). Data from `GET /me/attendance-record/details`. Super admins get the same links on `/u/<handle>` via `GET /admin/users/:id/attendance-record/details`; nobody else does. The tiles themselves use `TapTooltip` in `block` mode so they fill their grid cells (the inline-flex wrapper used to shrink them to content width).

**Your Plans, Past tab (Sept 2026):** hosted and attended plans are one merged list, newest first (the card already says "Hosted by you"), with a search box (title, venue, area, host name or handle, hobby name; server-side via `GET /events/mine?filter=past&q=&offset=&limit=`, debounced 300 ms) and a "Load more" button driven by the response's `hasMore` (page size 50). The tab count comes from the first, unfiltered page and shows `50+` when more exist. The Upcoming tab keeps its hosting / attending split.

**Public profile photo:** tapping the avatar on `/u/<handle>` opens the photo full size in a dialog (initials fallback is not tappable).

**Profile bio is rich text (2026-09-07):** the Bio field on `/profile` uses the same editor as plan descriptions (bold, italic, underline, strike, lists, divider, link, emoji) and is stored as HTML through `sanitizeDescriptionHtml` on `PUT /profile`. The 500-character limit counts the text (`htmlToPlainText`), with a 4000-character cap on the markup. Legacy plain-text bios still render (the renderer keeps their line breaks) and load into the editor as paragraphs (`plainTextToEditorHtml`); public-profile metadata strips the markup.
...
**Unified access model:** The forms present a single "Access" setting with three options: **Open** (visibility=public, join_mode=open), **Approval required** (visibility=private, join_mode=approval_required; labelled "Private" before September 2026) and **Invite only** (visibility=private, join_mode=invite_only, migration 124). The API accepts an `access` field (`open`, `approval_required`, `invite_only`; the old value `private` is still accepted as approval_required) that maps to these DB column pairs. The underlying `visibility` and `join_mode` columns are preserved for backward compatibility, but the only supported combinations going forward are these three. Approval-required communities are discoverable in the communities discovery feed and in the community detail page; non-members see a preview (description, hobbies, metadata) but internal contents (plans, members) are restricted until the viewer is an approved member. **Invite-only communities are not discoverable:** `GET /communities` lists them only to their active members (the public list never returns private communities anyway), the public-profile communities list omits them, and the slug URL serves the same restricted preview as an approval-required community but with an "invite only" notice and no request button. The credential is the invite link `/communities/<slug>?invite=<code>`: `GET /communities/:slug?invite=` returns `viewerInvited: true` on the restricted shapes when the code matches, and `POST /communities/:id/join` with `invite_code` in the body joins instantly (no approval, no join-request row); without a matching code it returns 403 `INVITE_REQUIRED`. The code (14 characters, no look-alike letters) is minted when a community is created as or switched to invite-only, is returned in the full detail payload only to active members and super admins (stripped otherwise, since public communities serve the full shape to everyone), and can be rotated by the owner with `POST /communities/:slug/invite-code/reset`. The detail page's Share button becomes **Invite link** for members of invite-only communities and copies the link; the edit form shows the link with Copy and Reset. The Requests tab exists only for approval-required communities. In-app notifications (`community_join_request`, `community_join_request_approved`, `community_join_request_declined`) and email notifications support the full join request lifecycle including optional requester messages.
...
| Route | Description |
|-------|-------------|
| `POST /communities` | Create a community. Validates name, slug (3-50 chars), description (required). Accepts unified `access` field (`"open"`, `"approval_required"` or `"invite_only"`; legacy `"private"` = approval_required) which maps to the DB columns: open = `visibility=public, join_mode=open`; approval_required = `visibility=private, join_mode=approval_required`; invite_only = `visibility=private, join_mode=invite_only` plus a freshly minted `invite_code`. Also accepts legacy `visibility`/`join_mode` for backward compatibility. Requires at least one hobby (`interest_items`). Requires location for offline communities (`is_online = false`). Accepts `is_online`, `website`, `discord_url`, `whatsapp_url`, and optional `operating_hours` (see **Community operating hours** below). Creator becomes owner + member. Links hobbies via `community_interests`. |
...
| `GET /communities/:slug` | Community detail. **Auth optional**, the slug URL is the canonical public / shareable destination for a community (see **Canonical community URL** below) and this endpoint mirrors that: logged-in viewers get full detail (membership, plans metadata, website, Discord / WhatsApp links if private-and-member), logged-out viewers get either the full public view (for `visibility = 'public'` communities) or the same restricted preview a logged-in non-member would get (for `visibility = 'private'`). The privacy contract is identical across authenticated and anonymous non-members: no member list, no plan details, no `website`, no `discord_url`, and no `whatsapp_url` leak. Returns full community info including `is_online`, `website`, `discord_url`, `whatsapp_url`, `hobbies` array, member_count, viewer's membership role/status, pending join request status. **Private communities** return a preview for non-members (name, description, avatar, hobbies, location, member count, visibility, plus `upcoming_plan_count` so the locked preview can surface a real number without leaking plan detail) with `restricted: true`; plans, members, `website`, `discord_url`, and `whatsapp_url` are hidden from non-members to keep those links scoped to approved members only. Non-member responses also include `viewerPendingRequestSentLabel` (pre-formatted "Sent N days ago" string), `viewerPendingRequestRefreshable` (boolean; true once the pending request has aged past the cooldown), `viewerPendingRequestDaysUntilRefreshable` (number or null), and `viewerPendingRequestCooldownDays` (echoes the server-side constant) so the client can render the pending-state card without doing any time math of its own. Anonymous-viewer responses omit those viewer-specific fields, there is no viewer identity to key them on. Owner/admin sees pending join requests with message and avatar URLs. |
| `PATCH /communities/:slug` | Update community (owner or super admin). Accepts unified `access` field (`"open"`, `"approval_required"` or `"invite_only"`, preferred; legacy `"private"` = approval_required) or legacy `visibility`/`join_mode`. Switching into invite-only mints an `invite_code` (a save while already invite-only keeps the existing code). Also: name, description (required), chat_enabled, `is_online`, `website`, `discord_url`, `whatsapp_url`, `operating_hours` (see **Community operating hours**), location fields, avatar_key, `banner_key` (only `null` is accepted to clear; setting a key is done via `/media/finalize` so the Pro gate is enforced there, not here), `interest_items` (replaces all community hobbies; at least one required). |
...
| `POST /communities/:slug/invite-code/reset` | Owner or super admin. Mints a new `invite_code` for the community and returns it; the old invite link stops working immediately. |
| `POST /communities/:id/join` | Join (open), join with the invite link (invite_only: body `invite_code` must match, else 403 `INVITE_REQUIRED`; no request row, no notification) or request to join (approval_required). Accepts optional JSON body with `message` (max 500 chars) and `invite_code`. Idempotent. For approval_required: creates a join request, creates in-app notification for owner (`community_join_request`), sends join-request email to owner (`communityJoinRequest` template, respects `community_join_request_received` notification pref). The email's "Review request" CTA links to `/communities/:slug?tab=requests` so the owner lands directly on the Requests tab (see **Community detail tab deep-links** below). The message field is passed to the template via the `hasMessage` + `message` pair so the "Their message" block only renders when the requester included a note. **Re-request cooldown:** if a pending request already exists, the server checks its age. Within `COMMUNITY_JOIN_REQUEST_COOLDOWN_DAYS` (default 7) the endpoint returns `{ ok: true, status: "already_pending", cooldownDays, daysRemaining }` without notifying anyone. After the cooldown, the existing pending row is refreshed in place (`created_at = NOW()`, `message` replaced), the owner is re-notified, and the response is `{ ok: true, status: "refreshed" }`. The partial unique index guarantees only one active pending row per `(community, user)`. |
...
Uses the shared media upload pipeline (`POST /media/init` → `PUT /upload/:token` → `POST /media/finalize`) with purpose `community_avatar`. Object key pattern: `community_avatars/{userId}/{timestamp}.{ext}`. Finalize requires community ownership or super admin. Served via `GET /communities/:communityId/avatar`. The browser crops the logo square and exports it at 256×256 WebP (`CommunityLogoEditor`, sharp at the largest place it shows, 72 px in the community header); the input file is capped at 20 MB. Create Community and Edit Community show it to the right of the banner through `CommunityImagesEditor`, stacked below it on phones: the columns split 3 : 1 and the logo tile takes 3/5 of its column, so the square logo is exactly as tall as the 5:1 banner preview at every width. Both tiles are keyboard-operable (Enter or Space opens the file picker). The Edit form versions the logo URL by its key (`communityAvatarUrl`), so it reloads only when the logo changes.

**Community banner (free for all plans):**
Same shared media pipeline with purpose `community_banner`. Object key pattern: `community_banners/{userId}/{timestamp}.{ext}`. Output size target: 1600×320 WebP (5:1), max 600KB (`MAX_COMMUNITY_BANNER_BYTES`). Served via `GET /communities/:communityId/banner` (public, no auth, cache-control `public, max-age=86400`). **Access**: `POST /media/finalize` only requires that the caller is the community owner (super admins bypass so they can manage on behalf of an owner, same pattern as the community avatar). The earlier Community Pro gate (`hasCommunityProAccess()` against the owner's plan) was removed when banner upload moved to Free. `GET /communities/:slug` exposes `viewerCanEditBanner` (true for owner / super admin) so the Edit form can render the uploader for the right viewers; the legacy `viewerHasProBannerAccess` field is kept as an alias for older clients and is identical to `viewerCanEditBanner`. The banner key can be **cleared** via `PATCH /communities/:slug` with `{ banner_key: null }`; setting a key is only accepted through `/media/finalize` so ownership is always re-verified. The banner renders on every community detail surface, **public**, **logged-in**, and **restricted private-community landing**, because it's a visual element that carries no plan/member information and doesn't change the privacy contract.
...
### MTG Card Evaluation Challenge (specialized communities)

Product spec: `docs/MTG-Bets-Spec.md` (its Change Log records every design decision). A community created with `specialization = 'mtg_prediction_challenge'` keeps the normal community header and replaces the tab body with the challenge view (since 2026-09-17 on a *Challenge* tab, with the Members and Requests tabs kept). The value is set once by `POST /communities` (validated against `MTG_SPECIALIZATION`) and never changed by PATCH.
...
**Web:** `web/src/components/mtg/MtgChallengeHome.tsx` (phase card with a live lock countdown, pool counts, rarity tabs over a lazy-loading card grid at 3/5/6/7 columns, `SeasonTimeline`, attribution footer) is rendered by `CommunityDetailClient` in place of the Plans, Announcements and Schedule tabs when `community.specialization` matches, on a first *Challenge* tab (`tabIndexMap.challenge`), with Members and Requests kept and deep-linkable by `?tab=`; `CommunityListCard` shows an "MTG Card Evaluation Challenge" chip; `CreateCommunityClient` has the *Specialized community* switch and dropdown and sends `specialization`; `/admin/mtg` (`AdminMtgClient`, nav entry *MTG Seasons*) edits season dates and the feed address and runs *Sync cards now*. Card images are always shown whole (`object-fit: contain`), per Scryfall's rules. The attribution string lives in `web/src/components/mtg/mtgTypes.ts`.

**Picks (Batch 2, migration 126).** `mtg_entries` (one per user per set, `UNIQUE (user_id, set_id)`; `updated_at` moves on every player save and is the tie-break input; `completed_at` is when the entry most recently reached twenty picks, cleared below twenty; `hide_from_everyone_board`), `mtg_picks` (`PRIMARY KEY (entry_id, rarity, slot)`, `UNIQUE (entry_id, card_id)`, slot 1 to 5, `note` up to 140 characters for Receipts), `mtg_rarity_reviews` (`last_reviewed_at` per user, set and rarity for NEW ribbons). `validateEntryPicks` and `mtgPicksOpen` in `api/src/lib/mtg.ts` are pure and unit-tested (`__tests__/mtgEntry.test.ts`).

- `GET /mtg/sets/:code/entry` returns only the caller's entry; other players' picks never leave the server before the lock. Before the lock it also self-heals: a pick whose card is no longer `in_pool`, is voided, or no longer has the rarity it was picked under is deleted, the rest at that rarity are renumbered from 1, and the response lists it once in `dropped`.
- `PUT /mtg/sets/:code/entry` is a full replace. Only players may save: an active member of an active challenge community, or a super admin (403 `NOT_A_PLAYER` otherwise). A card in the save that is no longer in the pool, or whose rarity has changed, is dropped, the rest at that rarity move up, and the response lists it in `dropped`; unknown card ids and malformed input still fail validation (at most five per rarity, unique slots 1 to 5, no card twice, notes at most 140 code points) with 400. A save identical to what is stored returns `unchanged: true` without touching the entry, so it cannot move `updated_at`, the tie-break input. Otherwise one `sql.transaction` checks the lock first (a `1 / 0` guard when `now() >= lock_at` or the set is not active, mapped to 423 so a save that started just before the lock cannot commit after it), then deletes the entry's picks, inserts the new ones via `UNNEST`, and updates `updated_at` / `completed_at`. 423 `LOCKED` at or after `lock_at` or when the set is not active; 409 `NOT_OPEN` before `previews_start_at` (or `picks_open_at` when there is no previews date).
- `GET /mtg/sets/:code/cards` adds `isNew` for a signed-in caller (`first_seen_at` later than their `last_reviewed_at` for that rarity; no review row means nothing is new). `POST /mtg/sets/:code/reviewed { rarity }` stamps the visit.
- `GET /mtg/communities/:id/progress` returns each active member's pick count and `complete` flag for the current set, never card data; members of that challenge community and super admins only.
- `mtgSetPayload` carries `picksOpen`.

**Web:** `/communities/[slug]/picks` renders `web/src/components/mtg/picks/PickWizard.tsx` (steps Commons, Uncommons, Rares, Mythics, Review; autosave about 0.7 seconds after the last change as a full replace on a promise chain, so saves never overlap or land out of order; a change that leaves the state identical is not counted; leaving the wizard flushes a pending change first: Done and the back link wait up to four seconds for it, and `pagehide`, a hidden tab or unmounting send it with `keepalive`; network and server errors retry every four seconds; 401 stops saving and shows a sign-in link; 400, 403, 404, 409 and 423 reload the stored entry; a `dropped` list in a save response is shown as a notice; removing a pick offers Undo; one visually hidden `role="status"` region announces save state and the visible indicators are `aria-hidden`; a beforeunload warning while unsaved), `CardGrid.tsx` (search on name and rules text, colour / type / mana value chips, New only, Hide picked, sort; memoised tiles with slot badge and NEW ribbon), `CardViewer.tsx` (large image, flip for double-faced cards, preview credit, previous and next by button, arrow key or swipe, Add as #n / Remove / Replace which pick), `PickList.tsx` (dnd-kit sortable list with a drag handle, up and down buttons as the fallback, Receipts fields on the Review step; on phones the controls are 40 px and sit on their own line with remove set apart), `PickTray.tsx` (sticky rail on desktop; on phones a bottom bar with a sheet, hidden once picks are read-only, whose slot thumbnails shrink and whose save status is icon-only so it fits 320 px), `ReviewStep.tsx`, `ManaCost.tsx` and `pickUtils.ts`. `MtgChallengeHome` links to the wizard ("Make your picks" or "Edit your picks (14 of 20)", the count taken from the progress route rather than the entry route, so the entry route's one-time "left the pool" notice is left for the wizard) and shows **Who's finished** to members. `SeasonTimeline` renders times in Eastern time on the server and during hydration, then in the reader's zone, so server-rendered pages hydrate cleanly. Dependencies: `@dnd-kit/core`, `@dnd-kit/sortable`, `@dnd-kit/utilities`.

**Emails, calendar and the scoring page (Batch 3, migration 127).**

- `mtg_email_log (user_id, set_id, email_type, period)` is the one-email-per-event guarantee (`welcome`, `lock_warning`, and later `revealed` and `results`; the check constraint still allows `weekly` and `period`, left from a weekly email that was built and removed on 2026-09-16).
- `email_outbox.event_id` is now nullable and `set_id` (FK `mtg_sets`) was added, with `CHECK (event_id IS NOT NULL OR set_id IS NOT NULL)`. The identity index became `idx_email_outbox_identity_v2` on `(kind, COALESCE(event_id::text, set_id::text, ''), user_id, COALESCE(group_key, ''))`. `processEmailOutbox` left-joins `events`, marks a plan-kind row with no plan `gave_up` instead of letting it fall through to the wrap-up email, maps `mtg_*` kinds to the `mtg_challenge` preference, and keys their idempotency on the set.
- Preference key `mtg_challenge` ("MTG Card Evaluation Challenge", default on) in `notificationPrefs.ts`, the settings list and the unsubscribe page labels.
- **Welcome** (`sendMtgWelcomeIfFirst` in `api/src/index.ts`, templates `mtgWelcome.html/.txt`, subject `mtgWelcome`): never sent to a community's owner, so creating a challenge community sends no email (the creator variant was removed 2026-09-16); called after the join in `POST /communities/:id/join` (open and invite-only), and in `PUT /communities/:id/join-requests/:requestId` on approve, where a sent welcome replaces `sendCommunityJoinApprovedEmail`. Skipped for non-challenge communities, after the lock, when the preference is off, and when the log already has a welcome for the player and set. Sent directly, waiting up to eight seconds for Resend: a refused send deletes the log row and returns false (so a later join tries again and an approval falls back to the generic approval email); a slow send finishes under `waitUntil` and counts as sent.
- **Lock warning** (`processMtgLockWarnings`, template `mtgLockWarning`, subjects `mtgLockWarning` and `mtgLockWarning_tonight`, outbox kind `mtg_lock_warning`): hourly, runs from `mtgLockWarningAt(lock_at)` (10:00 AM ET on the Eastern calendar day before the lock) until the lock. One set-based statement logs every candidate (players with an entry for the set plus active members of active challenge communities, up to 2,000 per run, skipping anyone already logged) and queues outbox rows only for those who want the email and were not welcomed in the last 12 hours. The payload holds only the set name, the lock time and the links. At delivery (`mtgLockWarningDetails`) the player's pick count and each group's finished count are read live, the heading and subject say "tonight" when the lock falls later that Eastern day, and a row whose lock has passed is marked `gave_up` instead of sent. Season emails get their own outbox pass right after the job (`processEmailOutbox(..., "mtg")`, up to 200 rows) so a wave neither waits behind plan email nor delays it, and a kind with no sender is marked `gave_up` rather than falling through to the plan wrap-up.
- `lib/mtg.ts` helpers `easternToUtc`, `mtgLockWarningAt`, `formatEasternLong`, `formatEasternShort`, `buildIcsEvent`, `escapeIcsText`, `foldIcsLine`, tested in `__tests__/mtgEmail.test.ts` (both daylight-saving changeovers included).
- **Calendar files** (linked from the welcome and lock warning emails; the Season timeline stopped linking them on 2026-09-16): `GET /mtg/sets/:code/calendar/lock.ics` and `final.ics` on the API (public, one hour of caching, a display alarm three hours before the lock and one hour before the final day). The web route `web/src/app/(public)/mtg/calendar/[code]/[file]/route.ts` passes them through so every link is on newchums.com; `SeasonTimeline` shows "Add to calendar" on upcoming entries flagged `calendar` when given a `setCode`.
- **How Scoring Works:** `web/src/app/(public)/mtg/how-scoring-works/` (server page in `LandingLayout`, the current set fetched with one hour of revalidation, `HowScoringWorksContent` with the spec 10.9 copy, the slot multiplier strip, dynamic season dates in Eastern time and the Season timeline). Linked from `MtgChallengeHome`, `PickWizard` and both emails.

**Lock, Reveal and badges (Batch 4, migration 128).**

- Schema: `mtg_sets.locked_at` (when the lock job finished); `mtg_entries.locked_at` and `locked_pick_count` (the entry as frozen); `mtg_group_locks (community_id, set_id, entries, locked_at)`, which marks a group's Group Mind and Early Bird as done; `mtg_group_minds (community_id, set_id, rarity, slot, card_id, votes, pickers, entries)`; and `mtg_badge_awards (id, set_id, user_id, community_id, badge_code, award_key, detail, status, awarded_at)` with a unique index on `(set_id, user_id, COALESCE(community_id::text, ''), badge_code, award_key)`, so every award insert is `ON CONFLICT DO NOTHING` and safe to repeat. `community_id` is null for entry badges and set for group honors.
- Pure helpers in `api/src/lib/mtg.ts`, tested in `__tests__/mtgLock.test.ts`: `MTG_BADGES` (code to name, tier and description), `badgeLabel` (names the Loyalist color), `colorLetters`, `computeEntryBadges` (the eight entry badges; a multicolored card counts toward each of its colors; Rainbow and Loyalist need all 20 picks; Loyalist ties go to WUBRG order), `computeGroupMind` (5 votes for a #1 down to 1 for a #5, top five per rarity; ties by #1 votes, players, collector order, then card id), `earlyBirdWinners` (at least three members with picks; the earliest `completed_at` among complete entries; ties share), `mtgMorningAfter` (9 AM ET on the next Eastern day, used for the reveal email and the first-standings timeline entry), `mtgRevealedEmailAt` (the first 9 AM ET at least an hour after the lock), `formatEasternDate` and `revealFunFact`.
- `processMtgLock(sql, only?)` runs first among the challenge steps of the hourly pass, at least 30 seconds after `lock_at`. The first run reads every unstamped entry's picks with their validity (`in_pool AND NOT voided AND c.rarity = p.rarity`), rewrites entries with stale picks through `writeMtgPicks(..., lockCleanup = true)` (no lock guard, `updated_at` untouched, `completed_at` cleared below twenty), stamps every entry, inserts entry badges for every entry with picks, runs `ensureMtgGroupLock` for each active challenge community created by then (each in its own try/catch), and sets `mtg_sets.locked_at`. Later passes only retry communities created before `locked_at` that have no `mtg_group_locks` row. `ensureMtgGroupLock(sql, set, communityId)` reads the active members' picks, computes the Group Mind (at least two members with picks) and Early Bird (at least three), and writes them in one statement whose first step claims the `mtg_group_locks` row, so two runs at once can't mix rows; nothing else locks a group. `POST /admin/mtg/sets/:code/lock` (super admins; 409 `NOT_ACTIVE` for an inactive season, `NOT_YET` before the lock time) repeats the first run's steps and returns entries, new badges, groups locked, failures and dropped picks; the *MTG Seasons* page has *Run the lock now* or *Run the lock again* with the time it locked. `PUT /admin/mtg/sets/:code` returns 409 `LOCK_RAN` for a changed `lock_at` once `locked_at` is set.
- `GET /mtg/communities/:id/reveal`: 401 when signed out, 404 unless the id is an active challenge community, 403 `FORBIDDEN` unless the caller is an active member or a super admin, and 403 `SEALED` (with `lockAt` and no card data) before the lock. It never writes. It returns the set, the community, `entries` (members with picks now), `players` (the viewer first, then members with picks, then members without; each with `picks` by rarity carrying `note` and `onlyYou`, set when nobody else in the group picked that card and the group has at least two entries, and `badges` with name, tier, description and `groupHonor`), `mind` by rarity (card, votes, pickers) with `mindAtLock` and `mindEntries` (the stored Group Mind, or for a group without one, `computeGroupMind` over the current members' picks), and `mostPicked` by rarity (up to three cards picked by at least two players). `?view=summary` returns only the viewer's player row without picks, the Group Mind's mythics and empty most-picked lists, for the group home. `mtgSetPayload` adds `revealOpen` (`now >= lock_at`) and `lockedAt`.
- **Picks revealed** (`processMtgRevealedEmails`, template `mtgRevealed.html/.txt`, subject `mtgRevealed`, outbox kind `mtg_revealed`, log type `revealed`): after the lock job has run, from `mtgRevealedEmailAt(lock_at)` for 36 hours, one set-based statement logs every active member of an active challenge community who is not logged yet (up to 2,000 per run) and queues rows for those with the preference on. The payload holds the set name, the lock label, `firstStandingsAt`, the links and `sendBy`. At delivery `mtgRevealedDetails` reads the pick count, the awarded badges (entry badges, plus group honors from groups the player is still in, labelled like "Early Bird in FRA Crew") and each group's `revealFunFact` and entry count, cached for the rest of the outbox pass so a group's picks are read once; the standings line is chosen from `firstStandingsAt` and the current time; a row past `sendBy` is marked `gave_up`.
- **Web:** `/communities/[slug]/reveal` renders `web/src/components/mtg/reveal/RevealView.tsx`: rarity tabs; the Group Mind with votes and a note on whether it was set at the lock; Most picked chips; a memoised card per player with five thumbnails (or a one-line note when they picked nothing at that rarity), *Only you* and *Solo* under solo cards that get an orange frame (Scryfall images are never covered), Receipts, then badge chips; the no-entry list; `CardViewer` with `hideAction`; and one `ConfettiBurst` per season per browser (a `localStorage` flag). `BadgeChip.tsx` shows tier colors from `BADGE_TIER_STYLE` and its reason in a `TapTooltip` with the new `describeChild` option, and takes keyboard focus. `RevealSummary.tsx` (fetching `?view=summary`) replaces *Who's finished* on `MtgChallengeHome` once the lock passes, and the home's minute tick makes that switch without a reload; after the lock the card pool grid folds behind *Browse the cards* and fetches cards only when opened. The locked wizard's notice links to the Reveal. `CardViewer` labels the dialog paper and gives focus back to the Previous or Next button after each card. Types live in `mtgTypes.ts` (`MtgRevealPayload`, `MtgBadge`, `MtgMindPick`).
- **For later batches:** the award identity index leaves out `status`, so a final-day award of a badge with an `on_track` row must use `ON CONFLICT (set_id, user_id, (COALESCE(community_id::text, '')), badge_code, award_key) DO UPDATE SET status = 'awarded'`. The leaderboard's Group Mind ghost should read `mtg_group_minds` and fall back to `computeGroupMind` over the current members, as the Reveal does.

**Stats ingest, scoring and the leaderboard (Batch 5, migration 129).**

- Schema: `mtg_snapshots` (one row per set per Eastern `snapshot_date`, with `source` feed or paste, `raw_key`, `raw_json`, `total_games`, `matched`, `pool_size`, `scoring_version`, `taken_at`, `scored_at`, `is_final`), `mtg_ingest_runs` (every attempt: `trigger` schedule, admin or paste; `outcome` published, not_newer, fetch_failed or failed_validation; `notes`, `raw_key`, `total_games`, `matched`, `alerted`, `alert_kind` first or final, `dry_run`, `snapshot_id`), `mtg_card_stats` (per snapshot and pool card: `gih_games`, `gih_wr`, `adj_wr`, `rarity_rank`, `ranked`, `card_score`, `alsa`, `ata`, `iwd`), `mtg_entry_scores` (per snapshot and entry: `total`, the four rarity subtotals, `slot1_points`), and `mtg_cards.stats_arena_id` / `stats_name` match overrides. Select `snapshot_date::text`: the driver would otherwise turn a DATE into a local-midnight Date.
- Pure helpers in `api/src/lib/mtgScoring.ts`, tested in `__tests__/mtgScoring.test.ts`: `parseCardDataFeed` (wrapper or bare list, content type ignored, refuses non-JSON, missing fields and win rates outside 0 to 1), `normalizeCardName`, `matchFeed` (override, then Arena ID, then a name unique in the feed), `checkSnapshot` (95% match, no falling total, at most five shrinking cards, not newer when identical or empty; `force` skips all but an empty match), `computeCardScores` (spec 12.4, ties by games then collector order), `scoreEntry` (cards outside the pool score 50, totals rounded to four decimals), `mtgIngestSlot` (9, 11, 13, 16 and 20 ET inside the window), `mtgStandingsDay` and `rankStandings` (points, #1 points, completed first).
- `processMtgIngest` runs in the hourly pass right after the lock job. `runMtgIngest(sql, env, set, { trigger, text?, force?, date?, dryRun? })` fetches `feed_url` with `MTG_USER_AGENT` and a 30-second timeout (or takes pasted text), archives the body to R2 `mtg/17lands/<code>/<date>-<ms>.json`, parses it, matches the scoring pool (in the pool and not voided), compares it with the latest snapshot on or before today, and records the attempt. A published day is written in one `sql.transaction`: the snapshot upsert, `mtgSnapshotWrites` (stats for every pool card, points for every entry with picks) and its run row. Scheduled attempts skip a published day; admin attempts replace it. `date` (checked by `mtgIngestDateAllowed`, which unlike `mtgIngestWindow` doesn't ask what day it is now) publishes a named day of the season; `dryRun` archives and logs the attempt but writes no snapshot. `rescoreMtgSnapshot(sql, set, date, shared?)` re-parses a day's `raw_json` with the current pool, overrides and picks, and `rescoreAllMtgSnapshots` runs it over every non-final day oldest first from one read of the pool and picks. `alertMtgIngest` marks the day's latest run `alerted` and sends `sendMtgIngestAlertEmail` (template `mtgIngestAlert`) to the admin inbox, then reports a `Sentry.captureMessage` warning. The claim is scoped by `alert_kind` and only takes a run row whose `alert_kind` is still null, so a day sends one `first` (a failed check) and one `final` (the 8 PM attempt without new standings), each pinned to its own attempt; dry runs are never claimed. `sendResendEmail` throws on anything but a 2xx, so a thrown error is the only signal that nothing arrived: it releases the claim, and goes to Sentry itself.
- Admin routes (super admins): `POST /admin/mtg/sets/:code/ingest { force?, date?, dryRun? }`, `POST /admin/mtg/sets/:code/ingest/paste { raw, force?, date?, dryRun? }` (raw capped at 2 MB), `POST /admin/mtg/sets/:code/snapshots/:date/rescore`, `POST /admin/mtg/sets/:code/snapshots/rescore-all`, `GET /admin/mtg/sets/:code/stats` (recent runs, published days, the latest day's unmatched cards and records, overrides and voided cards), `PUT /admin/mtg/cards/:id/stats-match { statsArenaId, statsName }` and `PUT /admin/mtg/cards/:id/voided { voided }`, rendered on *MTG Seasons* by `web/src/app/(app)/admin/mtg/AdminMtgStats.tsx`.
- `GET /mtg/communities/:id/leaderboard?date=` (members and super admins, after the lock; `standings: null` before the first day) ranks the day and the day before with `rankStandings`; the previous day's ranking excludes members who joined the community after that snapshot was taken, so a new arrival isn't slotted into it after the fact and doesn't push everyone below them down a place, and returns rows (`rank`, `previousRank`, `total`, `change`, `behind`, rarity `subtotals`, top three `badges` and `badgeCount`), `noEntry`, `groupMind` (stored picks, or `computeGroupMind` over the members, scored with that day's Card Scores), `randomPicks: 1000` and the published `dates`. `mtgSetPayload` adds `standings` (latest date, time taken, day and total days).
- Web: `web/src/components/mtg/leaderboard/Leaderboard.tsx`, shown by `MtgChallengeHome` to members once `set.standings` exists, above the Reveal summary; types `MtgLeaderboardPayload`, `MtgLeaderboardRow` and `MtgStandingsSummary` in `mtgTypes.ts`.

- **From the review, before shipping:** `parseCardDataFeed` requires both GIH fields, so a renamed win-rate key fails instead of scoring every card 50, and an empty list is zero records rather than an error; `checkSnapshot` fails a day where more than a tenth of the cards with `MTG_WELL_PLAYED_GAMES` or more games have no win rate, and compares growth only over cards present in both days; `runMtgIngest` refuses dates outside `mtgIngestWindow` and any day already `is_final`, and treats an empty feed as `not_newer`; the publish transaction and `mtgSnapshotWrites` find the snapshot by `(set_id, snapshot_date)` rather than a generated id, so losing the upsert race can't orphan the child rows; `rescoreMtgSnapshot` aborts through a `1 / 0` guard on `taken_at` when the day was replaced mid-way, and returns typed 404s and 409s; `processMtgIngest` records a thrown attempt as `outcome = 'error'`, reports it to Sentry and alerts on the day's last attempt; `alertMtgIngest` releases its claim when the send fails, and the email says whether standings already exist and whether more attempts are due today; `syncScryfallSet` may add pool cards after the lock but never drops one or changes a rarity; and `GET /admin/mtg/sets/:code/stats` builds its lists from the newest archived response in R2 (falling back to the latest published day) and returns `listsFrom`. On the web, the leaderboard reloads when the tab returns and every half hour and appears from the Arena launch; the admin's publish-anyway is offered per failed attempt instead of a standing checkbox, and matching or voiding a card re-scores the latest day straight away.
- **From the post-ship review (migration 130):** `mtg_ingest_runs` gained `dry_run` and `alert_kind`. On the API, an admin can name a day (`mtgIngestDateAllowed`) or rehearse a fetch (`dryRun`), which is what fills the match queue before day one; `rescoreAllMtgSnapshots` re-scores every non-final day after a match or a void; the re-score guard counts rows and pins on the epochs of both `taken_at` and `scored_at`, so a concurrent re-score aborts too; alerts are claimed per kind; the paste cap fell to 2 MB; a failing `processMtgCardSync` reports to Sentry; `syncScryfallSet` retries a 429 or 5xx twice and, before the lock only, drops pool cards Scryfall no longer lists; and the leaderboard's previous ranking ignores members who joined since. On the web, matching or voiding re-scores every day and each published day has its own *Re-score*, the toolbar gained *Dry run* and a *Day* field, publish-anyway is offered only for a failed check (and names what forcing costs), attempt times are Eastern and carry the day they were for, a match is confirmed before it is saved, the rarity filter says when it fell back to every record, and the card list can be retried. On the leaderboard, badge dots and counts show on phones (spec 10.5), the Group Mind ghost truncates like the rows above it, a failed refresh is visible, "Last updated" waits until 10 AM before calling a day stale, and an empty group gets no ghost rows.

**Player and card pages, the Everyone board (Batch 6, no migration).**

- `rankGroupDay(members, scores, joinedBy?)` in `api/src/lib/mtgScoring.ts` ranks one day of a group's standings: the members with points that day and, given `joinedBy`, only those who had joined by then. The leaderboard ranks through it (today with no cutoff, the previous day cut off at that snapshot's `taken_at`) and so does the player page, so a player's rank and movement there always match their leaderboard row. `mtgGroupAccess` (a signed-in viewer, a live challenge group, an active member or a super admin) and `loadMtgGroupMembers` are shared by the leaderboard and the new routes.
- `GET /mtg/communities/:id/players/:userId`: members and super admins; 403 `SEALED` with `lockAt` for another player's page before the lock, while your own opens; 404 unless the player is an active member. Returns `player` (with `hasEntry` and `pickCount`), `standing` (rank, previous rank, players ranked, total, change, behind and rarity subtotals; null before the first standings), `history` (a point per published day, oldest first, each ranked as the group was that day), `picks` by rarity (slot, multiplier, note, card, the latest day's `stats` from `mtg_card_stats`, `cardScore` with a neutral 50 when a card has no stats row, `points`, and `trend` against the day before), `top` (ranks 1 to 5 per rarity on the latest day), `compare` (the viewer's own picks, on someone else's page after the lock) and the awarded `badges` (entry badges plus this group's honors).
- `GET /mtg/communities/:id/cards/:cardId`: members and super admins; the card with `inPool` and `voided`, `latest` (only when the card has a row on the latest day), a `history` of rank, ranked count, Card Score, GIH WR and games, `pickedBy` (active members with slot and note; null before the lock) and `links` to Scryfall and 17Lands. It replaces the planned `GET /mtg/sets/:code/cards/:id/history`, because *Picked by* needs a group.
- `GET /mtg/sets/:code/everyone`: signed-in players; the latest day's `mtg_entry_scores` for every entry with `hide_from_everyone_board = false`, ranked with `rankStandings` and sent as `{ rank, handle, total, isViewer }` only: the top 100 (`MTG_EVERYONE_ROWS`), `viewerRow` when the viewer ranks below them, `players`, and `viewer` (`hasEntry`, `hidden`). `PUT /mtg/sets/:code/entry/everyone { hidden }` sets the flag at any time, before or after the lock, without touching `updated_at` (a standings tie-break); 404 `NO_ENTRY` without an entry.
- Web: `/communities/[slug]/players/[userId]` renders `web/src/components/mtg/player/PlayerView.tsx` (standing tiles; points over time with the random-picks line; badges; a block per rarity with each slot's thumbnail, card link, GIH WR and games, rank such as "3rd of 81 commons", Card Score with a trend arrow, points and Receipt; *Actual top 5 right now* folds out and *Compare with me* adds the viewer's pick under each slot). `/communities/[slug]/cards/[cardId]` renders `card/CardPageView.tsx` (the card whole with a flip for double-faced cards, stat tiles, rank over time with first at the top, *Picked by* linking to player pages, Scryfall and 17Lands links). `charts/HistoryLineChart.tsx` is the Recharts line both use, drawn to the dataviz rules: a 2 px line in the primary color (validated against the card surface), a ringed end dot labelled with the latest value, a solid hairline grid, a crosshair tooltip, and a *Show the numbers* table so no value lives only in the tooltip. `Leaderboard` gained *This group* and *Everyone* tabs (`leaderboard/EveryoneBoard.tsx`, with the *Show me on this board* switch) and a *See ... picks and stats* link in each opened row; `MtgChallengeHome` passes it `slug` and `setCode`. `challengeGroup.ts` resolves a slug to its challenge group, and `pageBits.tsx` holds `IconTitle` and `StatTile`. Both pages end with the attribution line.

**Edits (2026-09-16).** Players see *MTG Card Evaluation Challenge* everywhere (UI, emails, attribution, user agent); the specialization value stays `mtg_prediction_challenge`. `PUT /admin/mtg/sets/:code` refuses a `lock_at` later than `prerelease_start_at` ("Picks must lock before prereleases start"). `mtgTimeline` drops the `picks_open` entry (`mtgPicksOpen` already opens picks at the earlier of previews and `picks_open_at`), labels the first entry "Previews start, picks open", puts the lock ahead of prereleases that start at the same moment, builds weekly entries with `easternToUtc` so they stay at 10 AM ET after the clock change, and no longer sets `calendar`. The lock-warning subjects are "Picks lock tomorrow" and "Picks lock today". On the web, `SeasonTimeline` has no calendar links, centres each dot in a 24 px title line and colours *Picks lock* red; `MtgChallengeHome` renders `BlankStandings` in the phase card until standings go live, the *How scoring works* button, *Pick status*, a one-request pool (`GET /mtg/sets/:code/cards` without `rarity`) with placeholder tiles, and a `CardViewer` popup; `CommunityDetailClient` hides the header's *Start a plan* for challenge communities (the app shell's sidebar button stays) and sends `hobbies` to `/explore/local-signal`. The theme's `MuiOutlinedInput` input padding overrides MUI's zero left padding beside a start adornment, so `CardGrid`'s search clears `.MuiInputBase-inputAdornedStart` padding locally; other adorned inputs still carry the wider gap.

**Season badges and the on-track refresh (Batch 7, no migration).**

- `computeSeasonBadges({ cards, entries, groups })` in `api/src/lib/mtgBadges.ts` is pure and judges one day: `cards` are the day's `mtg_card_stats` rows (rank, ranked count, Card Score, ALSA), `entries` the entries with points (total, subtotals, #1 points, completion times, `hidden`, picks with notes), and `groups` each challenge group's standings on every published day (`rankGroupDay`, a past day cut off at its snapshot's `taken_at`) with its Group Mind. It returns `{ userId, communityId, code, key, detail }` rows: `communityId` is set for group honors and for Beat the Crowd, Wire to Wire, Told You So and Lone Wolf; `key` is the rarity for the badges that stack per rarity (Clean Sweep, Perfect Order, Sniper, Called It, Bust) and empty otherwise. Constants: `MTG_BADGE_MIN_RANKED` (a rarity counts toward rank badges once more than 10 cards are ranked), `MTG_HONOR_MIN_PLAYERS` 3, `MTG_BIG_GROUP_PLAYERS` 4, `MTG_EVERYONE_MIN_ENTRIES` 20 and `MTG_RANDOM_PICKS_POINTS` 1,000. The Everyone board ranking for Oracle and Sharp Eye is `rankStandings` over entries that aren't hidden. Every rule is unit tested in `__tests__/mtgBadges.test.ts`.
- `MTG_BADGES` in `api/src/lib/mtg.ts` now holds all 46 badges; `badgeDescription(code, detail, onTrack?)` writes each reason from `detail` (past tense when earned, present when on track; card lists show three cards and count the rest; it falls back to the catalogue line when the detail lacks something), with `ordinal` exported beside it.
- `refreshMtgOnTrackBadges(sql, set)` in `api/src/index.ts` loads the latest day's stats and entry scores, every pick with its note, each active challenge community's members with entries, their scores on every published day and the stored Group Minds (or `computeGroupMind` over the members when a group has none), runs `computeSeasonBadges`, and in one `sql.transaction` takes `pg_advisory_xact_lock(hashtext('mtg-on-track:' || set_id))`, deletes the set's `status = 'on_track'` rows and inserts the new ones through `UNNEST ... ON CONFLICT DO NOTHING`. It does nothing once any snapshot `is_final`. `refreshMtgOnTrackBadgesSafely` wraps it (console and Sentry, never throws) and runs after a published `runMtgIngest` (outside its publish transaction), after `POST /admin/mtg/sets/:code/snapshots/:date/rescore` succeeds, and at the end of `rescoreAllMtgSnapshots` when a day was re-scored.
- Reads go through `mtgBadgeList(rows)`, which stacks rows by status, community and code into `{ code, name, tier, description, groupHonor, onTrack, count }` (earned before on track, then `compareBadges`, a stack's reasons joined mythics first). The leaderboard reads `status = 'awarded'` only and counts `badgeCount` in stacks; the player page reads `status IN ('awarded', 'on_track')`.
- A weekly standings email (`processMtgWeeklyEmails`, outbox kind `mtg_weekly`, `POST /admin/mtg/sets/:code/weekly-email`) shipped with this batch and was removed the same day, along with the timeline's weekly entries; production never queued one.
- Web: `web/src/components/mtg/badgeIcons.tsx` maps every badge code to a MUI icon: `BadgeGlyph` renders it as an element (so no component is chosen at render time, and a Chip's icon class passes through) and `BadgeIcon` draws it in a tier-colored disc, dashed when on track. `BadgeChip` shows the icon, dashed outlines for on-track badges and "×N" for stacks. The leaderboard's `BadgeHints` shows up to three icons (24 px, or 20 px overlapping by 5 px below the `sm` breakpoint) and "+N". `PlayerView`'s `BadgesCard` splits *Earned* and *On track* with a plain-words caption. `MtgBadge` gained `onTrack` and `count`.

**Batch 6 and 7 review (2026-09-16).** The weekly standings email was removed (see the Batch 7 notes above). Then:

- `mtgJoinCutoff(nextSnapshotDate)` in `mtgScoring.ts` (9 AM ET on the next published day's date) replaces `taken_at` as the join cutoff for a past day everywhere a past day is ranked: the leaderboard's previous day (and the requested day with `?date=`, whose `noEntry` list also respects it), the player page's previous day and history, and `refreshMtgOnTrackBadges`. Dates, not publish times, so re-publishing a day doesn't change who counted, and a member who joins after a day's publish isn't dropped from it again the next day. Leaderboard `change` and the player page's standing `change` come from the entry's own previous-day score; only `previousRank` uses the cutoff.
- `mtgGroupAccess` is one query (user, community and membership), and the leaderboard now uses it. The leaderboard, player and card routes run their independent reads with `Promise.all` through `mtgRows<T>(query)`, a typed wrapper for Neon's query promise. The player route folds the latest and previous days' card stats into its picks query with two `LEFT JOIN`s, selects the rarity subtotals only for the latest day, indexes scores by snapshot once, and returns `voided` on each pick. `loadMtgGroupMembers` also returns `hidden` (`hide_from_everyone_board`), and the player page leaves on-track Oracle and Sharp Eye off a hidden player's badges.
- `refreshMtgOnTrackBadges` reads everything in one `sql.transaction([...], { isolationLevel: "RepeatableRead", readOnly: true })`, including a fingerprint of the set's snapshot ids and `scored_at` epochs and each snapshot's largest `ranked` count (`judgedDays`). The write transaction takes the advisory lock, then aborts through a `1 / 0` guard when the fingerprint changed, so an older verdict can't overwrite a newer one (`refreshMtgOnTrackBadgesSafely` logs that as expected, not to Sentry), and its insert skips users and communities deleted since the read.
- `computeSeasonBadges` takes `judgedDays` and `SeasonCard.adjWr`. `judgedRarities` (more than `MTG_BADGE_MIN_RANKED` ranked) gates the subtotal honors per rarity, the standings honors (Champion, Runner-Up, Third Place, Wooden Spoon, Photo Finish, Beat the Crowd, King of the Hill, Wire to Wire, Rollercoaster, Comeback Kid) on any rarity, and Monkey Business on all four. Day-based honors count only days in `judgedDays` with at least `MTG_HONOR_MIN_PLAYERS` ranked players, looked up through a rank `Map` per day. Told You So needs three players, Wooden Spoon four, and Pick of the Season breaks point ties on `adjWr`. `badgeDescription` shows the decimals that keep two totals apart (`pointsPair`) and adds "and N more" to Pick of the Season and Whiff of the Season.
- Web: `challengeGroup.ts` caches a group's id and name in `sessionStorage` (`rememberChallengeGroup`, written by `MtgChallengeHome`, which now takes `communityName`) so player and card pages skip `GET /communities/:slug`, and it throws on a failed lookup instead of reporting a missing group; both pages offer *Try again*. `pageBits.tsx` exports `srOnly`. `BadgeChip` carries its reason in visually hidden text and passes the tooltip title as an element, so MUI adds no `title` attribute. `Leaderboard`'s `BadgeHints` shows the best icon plus a count below `sm`; `GroupMindRow`, `RandomPicksLine` and Everyone rows speak through hidden text rather than `aria-label` on a list item; `UpdatedLine` names the date once the season is over; the no-standings text says when the first standings run late (`firstStandingsAt` prop); and `EveryoneBoard` mounts on first open, stays mounted, reloads when `standingsDate` changes, applies a saved switch at once, guards double taps with a ref instead of `disabled`, and shows a failed refresh. `HistoryLineChart` sets `accessibilityLayer={false}` inside a `figure role="img"`, labels the reference line under the plot, uses round rank ticks, and gives *Show the numbers* 44 px and a table caption. `PlayerView` and `CardPageView` use whole-row links (an `::after` overlay on the name link, with the compare box raised above it) with chevrons, `smallCardImage` thumbnails, a `srcSet` for the card image, plainer copy, and memoised `cardHref` and chart points. `BADGE_TIER_STYLE` borders for uncommon, rare and shame are darker, for 3:1 on white. `MtgChallengeHome` shows "Final day" copy until the final standings exist.

**The season's end, rosters, past seasons and the trophy case (Batch 8, migration 131).**

- Migration 131: `mtg_group_rosters (community_id, set_id, user_id, added_at)` with its primary key and an index on `(user_id, set_id)`; `mtg_sets.finalized_at`; and the SQL function `newchums.mtg_season_player(community, set, user, joined_at)`: on the roster, or, for a group without a `mtg_group_locks` row, joined by `lock_at`. Apply it before deploying the API.
- Rosters: `ensureMtgGroupLock` reads only members who joined by `lock_at` and writes the ones with picks to `mtg_group_rosters` in the statement that claims the lock row, so a lost claim writes nothing. `loadMtgGroupMembers` returns `entry_id`, `completed_at`, `updated_at` and `hidden` only for members who play (`mtg_season_player`) and flags `joined_after_lock`; the leaderboard's live Group Mind picks, the card page's pickers, the Reveal, the reveal email's facts, the results email and `judgeMtgSeason` use the same function. `mtgJoinCutoff` and `rankGroupDay`'s `joinedBy` are gone: every day ranks the roster. The leaderboard's `noEntry` rows and the player route's `player` carry `joinedAfterLock`, and the player route returns no badges for a member who doesn't play. The Reveal reads only `MTG_LOCK_BADGE_CODES`. The Everyone board and the judge's `hidden` count an entry only when its player is an active member of an active challenge community.
- Ingest: `runMtgIngest` skips a final season, and an admin fetch (not a dry run) may name only today or, once it has ended, the final day. The re-score routes answer 409 `FINAL` for a final season.
- Judging: `judgeMtgSeason(sql, set, targetDate?)` does the on-track refresh's reads in one read-only RepeatableRead transaction and returns `{ target, hasFinal, fingerprint, badges }`; `mtgFingerprintGuard` and `mtgBadgeInsert(sql, setId, badges, "on_track" | "awarded")` are shared. `refreshMtgOnTrackBadges` does nothing for a season that isn't active or has a final day.
- Finalize: `finalizeMtgSeason(sql, set, targetDate?)` runs one transaction: the advisory lock, the fingerprint guard, an active-season `1 / 0` guard, `is_final` set on the target snapshot and cleared elsewhere, the on-track rows deleted, awards upserted with `ON CONFLICT (set_id, user_id, (COALESCE(community_id::text, '')), badge_code, award_key) DO UPDATE SET status = 'awarded', detail, awarded_at`, and the set made `final` with `finalized_at`. Errors: 409 `NOT_ACTIVE`, 404 `NO_STANDINGS`, 409 `CHANGED`. `processMtgFinalize(sql)` runs after `processMtgIngest` in the hourly pass for active sets past `final_at`, when `mtgFinalizeDue(set, latestDate, now)` holds: the final day's standings are published, or `MTG_FINALIZE_GRACE_MS` (26 hours) has passed; a finalize with an earlier day goes to Sentry. Super admins: `POST /admin/mtg/sets/:code/finalize { date? }` (409 `NOT_YET` before `final_at`) and `POST /admin/mtg/sets/:code/reopen` (409 `NOT_FINAL`; clears `is_final`, deletes every award but the lock's, sets the set active with no `finalized_at`, then refreshes the on-track badges). `PUT /admin/mtg/sets/:code` answers 409 `USE_FINALIZE` or `USE_REOPEN` instead of moving a season to or from final. `mtgSetPayload` adds `finalizedAt`.
- Results email: `processMtgResultsEmails` queues outbox kind `mtg_results` (log type `results`, group key `mtg:<code>:results`, payload `setName`, `sendBy`, `scoringUrl`) for finalized seasons from 10 AM ET on the final day (`mtgResultsEmailAt({ final_at })`) until three days after `mtgResultsEmailAt(set)`, the later of that and `finalized_at`. The start ignores `finalized_at` on purpose: it comes from the database's clock, which can run ahead of the Worker's, and waiting for it would hold the email an hour. Candidates are active members with an entry who play in a challenge group. At delivery `mtgResultsDetails` builds each group's line from `loadMtgGroupFinal` (the roster's final-day ranking and the group's awarded honors), cached per pass in `MtgResultsCache` with the next season's line (`mtgPicksOpenAt`); `sendMtgResultsEmail` renders `mtgResults.html/.txt` with subject `mtgResults` or `mtgResults_champion`, and links go to `/communities/<slug>/seasons/<code>`. A row with no final standings for its player is marked `gave_up`.
- Reads: `mtgSeasonParam(c)` takes `?set=` (`^[a-z0-9]{2,6}$`, otherwise ignored) for the results, leaderboard, player, card and Reveal routes. `GET /mtg/communities/:id/results` returns `set`, `community` and `results` (null until final): `date`, `day`, `totalDays`, `players`, `podium` (ranks 1 to 3 with avatars), `badges` (every awarded badge of the roster's players in the group, grouped by code and label, rarest first, recipients best-ranked first with `count` and `reason`) and `viewer` (`rank`, `total`, `name`, `badgeCount`, `topBadges`). `GET /mtg/communities/:id/seasons` lists the sets the group has rosters for plus the current one, newest first, with `final` and `isCurrent`. `GET /public/users/:handle/mtg-badges` (optional auth) returns awarded badges by season, each with `group` null, `{ name, slug }` or `{ private: true }`: named for the player, super admins and the group's members, or when the group is public, active and not invite-only and the player doesn't hide their communities.
- Tiers: Runner-Up, the four subtotal honors, Pick of the Season and Sleeper Agent are Uncommon, Third Place Common (`MTG_BADGES`). Tests in `__tests__/mtg.test.ts` cover `mtgFinalizeDue`, `mtgResultsEmailAt`, `mtgPicksOpenAt` and the tiers.
- Web: `web/src/components/mtg/results/SeasonResults.tsx` (the podium, first names below `sm`, the viewer's finish, `ShareResultsImage`, an optional `between` slot for the final standings, and `BadgeCeremony` with *Show all* after eight), `results/ShareResultsImage.tsx` (a 1,080 px PNG drawn on a canvas in the page's font after loading each weight it uses, measured and cut with "…" to stay inside the card, shown in a dialog with *Download* and, when `navigator.canShare` takes files, *Share*; object URLs are revoked on close and unmount) and `results/SeasonView.tsx` for `/communities/[slug]/seasons/[code]` (switcher, results with the season's leaderboard between, *See everyone's picks*). `MtgChallengeHome` shows the results once `finalizedAt` is set, last season's results until the next season locks (from `GET .../seasons`), a *Past seasons* card and the after-lock copy for non-members. `Leaderboard` takes `past` (adds `?set=` to its fetch and links) and lists late joiners; `PlayerView`, `CardPageView` and `RevealView` read the season with `seasonFromSearch(useSearchParams())` and send back buttons to the season page. `mtgTypes.ts` adds `MtgResultsPayload`, `MtgCeremonyBadge`, `MtgSeasonRef`, `MtgTrophyCasePayload`, `seasonQuery` and `seasonPageHref`. `publicProfile/ProfileMtgBadgesSection.tsx` renders the trophy case in `PublicProfileView` below Kudos. `AdminMtgClient` adds *Finalize now* and *Reopen the season* with confirms and disables the status options they own.

**Batch 9 review (2026-09-17, migration 132).**

- Card pool hotfix (4bc3903, shipped ahead of the rest): `syncScryfallSet` narrows the pool to `booster = true` only once some printing in the set carries the flag, and before the lock refuses to take out more than `MTG_POOL_DROP_LIMIT` (a tenth) of the pool, and always more than three cards, in one run. Such a run keeps the pool, adds a note and sets `summary.poolHeld`, which `runMtgCardSync` reports as a Sentry warning. Unit tests in `__tests__/mtg.test.ts` stub `fetch` and the `sql` tag.
- Migration 132: `mtg_sets.reopened_at`; `mtg_ingest_runs.alert_kind` allows `catchup`; `idx_mtg_badge_awards_user (user_id, set_id)` for the trophy case; and `newchums.mtg_group_season_players(community, set)` returning `(user_id, joined_at, left_group)`: active members who play the season, plus, for a finished season (`status <> 'active' AND finalized_at IS NOT NULL`), roster players who have since left. The Reveal's picks and the card page's `pickedBy` join it. `loadMtgGroupMembers` unions the departed roster players in (`left_group`) and leaves out members who joined after a finished season's lock, so the leaderboard, player page, results and the Reveal's player list keep a finished season as it ended.
- Lock: `processMtgLock(sql, only?, scheduledTime?)`. `handleScheduled` passes `event.scheduledTime`; when the run is scheduled at or up to a minute before `lock_at`, the job waits until 30 seconds past the lock instead of skipping to the next hour.
- Ingest: `mtgIngestSlot` returns `catchUp` on the day after the final day, at the same hours and with the final day's date; `processMtgIngest` selects seasons up to two days past `final_at`, skips a catch-up slot once the final day is published, runs it with `date`, and alerts only on the catch-up day's last attempt (kind `catchup`). `sendMtgIngestAlertEmail` takes `stage` (`day`, `final_day`, `catch_up`), `finalizeAtLabel` and `latestLabel`. `MTG_FINALIZE_GRACE_MS` is 36 hours.
- Finalize and the season's end: `mtgFinalizeDue` returns false while `reopened_at` is set; `processMtgFinalize` selects `reopened_at IS NULL`; `finalizeMtgSeason` clears it; the reopen route sets it and accepts final or archived seasons. `POST /admin/mtg/sets/:code/finalize` answers 409 `NOT_YET` until the final day's standings are published or 8:05 PM ET on the final day, and 409 `NOT_LATEST` for a `date` other than the latest. `mtgSeasonOpenGuard(sql, setId)` (a `1 / 0` guard: season active, no `is_final` day) joins the season's advisory lock in the publish transaction, the re-score transaction and the on-track refresh write; a publish that trips it returns `skipped`, a re-score returns `CHANGED`, and "re-score all" stops at the first `CHANGED`. Ingest, re-score and results emails treat any non-active season as over, not just `final`.
- Results: `MtgResultsCache.seasons` holds `"waiting"` for a season that isn't final, and the `mtg_results` outbox branch leaves such a row pending, as it does when reading the details throws. Champion lines use the group's `champion` award holders, and the results route's `viewer` carries `champion`. The Reveal gives lock badges only to players with picks. `loadMtgSet(sql, null)` orders non-active seasons newest first.
- Timeline: the previews entry ends at `lock_at`; the final entry is `now` while the season is active and `done` from `finalized_at`.
- Web: `SeasonResults` renders `between` (the final standings) while the podium loads or fails, keeps its Try again button mounted, drops stale answers through a request counter, links a single player's podium step as a whole, reads "You're the champion of <group>" from `viewer.champion`, and dates carry the year. `ShareResultsImage` shares the file alone, names downloads with the set code, draws medal numbers in dark ink, titles the dialog for non-players, announces drawing in a status region and tells in-app browsers to press and hold. `SeasonView`'s switcher isn't full width. `MtgChallengeHome` shows "Final standings on the way" after the final date, ticks every second in the two minutes around the lock, gates Past seasons on membership, and uses `text.secondary` for the attribution and lock date. `Leaderboard` stops refreshing final standings and uses the past tense for them. `PlayerView` says "Top 5 on the final day". `SeasonTimeline` shows midnight-Eastern milestones as Eastern dates. Copy: no "entry" or "sealed" in the Reveal, review step, wizard or How Scoring Works; `notificationConfig.ts` lists all four challenge emails; the pick search reads "Search" on phones. Headings: section titles render `h2` (`ProfileSectionHeader` too, with a readable subtitle). `AdminMtgClient` shows a Reopened chip and passes `over` to `AdminMtgStats`, which disables fetch, paste, re-score, match and void for a finished season.

**QC pass before friends play (2026-09-17, migration 133).**

- Stale saves: `mtg_entries.revision` moves on in every `writeMtgPicks` (player saves and housekeeping). `writeMtgPicks(sql, entryId, picks, { playerChange, setId, lockCleanup?, expectRevision? })` adds a `SELECT ln(CASE WHEN e.revision = $expected THEN 1 ELSE 0 END)` guard, so a stale write aborts with "cannot take logarithm of zero" (`isMtgStaleWrite`), told apart from the lock's division by zero (`isMtgLockedWrite`). `PUT /mtg/sets/:code/entry` takes `baseRevision` (optional, for pages loaded before the deploy), returns 409 `STALE` before writing when it's behind and again if a race trips the guard, checks the unchanged signature first (a retried save whose response was lost), and returns `entry.revision`. `GET` returns `revision`; its pool clean-up writes with `expectRevision` and re-reads once if a save landed in between. `PickWizard` keeps `baseRef`, sends it, shows a toast and resyncs on `STALE`, ignores a resync answer when a change was made while it was out, queues the unmount and pagehide flush behind a save in flight, resyncs when the tab becomes visible with nothing unsaved, and clears the card lists after ten minutes hidden.
- Save shape: the PUT reads at most `MTG_ENTRY_BODY_MAX` (32,000) characters (413 `TOO_LARGE`, by `Content-Length` first) and rejects unknown rarity keys and lists over five before loading the season or any card.
- Completion time: `mtg_entries.prior_completed_at` and `incomplete_since`. A player's save below twenty moves `completed_at` into `prior_completed_at` and stamps `incomplete_since` (only when it was complete); back at twenty, `completed_at` is `prior_completed_at` when `incomplete_since` is null (a housekeeping drop, which leaves it null) or under 30 minutes old, else `now()`.
- Valid pick counts: the progress route (unless `e.locked_at` is set), the welcome email count and `mtgLockWarningDetails` join `mtg_cards` and count only `in_pool AND NOT voided AND c.rarity = p.rarity`. Progress and the lock warning's group counts leave out `users.is_suspended`.
- Season email delivery: `processEmailOutbox` selects `to_suspended` and `wants_mtg` (`user_profile.notification_prefs`) and marks an `mtg_*` row `gave_up` when either blocks it. The lock warning, revealed and results queues fold `NOT is_suspended` into `wants_email`. The lock warning's complete line is "All 20 of your picks are in. You can still change any of them until picks lock."
- Card pool: `mtg_cards.missing_since`, `MTG_POOL_MISSING_GRACE` ("1 day"). Before the lock the upsert keeps an in-pool card that no longer qualifies in the pool and starts its clock, taking it out once the clock is a day old (unless the run is held); a qualifying card clears it. The unlisted-cards update starts the clock and takes cards out after a day (`in_pool = held OR clock < 1 day`). The hold guard counts only in-pool cards overdue by a day. Each card's upsert is in its own try/catch (`summary.failed`, a note, and a Sentry warning from `runMtgCardSync`).
- Ingest: an empty `data` list is `failed_validation`. `checkSnapshot` takes `poolNames` and `pickedIds`: previously matched pool cards that are unmatched now add to `shrinking` and are named in the reason; any unmatched picked card fails the day; a card or total counts as losing games only past `max(10, 0.5%)`, and a total within that reads as `not_newer`. `runMtgIngest` loads picks before the checks, refuses a named day after today's Eastern date (`skipped`) unless it's a dry run, and a dry run with no day before the window opens uses the day after the Arena launch. `loadMtgScoringPool` orders by `collector_sort, collector_number, id`.
- Season's end: `judgeMtgSeason` returns `players` (community and user pairs it judged); `finalizeMtgSeason` sets `mtg_group_rosters.in_final` for the season in the same transaction. `mtg_group_season_players` (migration 133) returns, for a finished season, active members who play it minus rosters marked `in_final = false`, plus departed roster players not marked false (NULL keeps seasons finalized before the column). `loadMtgGroupMembers` derives `plays` from that function, so every finished-season surface reads the same list.
- Communities (shared code): `POST /communities/:id/leave` deletes only an `active` membership, so a `removed` row survives. `DELETE /communities/:slug` returns 409 `MTG_SEASON_HISTORY` for a non-super-admin when the group has `mtg_group_rosters` rows.
- Trophy case: `GET /public/users/:handle/mtg-badges` shows `mtgBadgeList` names and reasons only to the player, super admins and signed-in viewers who share an active challenge group with the player; others get `MTG_BADGES[code].name` and `.description`.
- Admin guards: `PUT /admin/mtg/sets/:code` rejects a changed `lock_at` once the current one has passed (409 `LOCK_PASSED`), a new or changed `lock_at` in the past for an active season (400), and an `arena_release_at` at or before the lock (400). `GET /mtg/sets/:code/everyone` returns `standings: null` before `lock_at`.
- Auth (shared code): `getSafeRedirectPath` also rejects backslashes and control characters (a browser reads `/\host` as `//host`). `withNextParam(path, next)` appends a safe `next`; the login page's *Create an account*, the signup page's *Sign in* and verification request, the pending page's buttons and resend, and the verify page's *Continue to sign in* carry it. `POST /auth/email-verify/request` accepts `next` (same rules, at most 1,000 characters) and adds it to the verification link.
- Community page (shared code): for a challenge community `CommunityDetailClient` builds `tabIndexMap` as Challenge 0, Members 1, Requests 2 (Plans, Announcements and Schedule -1), fetches only members, and tab changes go through `selectTab`, which keeps `?tab=` in step (also for *See all* members). A request approval or decline refetches the community, and a refused one shows its error. Challenge copy: the restricted preview rows, `InvitedJoinCard` and `CommunitySignupFooter` (`challenge` prop), the open-group heads-up under Join, the Leave and Remove dialogs, and no announcement mute. `generateMetadata` in `[slug]/page.tsx` describes a private challenge group (or a public one without a description) as the challenge. `EditCommunityClient` warns on close for a challenge community and hides its Community features card; Create and Edit reword *Approval required* for it. `MtgChallengeHome` drops `isOwner` and shows "We couldn't load the challenge." with Try again on a failed season load (the no-season answer is a 404 `NOT_FOUND`).
- Web: `fitNote(prev, next)` replaces `clampNote`: over 140 code points, an edit that isn't an append is refused and an append is trimmed by grapheme (`Intl.Segmenter`). A failed card list shows an error with *Try again* and doesn't stamp the rarity reviewed; a failed season load is an error, not "No season is set up yet". Refused saves settle to "Saved" after the resync instead of "Not saved, retrying". `RevealSummary` asks again every 5 seconds, up to six times, when the API answers `SEALED`. `formatWhenZoned` names the time zone on lock times in the picks, player and Reveal pages.

**Rob's review of the challenge pages (2026-09-17, migration 134).**

- Scoring: `MTG_SLOT_WEIGHTS` is gone. `scoreEntry` adds up the Card Scores of slots 1 to 5 (anything else is ignored) and keeps `slot1` (the #1 picks' points) as the first tie-break; Pick of the Season's points are the Card Score; the player route no longer sends `multiplier`, and a pick's `points` equal its `cardScore`. `computeGroupMind` still votes by slot (6 minus the slot). Nothing was scored in production yet, so `scoring_version` stays 1.
- Shortlist: `mtg_pick_shortlist (entry_id, rarity, slot 6..10, card_id, note)`, primary key `(entry_id, rarity, slot)`, `UNIQUE (entry_id, card_id)`, cascading from the entry and the card. `MTG_LIST_MAX` (10). `validateEntryPicks(input, pool, maxSlots = 5)`; the PUT validates with 10, closes gaps per rarity, and `writeMtgPicks` splits the list: slots up to 5 into `mtg_picks`, the rest into `mtg_pick_shortlist`, deleting and inserting both in the same locked transaction. `picked`, `complete` and `completed_at` count only `mtg_picks`. The GET entry, the unchanged-save signature and the lock job's pool clean-up read both tables as one list per rarity, so a dropped pick lets the next card move up. Everything else (scoring, badges, the Reveal, counts, emails) reads `mtg_picks` only.
- Notes: validation messages say "Your thoughts on a pick"; badge 36 is named Food for Thought (code `receipts_on_file`), and Told You So and Eats Words say "shared your thoughts".
- Web, shared: `ToastProvider` keys each toast by an id, so a new one restarts the four-second timer, and on phones the snackbar's bottom adds `var(--toast-bottom-offset, 0px)`. (An Undo action was tried and removed the same day at Rob's request.) `scrollPageToTop(behavior)` in `lib/scrollOffsets.ts` scrolls `#app-scroll-root` when it is the scroller, and the window. `components/mtg/pageBits.tsx` adds `BackButton` (a small ghost outlined link, `color="inherit"` so the theme's `outlinedInherit` hover keeps its text dark; the primary outlined hover fills orange with white text), `Notice` (a bordered panel with a tinted icon disc, in neutral, success, warning and error tones) and `StandingsHeader` (the Rank / Player / last-column captions over a standings list).
- Web, challenge home and pages: `MtgChallengeHome` drops the pool card, its fetch and the `CardViewer`, and renders `leaderboard/PreSeasonStandings` (members from `/progress` with a "?" rank and a pick-status line, skeleton rows while they load and a Try again on failure) until `standingsLive`. `CommunityDetailClient` builds the secondary actions once (`secondaryActions`; Share or Invite link and Edit are now the theme's outlined buttons, as on the plan page) and, for a challenge group's members (`actionsBesideMembers`), renders them at the end of `CommunityMemberPreview`'s row (or the member-count row before members load) and skips the divider and action row. `PreSeasonStandings` and `Leaderboard` show `StandingsHeader` over their rows. `Leaderboard`'s benchmark line reads "Random picks score about 1,000". `HowScoringWorksContent` drops the multiplier grid and the per-set date helpers. `PlayerView` and `CardPageView` show no multipliers, and a pick's points carry the Card Score's trend. `RevealView`, `PlayerView`, `CardPageView` and `SeasonView` use `BackButton` under their heading and skeletons while loading.
- Web, picks: `PickState` lists hold up to ten cards (`scoredCount`, `shortlistCount`; `totalPicked` counts the top five per rarity; `toPutBody` sends slots 1 to 10). `PickList` drops the up and down buttons and the multiplier labels, draws a "Shortlist, not scored" separator after #5 inside the sortable list, and opens a card from its row. `PickTray` puts `SaveStatus` on the title line; `SaveStatus` is keyed by state and fades "Saved" with a CSS keyframe after 4 seconds. `CardViewer` adds to the shortlist past five, replaces from all ten, and titles a two-faced card face by face with each face's cost (`cardFaces` splits name and `manaCost` on " // "). `CardGrid` marks shortlisted tiles in grey. `PickWizard` shows `PicksSkeleton` while loading, puts `BackButton` under the intro, uses `Notice` for the signed-out, read-only, dropped-card and card-load messages, confirms a removal with a plain `toast.success`, sets `--toast-bottom-offset` to 72px while the phone bar shows, and scrolls with `scrollPageToTop`. `ReviewStep` uses `Notice`, shows each rarity's top five with "Your thoughts?" fields and names its shortlist.

**Rob's phone test (2026-09-18, no migration).**

- Reordering: `useArrowReorder()` in `picks/PickList.tsx` is true below the `md` breakpoint or for `(pointer: coarse)`. When true, `SortableRow` renders no drag handle (its `useSortable` is disabled) and shows *Move up* and *Move down* icon buttons (36 by 40 px) before a slightly separated remove button; names clamp to two lines and the thumbnail hides below 360 px. The dnd-kit `TouchSensor` is gone: only the mouse and keyboard sensors remain. `PickTray`'s bottom bar has a contained *Reorder* button (`SwapVertRounded`) and its row of thumbnails is a `ButtonBase` that opens the same sheet; the hint says "move" or "drag" by mode. `ReviewStep` passes the five picks only, so *Move down* on #5 is disabled there.
- Notes removed: `ValidatedPick`, `SeasonPick`, `LockEntryInput` and the web's `PickSlot`, `MtgEntryPick`, `MtgRevealPick`, `MtgPlayerPick` and the card page's `pickedBy` lose `note`; `cleanPickNote`, `MTG_NOTE_MAX`, `fitNote` and `noteLength` are deleted; `validateEntryPicks` ignores a `note` key; `writeMtgPicks` no longer writes the column (`mtg_picks.note` and `mtg_pick_shortlist.note` stay in the schema, always null); the entry, Reveal, player and card routes return no `note`. Badges `told_you_so`, `receipts_on_file` and `eats_words` are removed from `MTG_BADGES`, `computeEntryBadges`, `computeSeasonBadges`, `badgeDescription`, `MTG_LOCK_BADGE_CODES` and `badgeIcons.tsx` (numbers 22, 36 and 45 stay unused).
- Community header (shared code): `accessTag(where)` and `nameSx` in `CommunityDetailClient`. From `sm` up the tag sits beside a one-line name as before; on `xs` the title row holds only the name (up to three lines, `overflowWrap: anywhere`) and the tag renders first in the hobby chips row, which now also renders for a private community without hobbies. Both the member view and the restricted preview use them.

**Join after sign-in (2026-09-18, web only, shared community code).**

- `web/src/lib/joinIntent.ts`: `rememberJoinIntent(slug)` writes `{ slug, at }` to `localStorage["nc:join-intent"]`; `takeJoinIntent(slug)` returns true once for a matching intent under two hours old, removing it (a stale one is removed and ignored, another community's is left alone). localStorage, not sessionStorage, because an email sign-up returns in a new tab from the verification email; not a query parameter, because a `?join=1` link would enrol any signed-in person who opened it.
- `CommunityDetailClient`: the signed-out Join buttons (`SignInToJoinCard` with `variant="join"`, the public page's action-row button unless the community is approval-required, and `CommunitySignupFooter`'s Sign up and Sign in when `instantJoin`) call `rememberJoinIntent` on click. An effect, once the community is loaded with `isAuthenticated === true`, runs from a zero-delay timer: `takeJoinIntent(slug) && canJoinInstantly` calls the current `handleJoin` through a ref. `canJoinInstantly` is: not closed, no membership, no pending request, not removed, and `join_mode === "open"` or (`invite_only` and `viewerInvited`). The invite code already rides `next` through sign-in, sign-up, verification and onboarding (`returnTo`).
- Labels: `joinLabel` and the cards' `challenge` prop give "Join this challenge" and "Request to join this challenge" for `mtg_prediction_challenge` communities.
...
- **Adopting an email-only invite onto an account MUST clear `email` in the same statement** (`SET user_id = ..., email = NULL`). The `event_invites_single_identity` CHECK (migration 075) requires exactly one identity column, so writing `user_id` alone throws; both adoption sites (`POST /auth/signup` and `GET /events/:id`) wrap the write in a non-fatal try/catch, so the violation was silent and email-invited users were simply never linked to their invite. Fixed July 2026.
...
- `newchums.users.username` nullability (migration 106). Migration 003 set it NOT NULL when every account came through credentials signup. Google OAuth later made a NULL username load-bearing: `getOrCreateAppUser` inserts the row with email + name only, and the `(app)` layout treats `username == null` as "send this user to /onboarding/username". Production had the constraint dropped without a migration, so databases built from `web/sql/` diverged and failed the first OAuth INSERT. Migration 106 records the drop; it is idempotent and a no-op on production.
...
- `newchums.events.run_again_nudge_processed_at` (migration 108), `TIMESTAMPTZ NULL`; stamped when the hourly "Run it again" scan considered the plan (sent or skipped), so a plan is nudged at most once ever. Backfilled for all pre-launch plans. Partial index `idx_events_run_again_pending` serves the scan.
...
- **Community roadmap tables** (`roadmap_items`, `roadmap_votes`, `roadmap_follows`, `roadmap_comments`, `roadmap_admin_notes`; migrations 045, 056, 066, 067, 072) were **dropped by migration 121** (Sept 2026) together with the pages, endpoints, follower emails and the `roadmap_updates` preference. `/roadmap` and `/roadmap/:id` redirect to `/contact?subject=Feature%20request`; the contact form preselects that subject from the query string. R2 objects under `roadmap_attachments/` are orphaned, nothing serves them.
...
- `newchums.users.share_link_modal_dismissed`: dropped by migration 109 (orphaned when the share moment replaced the first-use modal, Aug 2026).
...
- **Shout-outs** (`newchums.shoutouts`, migrations 073/074/076/113, and `users.is_hidden_shoutouts`) were **dropped by migration 121** (Sept 2026) and replaced by kudos, see the Kudos section under Post-plan wrap-up. Nothing was archived (Rob's call). Migration 121 also clears `shoutout_received` bell rows, pending outbox rows of that kind, the admin section timestamps, and completions of the renamed onboarding objective.
- `newchums.kudos_hidden_tags` (migration 125): `user_id` (FK CASCADE), `tag`, `created_at`, PK `(user_id, tag)`. A row exists only while that tag type is hidden from the recipient's public profile, so the default everywhere is visible. Presentation only: no kudos row is changed or deleted.
- `newchums.kudos` (migration 120): `id`, `plan_id` (FK CASCADE), `giver_user_id`, `recipient_user_id` (FK CASCADE), `tag` (validated in the API against `api/src/lib/kudos.ts`), `created_at`, `updated_at`, `notified_at` (stamped by the daily digest). Constraints `kudos_no_self` and `kudos_unique_per_slot UNIQUE (plan_id, giver_user_id, recipient_user_id)`; indexes on recipient, (giver, plan), and unnotified rows.
- `newchums.event_confirmations.email_opted_out` (migration 120), `BOOLEAN NOT NULL DEFAULT false`. Set by the attendance-check cron when the person has the `attendance_confirmation` email preference off: the row stays `pending` so the plan page can still take an answer, the roster shows "Going - Not asked" instead of "Didn't confirm", and the profile stat leaves the row out of its denominator unless they answer anyway.
...
- `newchums.product_events` (migration 103), append-only first-party product analytics (funnel mirror). Columns: `id` (BIGINT identity PK), `event_name` (TEXT), `user_id` (UUID NULL, FK `ON DELETE SET NULL`), `event_id` (UUID NULL, FK to `events` `ON DELETE SET NULL`), `params` (JSONB, small and flat), `created_at`. Index on `(event_name, created_at)` for windowed funnel aggregation, plus partial unique indexes enforcing the once-per-subject events (`rsvp_verified`, `signup_completed`, `first_plan_created`, `second_plan_created` per user; `plan_reached_3_rsvps` per plan). Application logic never updates rows; the only deletion path is the super-admin hard delete (below), which removes the subject's rows explicitly. FKs use SET NULL (not CASCADE) so funnel counts survive ORDINARY account/plan deletion while `DELETE /account` still leaves no dangling references. Full event catalogue in §12 "Product analytics events".
- `newchums.admin_audit` (migration 105), durable record of destructive/sensitive admin actions. Columns: `id` (BIGINT identity PK), `actor_user_id` (UUID, deliberately NO FK so the row survives everything), `action`, `subject_type`, `subject_id`, `subject_label`, `detail` (JSONB, impact counts), `created_at`. Written as the FIRST statement inside the hard-delete transaction so the audit commits atomically with the cascade. Actions: `user_hard_delete`, `plan_hard_delete`, `plan_chat_transcript_viewed`.
- **Plan-signup OTP + auth rate limiting (migration 104).** Additive columns on `email_verification_tokens`: `otp_hash` (TEXT NULL, sha256 of the 6-digit code; the magic-link `token_hash` is untouched and both credentials share one row), `otp_attempts` (INT, server-enforced 5-guess cap), `event_id` (UUID NULL, FK SET NULL) and `signup_intent` (TEXT NULL, CHECK going/maybe) persisting the invitee's chosen RSVP intent with the pending signup. New table `newchums.auth_rate_events` (`id`, `bucket`, `created_at`; index on `(bucket, created_at)`): append-only marker rows backing the Postgres sliding-window limiter in `api/src/lib/dbRateLimit.ts` (the KV limiter is a production no-op; repo-wide adoption is roadmap A3). Legacy flows on the shared token table (classic email-verify, signin-link) never set or read the new columns.
...
| `rsvp_intent_selected` | Invitee picks or changes Going/Maybe on the signup card (intent stage buttons or the chip row on later stages). | `status` ("going" \| "maybe") | GA |
...
| `signup_email_sent` | The plan-signup code + link email was accepted by the send provider (request or resend path). | `kind` ("initial" \| "resend") | First-party |
| `rsvp_otp_entered` | A 6-digit code submission finished on the plan page. | `result` ("ok" \| "wrong" \| "expired" \| "capped") | GA |
| `rsvp_code_resent` | Invitee tapped Resend and a new code email was issued. | none | GA |
...
| `share_link_copied` | The plan page's "Share plan link" button copied the tokenized share URL. Fires at tap time so a clipboard failure still counts the intent. `surface` is always `plan_page` since the share-moment dialog was removed (Aug 2026); older data also carries `post_publish` from the dialog era. | `surface` (`plan_page`; historical `post_publish`), `is_host` (boolean) | GA |
| `share_prompt_shown` | Historical (Aug 2026): the share-moment dialog opened. The dialog was removed (its unfurl-image preview lost its purpose when plan unfurls went text-only); Share plan link now copies directly. The event no longer fires but stays documented for old GA data. | `surface` (`post_publish` \| `plan_page`) | GA |
| `share_sheet_opened` | Historical (Aug 2026): fired when the native share sheet was invoked from the share moment. The sheet button was removed when the share moment went single-action, and the whole dialog was removed later that month; the event no longer fires but stays documented for old GA data. | `surface` (`post_publish` \| `plan_page`) | GA |
...
| `signup_completed` | Server: a signup completes. For password and plan-signup this is the `email_verified_at` NULL to set transition. For Google it CANNOT be, because the account row is created by the web layout's `getOrCreateAppUser` with `email_verified_at` already set, so nothing is left to flip by the time `mark-oauth` runs; that endpoint records it when the account was created within `SIGNUP_COMPLETED_GRACE_MS` (1 hour) or when the call itself verified an existing unverified account. Once per user. The pre-existing GA event with the same name fires earlier, at classic-signup account creation; the two measure different moments and only the first-party one is used in funnel math. | `method` ("password" \| "google" \| "plan_signup") | First-party (plus the older GA event at account creation with `has_hobbies`, `has_location`) |
