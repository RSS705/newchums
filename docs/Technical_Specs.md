...
- `mtg_email_log (user_id, set_id, email_type, period)` is the one-email-per-event guarantee (`welcome`, `lock_warning`, and later `revealed`, `weekly`, `results`; `period` is the ISO week for weekly emails).
...
**Season badges, on-track refresh and the weekly email (Batch 7, no migration).**
...
- **Weekly standings** (`processMtgWeeklyEmails(sql, env, { now?, set? })`, template `mtgWeekly.html/.txt`, subject `mtgWeekly`, outbox kind `mtg_weekly`, log type `weekly`): `mtgWeeklyStandingsTimes(set)` (in `mtg.ts`, shared with `mtgTimeline`) lists 10 AM ET on each Tuesday after the first standings, stopping six days before `final_at`; `mtgWeeklyStandingsDue(set, latestSnapshotDate, now)` returns `{ period, sendAt }` on such a Tuesday from 10 AM ET once that day's snapshot exists, or from 8 PM ET with any snapshot, where `period` is the Tuesday's Eastern date. One set-based statement logs every member of an active challenge community whose entry has a score on the latest snapshot and isn't logged for the period (up to 2,000 per run) and queues rows for those with the `mtg_challenge` preference on, with `group_key = 'mtg:<code>:weekly:<period>'` and `sendBy` a day after queueing. The outbox's Resend idempotency key for `mtg_weekly` adds the `group_key`, since every week sends one. At delivery `mtgWeeklyDetails` reads the latest snapshot and the one at least seven days older (or the first, flagged as "since the first standings"), once per pass in a cache with each group's `rankGroupDay` standings (keyed by set and group), then in one query the player's points, groups, ranked picks and on-track badges (every query is a subrequest, and a batch of emails shares one run), giving each group's top three with the player's rank and movement, the best and worst pick by Card Score, and the on-track badges (group badges named "in <group>"); it returns null, and the row is marked `gave_up`, when the player has no points on the latest day or no challenge group. `POST /admin/mtg/sets/:code/weekly-email { now? }` (super admins) runs the job for that set and then the challenge outbox pass; `now` is honoured only outside production. The hourly cron runs the weekly step after `processMtgRevealedEmails` in its own try/catch. `processEmailOutbox` now types its context as `{ waitUntil }`, so a request can call it.
- `mtgTimeline`'s weekly entries are labelled "Weekly standings" again and describe the email.
