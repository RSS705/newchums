...
- `newchums.users.share_link_modal_dismissed` (migration 062), `BOOLEAN NOT NULL DEFAULT false`; orphaned. The share-link first-use modal was replaced by the share moment (Aug 2026) and the endpoint/read that used this column are gone. Drop the column in the next migration sweep.
