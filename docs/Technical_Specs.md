...
- `newchums.users.is_hidden_shoutouts` (migration 074), `BOOLEAN NOT NULL DEFAULT false`. Section-level visibility toggle for the public-profile shout-outs section. When true, the Shout-outs section is hidden from non-owner viewers on `/u/<handle>`; the owner still sees the section in a dimmed "Hidden from visitors" preview state with an inline "Show on my public profile" button so they can reverse the toggle without leaving the page. Settable via Settings → Privacy ("Hide shout-outs from my public profile") and via the inline owner-only control on the public profile itself. Both controls write through `PUT /profile`. Replaces the previous private "Shout-outs received" section on `/profile` and the `GET /profile/shoutouts` endpoint, both of which were removed when this column shipped.
