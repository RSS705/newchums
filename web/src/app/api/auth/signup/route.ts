...
export async function POST(request: Request) {
  try {
    const { email, password, name, username: rawUsername } = (await request.json()) as {
      email?: string;
      password?: string;
      name?: string;
      username?: string;
    ...
    await sql`
      INSERT INTO users (email, name, username, username_norm, password_hash)
      VALUES (${normalizedEmail}, ${normalizedName}, ${usernameDisplay}, ${usernameNorm}, ${passwordHash})
    `;
    ...
    if (isMissingColumn) {
      ...
      console.error(
        "Signup error: username/username_norm column missing. Apply migrations 003 and 004. Raw:",
        err
      );
